...
function &fDefineQry(&$db, $pQry=false){
    ...
    if ($pQry) $slSql .= " WHERE com_tipocomp IN (select com_tipoComp from genclasetran where cla_aplicacion = 'IN') AND "  . $pQry ;
...
function before_group_BODEG (&$rpt, &$group) {
    ...
    $rpt->pdf->y-=14;
    $rpt->pdf->eztext($slText, 14, array('justification'=>'LEFT', 'leading'=>12));//        Putting text before group data
    }
