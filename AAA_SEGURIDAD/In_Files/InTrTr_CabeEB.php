 ...
 *      @rev    fah 23/11/14    Se agregan los campos Supervisor, Contenedor y Numero de Unidades      @author Antonio Rodriguez
 **/
...
//Class_Initialize Event @70-7B995C26
    function clsRecordInTrTr_comp()
    {
        ...
        if($this->Visible)
        {
            ...
            $this->com_TipoComp = new clsControl(ccsTextBox, "com_TipoComp", "Tipo de Comprobante", ccsText, "", CCGetRequestParam("com_TipoComp", $Method));
                $this->com_Order = new clsControl(ccsTextBox, "com_Order", "Orden", ccsText, "", CCGetRequestParam("com_Order", $Method));
            $this->com_OrderOwn = new clsControl(ccsTextBox, "com_OrderOwn", "Dueno de la orden", ccsText, "", CCGetRequestParam("com_OrderOwn", $Method));
            $this->com_NumComp = new clsControl(ccsTextBox, "com_NumComp", "N�mero de Comprobante", ccsInteger, "", CCGetRequestParam("com_NumComp", $Method));
            ...
            $this->com_CodReceptor = new clsControl(ccsTextBox, "com_CodReceptor", "C�digo del Receptor", ccsText, "", CCGetRequestParam("com_CodReceptor", $Method));
            $this->com_Valor = new clsControl(ccsTextBox, "com_Valor", "Valor de Transacci�n", ccsText, "", CCGetRequestParam("com_Valor", $Method));
    ...
    "FROM genparametros";
            ...
            global $DBdatos;
            //$parCategoria = CCGetDBValue("SELECT par_Categoria FROM genparametros WHERE par_ClaVE  = 'CAUTI' and par_Secuencia = '92'" , $DBdatos);
                        
            $this->com_Supervisor = new clsControl(ccsListBox, "com_Supervisor", "Com Supervisor", ccsInteger, "", CCGetRequestParam("com_Supervisor", $Method));
            $this->com_Supervisor->DSType = dsTable;
            list($this->com_Supervisor->BoundColumn, $this->com_Supervisor->TextColumn, $this->com_Supervisor->DBFormat) = array("per_CodAuxiliar", "per_Nombre", "");
            $this->com_Supervisor->ds = new clsDBdatos();
            //$this->com_Supervisor->ds->SQL = "SELECT per_CodAuxiliar, concat( per_Apellidos,' ', per_Nombres ) as per_Nombre  " .
            //"FROM conpersonas INNER JOIN concategorias on conpersonas.per_codauxiliar = concategorias.cat_codauxiliar WHERE cat_Categoria = '".$parCategoria."'";            
            ...
            $this->com_Supervisor->Required = true;
            /**/
            ...
            $this->com_Contenedor = new clsControl(ccsTextBox, "com_Contenedor", "Com Contenedor", ccsText, "", CCGetRequestParam("com_Contenedor", $Method));
            $this->com_Contenedor->Required = true;
            ...
            $this->com_NumUnit = new clsControl(ccsTextBox, "com_NumUnit", "Com Num Unit", ccsText, "", CCGetRequestParam("com_NumUnit", $Method));
            $this->com_NumUnit->Required = true;
...
"FROM genparametros";
            ...
            if(!$this->FormSubmitted) {
                ...
                if(!is_array($this->com_Order->Value) && !strlen($this->com_Order->Value) && $this->com_Order->Value !== false)
                $this->com_Order->SetText("");
                if(!is_array($this->com_OrderOwn->Value) && !strlen($this->com_OrderOwn->Value) && $this->com_OrderOwn->Value !== false)
                $this->com_OrderOwn->SetText("");
...
//Validate Method @70-2C462403
    function Validate()
    {
        ...
        $Validation = ($this->com_Order->Validate() && $Validation);
        $Validation = ($this->com_OrderOwn->Validate() && $Validation);
        ...
        //$Validation = ($this->com_Supervisor->Validate() && $Validation);
        $Validation = ($this->com_NumRetenc->Validate() && $Validation);
        //$Validation = ($this->com_Contenedor->Validate() && $Validation);
        $Validation = ($this->com_RefOperat->Validate() && $Validation);
        //$Validation = ($this->com_NumUnit->Validate() && $Validation);
...
//CheckErrors Method @70-612C24D1
    function CheckErrors()
    {
        ...
        $errors = ($errors || $this->com_Order->Errors->Count());
        $errors = ($errors || $this->com_OrderOwn->Errors->Count());
        ...
        $errors = ($errors || $this->com_Supervisor->Errors->Count());
        $errors = ($errors || $this->com_NumRetenc->Errors->Count());
        $errors = ($errors || $this->com_Contenedor->Errors->Count());
        $errors = ($errors || $this->com_RefOperat->Errors->Count());
        $errors = ($errors || $this->com_NumUnit->Errors->Count());
...
//InsertRow Method @70-02BF5213
    function InsertRow()
    {
        ...
        $this->ds->com_Order->SetValue($this->com_Order->GetValue());
        $this->ds->com_OrderOwn->SetValue($this->com_OrderOwn->GetValue());
        ...
        $this->ds->com_Supervisor->SetValue($this->com_Supervisor->GetValue());
        $this->ds->com_Libro->SetValue($this->com_Libro->GetValue());
        $this->ds->com_Contenedor->SetValue($this->com_Contenedor->GetValue());
        $this->ds->com_NumRetenc->SetValue($this->com_NumRetenc->GetValue());
        $this->ds->com_NumUnit->SetValue($this->com_NumUnit->GetValue());
...
//Show Method @70-2E9C2339
    function Show()
    {
            ...
            if($this->Errors->Count() == 0)
            {
                ...
                else if($this->ds->next_record())
                {
                    ...
                    if(!$this->FormSubmitted)
                    {
                        ...
                        $this->com_Order->SetValue($this->ds->com_Order->GetValue());
                        $this->com_OrderOwn->SetValue($this->ds->com_OrderOwn->GetValue());
                        ...
                        $this->com_Supervisor->SetValue($this->ds->com_Supervisor->GetValue());
                        $this->com_NumRetenc->SetValue($this->ds->com_NumRetenc->GetValue());
                        $this->com_Contenedor->SetValue($this->ds->com_Contenedor->GetValue());
                        $this->com_RefOperat->SetValue($this->ds->com_RefOperat->GetValue());
                        $this->com_NumUnit->SetValue($this->ds->com_NumUnit->GetValue());
        ...
        if($this->FormSubmitted || $this->CheckErrors()) {
            ...
            $Error .= $this->com_Supervisor->Errors->ToString();
            $Error .= $this->com_NumRetenc->Errors->ToString();
            $Error .= $this->com_Contenedor->Errors->ToString();
            $Error .= $this->com_RefOperat->Errors->ToString();
            $Error .= $this->com_NumUnit->Errors->ToString();
            ...
            $Error .= $this->com_Order->Errors->ToString();
            $Error .= $this->com_OrderOwn->Errors->ToString();
...
//$dias_diferencia = $segundos_diferencia / (60 * 60 * 24); 
        ...
        $this->com_Supervisor->Show();
        $this->com_NumRetenc->Show();
        $this->com_Contenedor->Show();
        $this->com_RefOperat->Show();
        $this->com_NumUnit->Show();
...
//Class_Initialize Event @70-B479E5A5
    function clsInTrTr_compDataSource()
    {
        ...
        $this->com_Order = new clsField("com_Order", ccsText, "");
        $this->com_OrderOwn = new clsField("com_OrderOwn", ccsText, "");
...
//Open Method @70-23DA9141
    function Open()
    {
        ...
        $this->SQL = "SELECT    com_TipoComp, 
                ...
                cla_Formulario, cla_Informe,        cla_TipoEmisor,     cla_TipoReceptor,   conpersonas.*,
                concat_ws(' ',per_Apellidos, per_Nombres) AS txtReceptor,  
                com_Supervisor, com_Contenedor, com_NumUnit, com_Order, com_OrderOwn " .
...
//SetValues Method @70-F4D799B9
    function SetValues()
    {
        ...
        $this->com_Order->SetDBValue($this->f("com_Order"));
        $this->com_OrderOwn->SetDBValue($this->f("com_OrderOwn"));
        ...
        $this->com_Supervisor->SetDBValue(trim($this->f("com_Supervisor")));
        $this->com_NumRetenc->SetDBValue(trim($this->f("com_NumRetenc")));
        $this->com_Contenedor->SetDBValue(trim($this->f("com_Contenedor")));
        $this->com_RefOperat->SetDBValue(trim($this->f("com_RefOperat")));
        $this->com_NumUnit->SetDBValue(trim($this->f("com_NumUnit")));
...
//Insert Method @70-A3776B6C
    function Insert()
    {
        ...
        $this->cp["com_Order"] = new clsSQLParameter("ctrlcom_Order", ccsText, "", "", $this->com_Order->GetValue(), "", false, $this->ErrorBlock);
        $this->cp["com_OrderOwn"] = new clsSQLParameter("ctrlcom_OrderOwn", ccsText, "", "", $this->com_OrderOwn->GetValue(), "", false, $this->ErrorBlock);
...
//die();
        ...
        $this->cp["com_Supervisor"] = new clsSQLParameter("ctrlcom_Supervisor", ccsInteger, "", "", $this->com_Supervisor->GetValue(), 0, false, $this->ErrorBlock);
        $this->cp["com_Contenedor"] = new clsSQLParameter("ctrlcom_Contenedor", ccsText, "", "", $this->com_Contenedor->GetValue(), " ", false, $this->ErrorBlock);
        $this->cp["com_NumUnit"] = new clsSQLParameter("ctrlcom_NumUnit", ccsText, "", "", $this->com_NumUnit->GetValue(), " ", false, $this->ErrorBlock);
        ...
        $this->SQL = "INSERT INTO concomprobantes ("
             ...
             . $this->ToSQL($this->cp["com_TipoComp"]->GetDBValue(), $this->cp["com_TipoComp"]->DataType) . ", "
                . $this->ToSQL($this->cp["com_Order"]->GetDBValue(), $this->cp["com_Order"]->DataType) . ", "
                . $this->ToSQL($this->cp["com_OrderOwn"]->GetDBValue(), $this->cp["com_OrderOwn"]->DataType) . ", "
             ...
             . $this->ToSQL($this->cp["com_Supervisor"]->GetDBValue(), $this->cp["com_Supervisor"]->DataType) . ", "
             . $this->ToSQL($this->cp["com_Contenedor"]->GetDBValue(), $this->cp["com_Contenedor"]->DataType) . ", "
             . $this->ToSQL($this->cp["com_NumUnit"]->GetDBValue(), $this->cp["com_NumUnit"]->DataType)
...
//Update Method @70-A829AA99
    ...
    {  
        ...
        $this->cp["com_Order"] = new clsSQLParameter("ctrlcom_Order", ccsText, "", "", $this->com_Order->GetValue(), "", false, $this->ErrorBlock);                
        $this->cp["com_OrderOwn"] = new clsSQLParameter("ctrlcom_OrderOwn", ccsText, "", "", $this->com_OrderOwn->GetValue(), "", false, $this->ErrorBlock);                
        $this->cp["com_NumUnit"] = new clsSQLParameter("ctrlcom_NumUnit", ccsInteger, "", "", $this->com_NumUnit->GetValue(), "", false, $this->ErrorBlock);
...
//             . "com_EstOperacion=" . $this->ToSQL($this->cp["com_EstOperacion"]->GetDBValue(), $this->cp["com_EstOperacion"]->DataType) . ", "
             ...
             . "com_NumPeriodo=" . $this->ToSQL($this->cp["com_NumPeriodo"]->GetDBValue(), $this->cp["com_NumPeriodo"]->DataType). ", "
                . "com_Order=" . $this->ToSQL($this->cp["com_Order"]->GetDBValue(), $this->cp["com_Order"]->DataType) . ", "
                . "com_OrderOwn=" . $this->ToSQL($this->cp["com_OrderOwn"]->GetDBValue(), $this->cp["com_OrderOwn"]->DataType) . ", "
                . "com_NumUnit=" . $this->ToSQL($this->cp["com_NumUnit"]->GetDBValue(), $this->cp["com_NumUnit"]->DataType) ;   
                //. "com_Supervisor=" . $this->ToSQL($this->cp["com_Supervisor"]->GetDBValue(), $this->cp["com_Supervisor"]->DataType) . ", "
             //. "com_Contenedor=" . $this->ToSQL($this->cp["com_Contenedor"]->GetDBValue(), $this->cp["com_Contenedor"]->DataType) . ", "
             //. "com_NumUnit=" . $this->ToSQL($this->cp["com_NumUnit"]->GetDBValue(), $this->cp["com_NumUnit"]->DataType);
        var_dump($this->SQL);
/*
...
//Delete Method @70-6001A975
    function Delete()
    {
        ...
        if($this->Errors->Count() > 0)
            $this->Errors->AddError($this->Errors->ToString());
        $this->SQL = "DELETE FROM invenlace";
        $wp->Criterion[1] = $wp->Operation(opEqual, "enl_RegNumero", $wp->GetDBValue("1"), $this->ToSQL($wp->GetDBValue("1"), ccsInteger),false);
        $Where = $wp->Criterion[1];
        $this->SQL = CCBuildSQL($this->SQL, $Where, "");        
        $this->query($this->SQL);        
        $this->close();
...
$FileName = "InTrTr_CabeEB.php";
$Redirect = "";
$TemplateFileName = "InTrTr_CabeEB.html";
$BlockToParse = "main";
$PathToRoot = "../";
//End Initialize Page
...
// Events
include("./InTrTr_Cabe_eventsEB.php");
BindEvents();
