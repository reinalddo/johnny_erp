...
//Class_Initialize Event @59-C3DC2ED4
    function clsEditableGridItem_lista()
    {
        ...
        $this->act_General = new clsControl(ccsTextBox, "act_General", "act_General", ccsText, "");        
        $this->act_Stock = new clsControl(ccsTextBox, "act_Stock", "act_Stock", ccsText, "");
...
//GetFormParameters Method @59-F2C33504
    function GetFormParameters()
    {
        for($RowNumber = 1; $RowNumber <= $this->TotalRows; $RowNumber++)
        {
            ...
            $this->FormParameters["act_Stock"][$RowNumber] = CCGetFromPost("act_Stock_" . $RowNumber);
            $this->FormParameters["act_General"][$RowNumber] = CCGetFromPost("act_General_" . $RowNumber);
...
//Validate Method @59-0F8F514C
    function Validate()
    {
        ...
        for($RowNumber = 1; $RowNumber <= $this->TotalRows; $RowNumber++)
        {
            ...
            $this->act_Stock->SetText($this->FormParameters["act_Stock"][$RowNumber], $RowNumber);
            $this->act_General->SetText($this->FormParameters["act_General"][$RowNumber], $RowNumber);
...
//ValidateRow Method @59-6309A282
    function ValidateRow($RowNumber)
    {
        ...
        $Validation = ($this->act_General->Validate() && $Validation);
        $Validation = ($this->act_Stock->Validate() && $Validation);
        ...
        if(!$Validation)
        {
            ...
            $errors .= $this->act_General->Errors->ToString();
            $errors .= $this->act_Stock->Errors->ToString();
            ...
            $this->act_Stock->Errors->Clear();
            $this->act_General->Errors->Clear();
...
//CheckInsert Method @59-22A84F80
    function CheckInsert($RowNumber)
    {
        ...
        $filed = ($filed || strlen($this->FormParameters["act_General"][$RowNumber]));
        $filed = ($filed || strlen($this->FormParameters["act_Stock"][$RowNumber]));
            ...
            if($is_next_record || ($EmptyRowsLeft && $this->InsertAllowed))
            {
                do
                {
                    ...
                    if(!$this->FormSubmitted && $is_next_record) {
                        ...
                        $this->act_General->SetValue($this->ds->act_General->GetValue());
                        $this->act_Stock->SetValue($this->ds->act_Stock->GetValue());
                        ...
                        $this->act_General->SetText("");
                        $this->act_Stock->SetText("");
                    ...
                    $this->act_General->Show($RowNumber);
                    $this->act_Stock->Show($RowNumber);
...
//Class_Initialize Event @59-27AD9AF6
    function clsItem_listaDataSource()
    {
        ...
        $this->act_Stock = new clsField("act_Stock", ccsText, "");
        $this->uni_Abreviatura = new clsField("uni_Abreviatura", ccsText, "");
        $this->act_General = new clsField("act_General", ccsText, "");
...
//Open Method @59-B2B51B65
    function Open()
    {
        ...
        $Method = $this->FormSubmitted ? ccsPost : ccsGet;
        $pEmisor = CCGetRequestParam("pEmisor", $Method);
        ...
        $slSql = "SELECT act_CodAuxiliar, act_codanterior, cat_Activo, uni_Descripcion, CONCAT(act_Descripcion,' ' , act_descripcion1)AS act_descrip, act_UniMedida, 
1 AS act_Multiplicador, 1 AS act_Divisor, CONCAT('../Co_Files/CoAdAc_mant2.php?act_CodAuxiliar=', act_codauxiliar) AS act_pagina, 
IFNULL(pre_PreUnitario,0) AS tmp_unitario, stock.act_stock AS act_Stock, general.act_general AS act_General 
FROM conactivos 
LEFT JOIN (SELECT IFNULL(SUM(det_CantEquivale*pro_signo),0) AS act_stock, act_codauxiliar AS act_codauxiliar_stock  FROM invprocesos JOIN
concomprobantes ON pro_codproceso = 1 AND com_tipocomp = cla_tipotransacc
            	...
            	WHERE com_emisor = '" . $pEmisor . "'
            	GROUP BY act_codauxiliar) AS stock ON conactivos.`act_CodAuxiliar` = stock.act_codauxiliar_stock
LEFT JOIN (SELECT IFNULL(SUM(det_CantEquivale*pro_signo),0) AS act_general, act_codauxiliar AS act_codauxiliar_general  FROM invprocesos JOIN
concomprobantes ON pro_codproceso = 1 AND com_tipocomp = cla_tipotransacc 
            	JOIN invdetalle ON det_regnumero = com_regnumero
            	JOIN conactivos ON act_codauxiliar = det_coditem           	
            	GROUP BY act_codauxiliar) AS general ON conactivos.`act_CodAuxiliar` = general.act_codauxiliar_general
...
LEFT JOIN invprecios ON pre_lisprecios = cla_lisprecios AND pre_coditem = act_codauxiliar" . $slCondAct ;
        
        
        
       /* $slSql     = "SELECT act_CodAuxiliar, act_codanterior, cat_Activo, uni_Descripcion,
                           ...
                           ifnull(pre_PreUnitario,0) as tmp_unitario, act_CodAuxiliar AS act_Stock, act_CodAuxiliar AS act_General
                        FROM conactivos
                	INNER JOIN concategorias ON concategorias.cat_CodAuxiliar = conactivos.act_CodAuxiliar
                	INNER JOIN genunmedida ON conactivos.act_UniMedida = genunmedida.uni_CodUnidad 
                        LEFT JOIN genclasetran ON cla_tipocomp = '" . $pTip . "' 
...
//SetValues Method @59-7D603572
    function SetValues()
    {
        ...
        $this->act_Stock->SetDBValue($this->f("act_Stock"));
        $this->act_General->SetDBValue($this->f("act_General"));
