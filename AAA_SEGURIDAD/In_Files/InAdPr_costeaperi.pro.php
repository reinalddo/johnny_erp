...
*   @param      integer		pQryTar  Condicion de busqueda
...
*   access  public
*   @param   object   Referencia a la conexion de BD
...
*   SEC:    Secuencia de cada invdetalle
   ...
   $alSql[] = "CREATE TEMPORARY TABLE tmp_invmovs
           SELECT
            ...
            FROM	concomprobantes 
                 JOIN genclasetran ON cla_tipocomp = com_tipocomp AND cla_contabilizacion = 1    /*  JVL AND cla_contabilizacion = 1   */
                 LEFT JOIN invprocesos  ON pro_codproceso = 1 AND cla_tipotransacc = com_tipocomp 
   ...
   $alSql[] = "INSERT INTO tmp_invmovs
           SELECT
            ...
            FROM	invprocesos JOIN
                genclasetran ON cla_tipocomp = cla_tipotransacc AND cla_contabilizacion = 1    /*  JVL AND cla_contabilizacion = 1   */ 
...
//
    ...
    $iteDbg     = fGetParam('pIteDbg', true);        // Item a Depurar, setear false a true para salida a pantalla
...
*/
    ...
    if ($dbg) echo "<html><head><style>.td {border:1px solid black;}</style></head>
                    ...
                    <tr style='border:1px solid black;'>
                        <td>TIPO TR</td><td>NUM</td><td>ORD</td><td>FECHA</td><td>ITEM</td><td>CANTIDAD</td>
...
// echo "<br> $rec['ITE']  $salCant  $salValo  TRAN: $cosUnit   $rec['CTR']";
                ...
                if ($dbg && $rec['ITE'] == $iteDbg ) echo "<tr><td>" .
                    ...
                    $ultCost . "</td><td>" . ($salCant <>0? $salValo / $salCant :'-0-') . "</td><tr>";
        }
        if ($recno > 0) { 
