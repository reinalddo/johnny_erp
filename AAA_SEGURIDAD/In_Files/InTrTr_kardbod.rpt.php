...
function &fDefineQry(&$db){
    ...
    global $giGeneral;
    global $giTipPro;
    $dlFecIni = nz(fDBValor($db, 'genparametros', "par_valor1", "par_clave = 'ININIC' "),'2010-01-01'); // @fah230108: fecha inicial de procesamiento de inventario
   ...
   $alSql[] = "CREATE TEMPORARY TABLE tmp_invmovs
		...
		000000000.00  as SUN,		
/* JVL         det_cantequivale * pro_signo  as SAC,   */
            	det_cantequivale * ifnull(pro_signo,00000.00)  as SAC,		
/* jvl            	det_cosTotal * pro_signo      as VAC,   */
            	det_cosTotal * ifnull(pro_signo,0000000.0000)      as VAC,
...
//            ORDER BY 1,5,2,3
   ...
   $alSql[] = "INSERT INTO tmp_invmovs
             SELECT com_emisor as 'BOD',
            	...
            	000000000.00  as VEG,
		000000000.00  as SUN,
/* JVL            	SUM(det_cantequivale * pro_signo)  as SAC,   */
            	SUM(det_cantequivale * ifnull(pro_signo,00000.00))  as SAC,
/* JVL            	SUM(det_cosTotal * pro_signo)      as VAC,         */
            	SUM(det_cosTotal * ifnull(pro_signo,0000000.0000))      as VAC,
...
$giTipPro  = fGetParam("pTipPro", 1);	// Codigo del proceso a ejecutar JVL - invprocesos.pro_codproceso = 1 KARDEX
