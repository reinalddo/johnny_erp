<?php
/*
*	@rev fah Jun29/05: garantizar que se grabe el valor default del emisor, cuando asi lo requiera la transacc.
*	@rev fah Sep29/05: garantizar que se actualize las transacc asociadas al modificar fechas y/o productores
* 	@rev fah 10/06/09: Incorporar restricciones en tipos de comprobante segun perfil ($_SESSION[IntrTr][ACC])
*   @rev    fah 10/09/09    Aplicar Restriccion de Duracin de comprobantes
*   @rev    fah 26/01/10    Cambiar validaciones de fechas:  fecvencim >= feccontab >= fectrans
*   @rev    esl	12/02/11	Comentar asignacion de numero de secuencial para el proceso de ingreso	de comprobante - Se lo va a asignar via trigger
*   @rev    esl	12/02/11	Excluir del proceso de grabar en segbitacora el ingreso de un nuevo comprobante, se lo va a realizar en el trigger trComprobantesBi
* 	*/
...
//													#fah10/06/09 Aplicar restricciones */
	switch($goTrans->f("cla_aplicacion")){
	    case "IN":
		$lsModulo = "InTrTr";
		break;
	    case "CO":
	...
	$lsRestri = $_SESSION["restr"][$lsModulo]["ACC"];
	$InTrTr_comp->com_Emisor->SetValue($lsRestri);
	if (strlen($lsRestri) > 0 ){
		if (strlen($slWherePer ) > 0 ) $slWherePer .= " AND " . " per_codauxiliar in (" . $lsRestri  . ")" ;
		else $slWherePer = " WHERE " . " per_codauxiliar in (" . $lsRestri  . ")" ;
	}
	$InTrTr_comp->com_Emisor->ds->SQL = " SELECT concategorias.cat_categoria, per_codauxiliar as cod , concat(per_Nombres, ' ', per_apellidos) as nomb " .
		" FROM conpersonas INNER JOIN concategorias on conpersonas.per_codauxiliar = concategorias.cat_codauxiliar " . $slWherePer;
/*echo "<br> <br> <br> SESION:";print_r($_SESSION);
echo "<br> <br> <br> SESION-Restr:";print_r($_SESSION["restr"]);
echo "<br> <br> <br> SESION-Restr-In:";print_r($_SESSION["restr"]["InTrTr"]);
echo "<br> <br> <br> SESION-Restr-In:";print_r($_SESSION["restr"]["InTrTr"]["ACC"]);
echo "<br> Restr:". $lsRestri ;
echo "<br>Modulo:". $lsModulo . " / " . $goTrans->f("cla_aplicacion");
echo "<br>". $InTrTr_comp->com_Emisor->ds->SQL ;*/
/*									//# fah 10/06/09 Emisor y receptor DEBEN SER PERSONAS!!!!!!!!!!!!!
		$InTrTr_comp->com_Emisor->ds->SQL = "SELECT concategorias.cat_categoria, act_codauxiliar as cod, concat(act_descripcion, ' ', act_descripcion1) as nomb " .
...
//			$InTrTr_comp->com_Emisor->SetValue($DBdatos->f("cla_EmiDefault"));
		...
		$Tpl->SetVar("gEmpre",    		  $_SESSION["g_empr"]);
		}
...
function InTrTr_comp_OnValidate()
{
	...
	if (!$InTrTr_comp->EditMode) {
		if  (!($InTrTr_comp->com_NumComp->GetValue() > 0))  {
			$InTrTr_comp->Errors->addError("Debe Definir un N�mero de Comprobante");
		}
	}
...
//	print_r($alFecCon);
	$ilFecCon=$alFecCon[0];	
	$ilFecTra=$alFecTra[0];
 	//if ($ilFecCon > $ilFecTra)    #fah 26/01/10  
	//	$InTrTr_comp->Errors->addError("La Fecha de Vencimiento Debe ser mayor que la fecha DE TRANSACCION");
 	if ($ilFecCon < $ilFecTra) 
		$InTrTr_comp->Errors->addError("La Fecha CONTABLE DEBE SER POSTERIOR A LA FECHA DE EMISION");
...
//echo $tlSql;
	...
	if (!$rsPer) {
		...
		if ($liEstado < 0 or $liestado > 95) {
			$InTrTr_comp->Errors->addError("La Fecha de Contabilizaci�n Est� en un Per�odo Cerrado");
		$InTrTr_comp_com_FecContab_OnValidate = false;
			return $InTrTr_comp_OnValidate;
		}
		$InTrTr_comp->com_NumPeriodo->SetValue($DBdatos->f("per_PerContable"));
	}*/
	$slCompSql = "com_TipoComp = '" . $InTrTr_comp->com_TipoComp->GetValue() . "'  AND com_Numcomp = " . $InTrTr_comp->com_NumComp->GetValue();
    ...
    if ($InTrTr_comp->com_NumComp->GetValue() == 0) { // Si viene un cero com numero de comprobte
    		/*$tlSql = " SELECT max(com_NumComp) + 1 as secue ".
    				 ...
    				 " WHERE com_TipoComp = '" . $InTrTr_comp->com_TipoComp->GetValue() . "' ";
    		$DBdatos->Query($tlSql);
    		$rsPer = $DBdatos->Next_Record();
	    ...
	    $InTrTr_comp->com_NumComp->SetValue(0);
            $InTrTr_comp_OnValidate = true;
    }
