...
function &fDefineQry(&$db, $pQry=false){
    ...
    $slSql = "SELECT
                tac_semana as SEM,
                tad_numtarja AS TAR,
                LEFT(concat(left(per_apellidos, 30 - length(left(per_nombres,12))) , ' ' , per_nombres), 30)  AS NOM,
                concat(left(buq_abreviatura,8), ' - ', emb_numviaje) AS  VAP,
                left(act_descripcion,15)  AS PRO,
                left(par_descripcion, 10)  AS MAR,
...
$rep->rptOpt = array('fontSize'=>8, 'titleFontSize' => 2, 'showHeadings'=>0,'shaded'=>1, 'splitRows'=>0,
                      'shaded'=>0, 'showLines'=>0, 'maxWidth'=>990, 'xPos'=>0, 'xOrientation'=>'right', 'innerLineThickness'=>0);
