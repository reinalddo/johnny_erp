<?php
/*
*   LiEmTj_detembcnt.rpt.php: Listado de Tarjas por Embarque y Contenedor
...
function &fDefineQry(&$db, $pQry=false){
    ...
    $slSql = "SELECT
                ...
                tac_contenedor    as CNT,
                act_Abreviatura AS PRO,
                ma.par_Valor2 AS MAR,
                ...
                if (tad_difunitario >= 0, tad_difunitario, 00000.0000) as DIF,
             ...
             FROM liqtarjadetal JOIN liqtarjacabec ON tar_NUmTarja = tad_NumTarja
                ...
                JOIN genparametros ma ON par_clave = 'IMARCA' AND par_secuencia = tad_codmarca
                "
             ;
...
function before_group_SEM (&$rpt, &$group) {
    global $db;
    $rpt->putText('SEMANA: ' . $group->currValue, 10,15);
...
function before_group_PRO (&$rpt, &$group) {
    global $db;
    global $glProTxt;
    $group->resume[0]['resume_text'] = 'SUB TOTAL ' . $group->currValue . ' --------------' ;
    }
...
function before_group_VAP (&$rpt, &$group) {
    ...
    } else $slVapor .= "-***** ERROR EN VAPOR *****-";
    $glVapTxt = $group->currValue . ' ' . $slVapor;
    $rpt->putText("                                       VAPOR: " . $slVapor, 10,25);
...
*   @return void
*/
function before_group_CNT(&$rpt, &$group){
    global $db;
    global $glProTxt;
    $rpt->putText($group->currValue , 10,25);
...
//--------------------------------------------------------------------------------------------------------------------
//
$slEmbSemIni = fGetParam('emb_SemInicio', false);
$slTacSemana = fGetParam('tac_Semana', false);
$slEmbVapor = fGetParam('emb_CodVapor', false);
$slQryTar   = fGetParam('pQryTar', "");
$slEmbSemIni = ($slEmbSemIni != false )? " emb_SemInicio = " . $slEmbSemIni : "";
$slTacSemana = ($slTacSemana != false )? " tac_Semana = " . $slTacSemana : "";
$slEmbVapor  = ($slEmbVapor != false )? " emb_CodVapor = " . $slEmbVapor : "";
$slQry= $slEmbSemIni . ((strlen($slEmbSemIni) > 0)? " AND " . $slTacSemana : "");
$slQry .= ((strlen($slQry) > 0 && strlen($slQryTar) )? " AND " : "") . $slQryTar;
$slQry .= ((strlen($slQry) > 0 && strlen($slEmbVapor) )? " AND " : "") . $slEmbVapor;
//echo $slQry;
//die();
...
$rep->title="DETALLE DE TARJAS POR CONTENEDOR";
//$rep->subTitle="-";
$rep->condition=$slTxt;
...
$rep->colOpt['PRO']['width']=60;
$rep->colOpt['VAP']['width']=50;
$rep->colOpt['MAR']['width']=120;
...
$rep->addGrp('SEM');                         // Create a group for column SEM
    ...
    $rep->addResumeLine('SEM','S', 'Total Semana ',0);
...
$rep->addGrp('VAP');
    ...
    $rep->addResumeLine('VAP','S', 'Total Vapor ',0);
...
$rep->addGrp('CNT');
    $rep->groups['CNT']->fontSize=6;
    $rep->groups['CNT']->textCol='NOM';
    $rep->addResumeLine('CNT','S', 'Total Contenedor-------',1);
        $rep->groups['CNT']->linesBefore=1;
        $rep->setAggregate('CNT',0, 'SEM','-');
        $rep->setAggregate('CNT',0, 'PRO','-');
        $rep->setAggregate('CNT',0, 'VAP','-');
        $rep->setAggregate('CNT',0, 'CNT','-');
        $rep->setAggregate('CNT',0, 'EMP','-');
        $rep->setAggregate('CNT',0, 'NOM','-');
        $rep->setAggregate('CNT',0, 'TAR','-');
        $rep->setAggregate('CNT',0, 'MAR','-');

$rep->addGrp('PRO');
    $rep->groups['PRO']->fontSize=6;
    $rep->groups['PRO']->textCol='NOM';
    $rep->addResumeLine('PRO','S', 'Sub Total--------------',0);
