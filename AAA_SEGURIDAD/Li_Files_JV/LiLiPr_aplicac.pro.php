...
//error_log ("Paso 1", 3, "/tmp/aaa-errors.log");
    ...
    $ilUltimoProd = false;
    $ilLiqSecue = NZ(fDBValor($db, 'liqsecuencias', 'ID'),1);//               Analiza la secuencia y decide utilizar el sigte numero usado o el generado
...
//                     " AND com_FecContab BETWEEN '" . $gdFecInicio . "' AND '" . $dlFecCierre . "'
    ...
    $alRubVirt = $db->getAll("SELECT par_valor1 AS par_Valor1,
				...
				par_valor3 AS par_Valor3
			     FROM genparametros WHERE par_clave = 'LGREX' ");
    $db->SetFetchMode(ADODB_FETCH_BOTH);
...
//echo "<br><br><br>";
		...
		$alDetalle['det_RegNumero']     = $ilRegNumero;
                ...
                $alDetalle['det_CanDespachada'] = round($rsLiquida->fields['tmp_CantEmbar'] *  ($flMultip ? $flMultip : 1),0); // El tercer parametro es un multiplicador de cantidad
...
//echo "<br>3 ";
		    ...
		    if ( $rsLiquida->fields['tmp_pagable'] == 0){ // #fah30/06/2010   Si el item NO es pagable  // #fah23/07/10
			$flPrecUnit = 0;
			$flCanti = 0;
		    } else {
...
//---- Definir si la liquidacion tiene una sola cabecera (LL) y un invdetalle y un condetalle �son  dos cabeceras (LI, LQ)
            fAgregaComprobante($db, 'LQ', $trj->TAD_LIQNUMERO, 0, $dlFechaLiq, $dlFechaLiq,
                                ...
                                'Liquidacion Sem ' . $ilSemana , 0, 1, 0, 0, $ilSemana, 5, 1,
                                $ilNumProces, 593, $slUsuario, $ilPeriodo, date("Y-m-d"));
