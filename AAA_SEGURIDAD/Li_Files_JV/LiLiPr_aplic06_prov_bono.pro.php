...
<body bgcolor="#fffff7" link="#000000" alink="#ff0000" vlink="#000000" text="#000000" style="font-size:10px; PADDING-RIGHT: 0px; PADDING-LEFT: 0px; FONT-SIZE: 11px; PADDING-BOTTOM: 0px; PADDING-TOP: 0px; magin: 0; spacing: 0" bottommargin="0" nowrap leftmargin="0" topmargin="17" rightmargin="0" marginwidth="0" marginheight="0">
<style type="text/css">
</style>
<?php
/** Proceso Post Liquidacion, paso 6
*   Genera un comprobante contable con el BONO DE PRECIO
...
*   @rev esl  10-abr-2012 leer DB2 de parametros generales->Liquidaciones->General-Datos--- LDATO
**/
...
//            echo str_pad(' ', 40*6, '&nbsp;') . $slProductor . str_repeat('&nbsp;',$p) . $lastrec['det_tipocomp'] . " - " . $ilNumComp .  "   Cheque: " . ($ilNumChq ) . "<br>";
            echo "&nbsp;&nbsp;&nbsp;" .
                $slProductor . "</td><td>" . (($rsdat['TT'] <> 0)? $lastrec['det_tipocomp'] . " - " . $ilNumComp  : " N.A. " ) . "</td><td> " .
                (($rsdat["TP"] == "TR")? "SPI": "CH") . "</td><td style='text-align:right'>&nbsp;" . number_format($rsdat['TT'] ,2,".",","). "</td></tr><tr><td>";
        if ($rsdat['TT'] <> 0) {
            $fTotal += $rsdat['TT'];
            $db->Execute("Delete FROM " . $DB2 . ".condetalle WHERE det_regnumero in " .
                            "(select com_regnumero from ". $DB2 . ".concomprobantes where com_tipocomp = 'PB' AND com_refOperat = $ilSemana AND com_codreceptor = " . $ilUltimoProd ." ) ");

            $db->Execute("Delete FROM " . $DB2 . ".concomprobantes where com_tipocomp = 'PB' AND com_refOperat = $ilSemana AND com_codreceptor = " . $ilUltimoProd  );
    ...
    echo "&nbsp;&nbsp;&nbsp; SUMATORIA:" .
                "</td><td>   ' '</td><td>  </td><td style='text-align:right'>&nbsp;" . number_format($fTotal ,2,".",","). "</td></tr><tr><td>";
    return true;
}
/*
...
//@TODO:  HAcer columnas a elegir dinamicas, abasadas en parametro de BD (liqprocesos).
    $slSql ="SELECT var_tipopago as TP,
                    ...
                    SUM(CASE WHEN var_tipopago <>  'TR' AND RUB_POSORDINAL  in (2) THEN tmp_valtotal ELSE 0 END)  AS 'D11',
		    SUM(CASE WHEN  var_tipopago = 'TR' AND RUB_POSORDINAL  in (2) THEN tmp_valtotal ELSE 0 END)  AS 'TR',
		    SUM(CASE WHEN  var_tipopago <> 'TR' THEN tmp_valtotal ELSE 0 END)  AS 'CH',
		    SUM(CASE WHEN  var_tipopago <>  'TR' THEN tmp_valtotal ELSE 0 END)  AS 'TT'
...
//$DB2 = fGetParam("pDB2", ""); /*@rev esl  10-abr-2012 leer DB2 de parametros generales->Liquidaciones->General-Datos--- LDATO*/
if (strlen($DB2) > 0) $DB2 .=  "";
...
/*@rev esl  10-abr-2012 leer DB2 de parametros generales->Liquidaciones->General-Datos--- LDATO
 *------------------------------------Base de Liquidacion----------------------------------------------*/
$rs2 = $db->Execute("SELECT IFNULL(par_valor1,'') BASELIQ FROM genparametros WHERE par_clave = 'LDATO'" );
if (!$rs2) fErrorPage('',"NO ESTA DEFINIDO EL PROCESO  " );
$rs2->MoveFirst();
$rec2 = $rs2->FetchNextObject();
$DB2=$rec2->BASELIQ;
//print_r($rec2);
//print("BaseLiq : ".$DB2."...");
/*-------------------------------------------------------------------------------------------------------*/
...
switch ($ilEstadoProc) {
    case 26:
        ...
        $ilInd = 1;
    case 6:
        echo "<br>PASO $ilEstadoProc.-  PROVISION DE BONO POR DIFERENCIA DE PRECIO :</font></td></tr><tr><td>";
        echo "<table width='70%' align='center' ><tr><td >NOMBRE</td ><td >COMPROB. NRO.</td ><td >TIPO PAGO</td ><td >VALOR</td ></tr ><tr ><td >";
        $pQry=fGetParam('pQryLiq', 0);
        set_time_limit (0) ;
        $gsTitGeneral="";

        $pQry=" liq_numProceso = " . $ilNumProces;
        ...
        $agPla['TAP'] = "";
        $agPla['AAP'] = "";
        if(!fContabiliza2($db, $rs, "PAGO DE BONO, SEM. " . $ilSemana)) $ilEstadoProc = -1;
