...
/* TOTALES POR LIQUIDACION */
	SELECT  5 AS tipo, c.com_CodReceptor, CONCAT(p.per_Apellidos,' ',p.per_Nombres) AS productor,
		...
		SUM(CASE WHEN li.liq_CodRubro = 23 THEN IFNULL(li.liq_ValTotal,0)*ru.rub_IndDbCr ELSE 0 END) AS R23,
		SUM(CASE WHEN li.liq_CodRubro = 24 THEN IFNULL(li.liq_ValTotal,0)*ru.rub_IndDbCr ELSE 0 END) AS R24,
		...
		SUM(CASE WHEN li.liq_CodRubro = 23 THEN IFNULL(li.liq_ValTotal,0)*ru.rub_IndDbCr ELSE 0 END)+
		SUM(CASE WHEN li.liq_CodRubro = 24 THEN IFNULL(li.liq_ValTotal,0)*ru.rub_IndDbCr ELSE 0 END)+
		...
		SUM(CASE WHEN li.liq_CodRubro = 23 THEN IFNULL(li.liq_ValTotal,0)*ru.rub_IndDbCr ELSE 0 END)+
		SUM(CASE WHEN li.liq_CodRubro = 24 THEN IFNULL(li.liq_ValTotal,0)*ru.rub_IndDbCr ELSE 0 END)+
		SUM(CASE WHEN li.liq_CodRubro = 22 THEN IFNULL(li.liq_ValTotal,0)*ru.rub_IndDbCr ELSE 0 END))AS TDesc,
		/* TOTAL */
		(SUM(CASE WHEN li.liq_CodRubro = 1 THEN IFNULL(li.liq_ValTotal,0)*ru.rub_IndDbCr ELSE 0 END)+
...
$sSql .= " UNION
	 SELECT  	1 AS tipo, t.tac_Embarcador, CONCAT(p.per_Apellidos,' ',p.per_Nombres) AS productor,
			 ...
			 0 AS R14, 0 AS R15, 0 AS R16, 0 AS R17, 0 AS R18, 0 AS R19, 0 AS R20, 0 AS R21, 0 AS R23,0 AS R24,0 AS R22, 0 AS RTotal, 0 AS RTotalFin, 0 AS TDesc,
			 0 AS TBenf, 
