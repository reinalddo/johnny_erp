<?php
/*
*   LiliTj_restar.php: Resumen de Tarjas valorizado, por Semana, Vapor, Marca, productor, precio
...
*/
    ...
    $slSql .= " GROUP BY 1,2,3,4,5,6,12  ORDER  BY 1,2,3,4,5,6,12";
...
function before_group_SEM (&$rpt, &$group) {
    global $db;
    $rpt->pdf->eztext('SEMANA:   ' . $group->currValue, 9, array('justification'=>'center', 'leading'=>15));//        Putting text before group data
...
function before_group_PRO (&$rpt, &$group) {
    ...
    $rpt->pdf->eztext("\n" . strtoupper($slProduc) , 9, array('justification'=>'center', 'leading'=>12, 'aleft' => $rpt->leftBorder));//        Putting text before group data
    $group->resume[0]['resume_text'] = substr('SUBTOT. ' . $slProduc, 0,20);
    }
...
function before_group_VAP (&$rpt, &$group) {
  ...
  $rpt->pdf->eztext("V A P O R : " . $slVapor ,9, array('justification'=>'left', 'leading'=>12, 'aleft' => $rpt->leftBorder));//        Putting text before group data
  $group->resume[0]['resume_text'] = substr('SUBTOT. ' . $slVapor,0,20);
  }
...
function before_group_MAR (&$rpt, &$group) {
    ...
    $rpt->pdf->eztext("M A R C A : ". $slMarca, 9, array('justification'=>'left', 'aleft' => $rpt->leftBorder));//        Putting text before group data
    $group->resume[0]['resume_text'] = substr("SUBTOT. " .$slMarca,0,20);
    }
...
$rep->title="RESUMEN DE TARJAS Y PRECIOS";
//$rep->subTitle="-";
