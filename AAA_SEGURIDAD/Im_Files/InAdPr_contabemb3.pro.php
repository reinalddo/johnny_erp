...
//-------------
   ...
   /* if (is_null($group->lastRec['GRU']) || empty($group->lastRec['GRU'])) {               // BYPASS
       fErrorPage('',$group->lastRec['DIT'] . " UN ITEM NO TIENE DEFINIDO EL 'TIPO DE COSTO' EN " . $lastrec['det_tipocomp'] . "-" . $group->lastRec['LIQ'] . "-" . $lastrec['det_secuencia']) ;
    }*/
...
//    $lastrec['det_idauxiliar']  = $group->lastRec['GRU'];
    ...
    if (strlen($lastrec['det_codcuenta']) > 0)  if (!fInsDetalleCont($db, $lastrec))
