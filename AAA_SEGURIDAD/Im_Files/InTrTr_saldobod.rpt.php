<?php
/*
*   REPORTES INV - SALDO DE BODEGA 2da COLUMNA 3era FILA
*   @date		07/Ago/2015    
*   @author    JVL
*   @asunto 	Query fue parametrizado para que respete las fechas de Inicio y Fin Seleccionadas
*/
...
function &fDefineQry(&$db){
    ...
    $slSql = "SELECT
		     ...
		     act_CodAnterior as 'UNI',
		     ' ' AS 'C99',
		     sum(if ( " . $slConI . " , det_cantequivale * pro_signo, 0)) as SAN,
		     sum(if ( " . $slConI . " , det_cosTotal * pro_signo, 0)) as VAN,
		     sum(if ( " . $slConD . " AND " . $slConF . " AND pro_signo = 1, det_cantequivale , 0)) as CIN,
		     sum(if ( " . $slConD . " AND " . $slConF . " AND pro_signo = 1, det_cosTotal , 0)) as VIN,
		     sum(if ( " . $slConD . " AND " . $slConF . " AND pro_signo = -1 , det_cantequivale , 0)) as CEG,
		     sum(if ( " . $slConD . " AND " . $slConF . " AND pro_signo = -1 , det_cosTotal , 0)) as VEG,
		     sum(if ( " . $slConF . " , det_cantequivale * pro_signo, 0)) as SAC,
		     sum(if ( " . $slConF . " , det_cosTotal * pro_signo, 0)) as VAC,
