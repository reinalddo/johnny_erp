...
*   @Rev    fah  Feb/07/07  Manejo correcto de saldo anterior en liquidacion, basado en la ultima liquidacion anterior, no en
*                           la semana de liquidacion que se ejecuta. Se Agrega tmp_ultSemana a tmp_product
*                           Manejo de politicas de cobro para cuentas de Inventario.
*   @rev	Ene/16/08   Parametrizacion de la semana inicial para liquidaciones (800)
*   @rev    fah	May/07/09   Manejar una tabla de descuentos obligatorio (liqdefaults) para ingresar manualmente descuentos sin depemnder de calculo o contabilidad
*   @rev    fah	10/06/09    Aplicar precio de lista (3) en el pago de inv a productores, no el promedio
*   @rev    fah	29/06/09    No aplicar valor minimo por liquidacion, es cero.
*   @rev    fah  Sep/02/09	Habilitar retenciones en la Fuente basadas en Valor Oficial Pagado (+ Bono), generado desde el detalle de tarjas (rubro TBASIMPO).\
*   @rev    fah  Feb/12/10	Habilitar Calculo de Rubros Virtuales, definidos en parametros/Liquidaciones/Rubros Extra
*   @rev    fah  Jun/14/10	Logica para laplicar lista de precios a Cobrar en liquidacion (3) diferente de Pagar en liquidacion (4)
*   @rev    fah 30/06/2010  incluir identificado de item pagable y filtrar solo los items inventariables
*   @rev    fah 21/07/2010  No pagar Cajas caidas: aplesa
*   @rev    fah 21/07/2010  error de calculo en tmp_CantEmbar (para descuento por caja emb, aplesa): mal enlace con concomprob
*   @rev	fah 20/01/2011	En rubros vituales aplicar formulacion para determinar cantidades mediante un multiplicador (par_valr3) que altera la cantidad
*   						Embarcada originalmente en un nuevo valor
*	@rev	fah 20/01/2011	Manejo de variable global  $gaOpt con Opciones generales de ejecucion. Incluye parametr 'LGLIPR' con Listas de Precio a aplicar
*   @rev esl	16/nov/2011	Cambio realizado para que el query traiga los datos por la fecha especificada en el detalle o por las fechas default
*
*/
...
/** Liberar las tarjas de un lote para que se pueda aplicar uno nuevo a ellas
*   Para cada registro encontrado, se actualiza el campo LiqProceso.
*   @param  $db             Object      ByRef, Conexion a la base de datos
*/
function fLiberaProc(&$db)   {
    global $ilAno;
    global $ilSemana;
    global $ilNumProces;
    $slSql= "Update liqtarjadetal join liqtarjacabec on tar_numtarja = tad_numtarja
	      SET tad_liqproceso = 0, tad_liqnumero = 0
              WHERE  tad_liqproceso = " .  $ilNumProces ;
    $db->execute($slSql);
}

/** Aplicaci� de un Lote de Liquidaci�, Recibe como via GET el A�, Semana, Nro de proceso y la instrucci�
...
function fAplicaProc(&$db)   {
    ...
    $slSql= "SELECT  distinct tad_NumTarja, tad_Secuencia, tac_Embarcador, IFNULL(tad_LiqNumero,0) as tad_LiqNumero,
                     concat(left(per_Apellidos,50), ' ', left(per_nombres,50)) as txt_Nombre
              FROM liqembarques
		    JOIN liqtarjacabec ON tac_semana = " .  $ilSemana . " AND tac_RefOperativa = emb_RefOperativa
              ...
              WHERE  " .  $ilSemana . " BETWEEN emb_SemInicio AND emb_SemTermino
	      AND  tad_LiqProceso=0 ";
...
//error_log ("Paso 1", 3, "/tmp/aaa-errors.log");
    ...
    $ilUltimoProd = false;
    $ilLiqSecue = NZ(fDBValor($db, 'liqsecuencias', 'ID'),1);
    //               Analiza la secuencia y decide utilizar el sigte numero usado o el generado
    $ilMaxNumLiq = NZ(fDBValor($db, 'liqtarjadetal', 'MAX(tad_LiqNumero)'),1);
...
//error_log ("Paso 4", 3, "/tmp/aaa-errors.log");
            if($ilUltimoProd != $result->fields['tac_Embarcador']) {                
            // Para cada Nuevo productor, Nuevo Numero de Liquidacion
               $ilLiqNumero = ($result->fields['tad_LiqNumero'] == 0) ? $db->GenID('liqsecuencias') : $result->fields['tad_LiqNumero'] == 0;
	       // echo "<br> 1 liquidacion" . $ilLiqNumero . "/ " . $result->fields['tad_LiqNumero'] . "/ " . $db->GenID('liqsecuencias') ;
               $ilUltimoProd = $result->fields['tac_Embarcador'];
            }
	    // echo "<br> 2 liquidacion" . $ilLiqNumero;
...
//echo 	$result->fields['tac_Embarcador'] . $result->fields['txt_Nombre'] . $ilLiqNumero . "<br>" ;
...
/**
*   Elimina Registrs de Detalle de INventario, segn datos enviados en un arreglo
* 	@access public
*   @param  variant $conn         Referencia a una Conexi� ya establecida
*   @param  array   $record       Arreglo que contiene los valores a insertar, con indide = nombre del campo
* 	@return bool                  Resultado de la inserci�
*/
...
*   @param  string  $ilNumproces    Nmero de proceso
* 	@return void
*/
function fSelDatosProc(&$db, $ilNumProces, $ilProductor=false)
{
    ...
    $slSql = "CREATE  temporary table tmp_liqdetal
                SELECT d.*, c.tac_Embarcador,
                        tad_CantRecibida - tad_CantRechazada  - tad_CantCaidas AS tad_CantEmbarcada,
...
//                                  Limpieza y regeneracion de tablas temporales utilizadas en calculos
    ...
    if(!$db->Execute("CREATE TEMPORARY TABLE tmp_product ".  // # #fah12/02/2010  incluir numero de cajas embarcadas
                     "(tmp_Codproductor INTEGER(10) KEY,
                       tmp_LiqNumero INTEGER(10) ,
	                   tmp_UltSemana INTEGER(5),
    				   tmp_CantEmbar DECIMAL(10,2),
	                   INDEX k1 (tmp_CodProductor))"))
                     fErrorPage('','NO SE PUDO CREAR LISTA DE PRODUCTORES', true); //   fah Feb/07/07
//                                  Tabla temp con resumen de productores y Nro de liquidacion   #fah12/02/2010  quito la sentencia DISTINCT
    if(!$db->Execute("INSERT INTO tmp_product ".
	                    "SELECT tac_Embarcador as tmp_CodProductor,
			                tad_LiqNumero as tmp_LiqNumero,
							ifnull(max(com_refOperat),0),
							SUM(tad_CantEmbarcada)
	                        FROM tmp_liqdetal
		                LEFT JOIN concomprobantes on com_tipocomp = 'LI'
	                        AND com_numcomp = tad_liqnumero AND com_codreceptor = tac_embarcador
				GROUP BY 1,2")) fErrorPage('','NO SE PUDO GENERAR LISTA DE PRODUCTORES', true) ; //   fah Feb/07/07  #fah21/07/10 agrego relacion por numero de liq
}
...
**/
    $db->SetFetchMode(ADODB_FETCH_BOTH);
    $result = $db->Execute("DROP TABLE IF EXISTS tmp_dat");
    if (!$result) fErrorPage('','0.- NO SE PUDO OBTENER DETALLE DE TARJAS PARA EXPLOSION DE INVENTARIO', true,false);
    ...
    $slSql =  "CREATE TEMPORARY TABLE tmp_dat " .  $slSel .
                "LEFT JOIN liqdosis ON dos_CodComponente = tad_CodCompon1)  LEFT JOIN conactivos ON act_CodAuxiliar = dos_CodItem " ;
    $result = $db->Execute($slSql);
    if (!$result) fErrorPage('','1.- NO SE PUDO OBTENER DETALLE DE TARJAS PARA EXPLOSION DE INVENTARIO', true,false);

//    $slSql =  "CREATE INDEX i_tmp  ON tmp_dat(tac_embarcador, tad_numtarja) ";
//    $result = $db->Execute($slSql);

    $slSql =  "INSERT INTO tmp_dat " . $slSel .
                "LEFT JOIN liqdosis ON dos_CodComponente = tad_CodCompon2)  LEFT JOIN conactivos ON act_CodAuxiliar = dos_CodItem " ;
    $result = $db->Execute($slSql);
    if (!$result) fErrorPage('','2.- NO SE PUDO OBTENER DETALLE DE TARJAS PARA EXPLOSION DE INVENTARIO', true,false);

    $slSql =  "INSERT INTO tmp_dat " . $slSel .
                "LEFT JOIN liqdosis ON dos_CodComponente = tad_CodCompon3)  LEFT JOIN conactivos ON act_CodAuxiliar = dos_CodItem " ;
    $result = $db->Execute($slSql);
    if (!$result) fErrorPage('','3.- NO SE PUDO OBTENER DETALLE DE TARJAS PARA EXPLOSION DE INVENTARIO', true,false);

    $slSql =  "INSERT INTO tmp_dat " . $slSel .
                "LEFT JOIN liqdosis ON dos_CodComponente = tad_CodCompon4)  LEFT JOIN conactivos ON act_CodAuxiliar = dos_CodItem " ;
    $result = $db->Execute($slSql);
    if (!$result) fErrorPage('','4.- NO SE PUDO OBTENER DETALLE DE TARJAS PARA EXPLOSION DE INVENTARIO', true,false);

    $slSql =  "SELECT * from tmp_dat ORDER BY 1,2";
    $result = $db->Execute($slSql);
...
{
    $slSql =  "SELECT tac_Embarcador, 0000000 AS tad_NumTarja, 0000000 AS tad_Secuencia,
                      0000000 as tac_RefOperativa, tac_Semana, tad_LiqNumero,
                      ...
                      SUM(000000.0000) AS TVALUNIT,
                      SUM(000000.0000) AS TADEUNIT,
		      SUM(ROUND(tad_CantEmbarcada * (tad_ValUnitario - if(tad_DifUnitario <0, tad_DifUnitario,0)), 2)) AS TBASIMPO,
                      ...
                      SUM(ROUND(tad_CantEmbarcada * IF(tad_DifUnitario >= 0, tad_DifUnitario, 000000.0000),2)) as PDIFPREC,
                      SUM(ROUND(tad_CantEmbarcada * IF(tad_DifUnitario < 0, tad_DifUnitario* (-1), 000000.0000),2)) as PBONPREC
                FROM tmp_liqdetal
...
/**
*   Procesa las Tarjas que corresponden al numemro de proceso. Graba la explosi� de insumos
* 	@access public
*   @param  object  $db       Ref. a la conexion
*   @param  object  $result   Resultado de la selecci� de tarjas
*   @param  date    $dlFecCierre    Fecha de Cierre de liquidaci�
*   @param  integer $ilSemana       Semana correspondiente
*   @param  integer $ilNumProceso   Nmero de proceso a ejecutar
* 	@return void
*/
function fProcesaTarjas(&$db, &$result, $dlFecCierre, $ilSemana, $ilNumProces,  $ilPeriodo) {
    global $slUsuario, $gaOpt ;
    $ilRegNum_em = false;
    $ilRegNum_rz = false;
    $ilRegNum_ca = false;
    $ilNumTarj = -1;
        ...
        while (!$result->EOF )
        {
        	if ($ilNumTarj != $result->fields['tad_NumTarja']) {
        	   $slSel = "SUM(tad_cantrecibida) AS tot_recibidas,
                        ...
                        SUM(tad_CantRecibida + tad_CantRechazada + tad_CantCaidas) AS tot_embarcado"; // No se plaica esto en ningun lado
...
//                                     "</td><td>";
		...
		$flPrecUnit = fTraePrecioItem($db, $result->fields['dos_CodItem'], $gaOpt["lisPagLiq"]); //#fah14/06/2010 : Lista de precios a pagar (antes estaba 3)
...
* 	@access public
*   @param  object  $db       Ref. a la conexion
*   @param  object  $rsTarjas       Resultado de la selecci� de tarjas
*   @param  date    $dlFecCierre    Fecha de Cierre de liquidaci�, para determinar saldos de Cuentas
*   @param  date    $dlFechaLiq     Fecha de liquidaci� (se aplica a comprobantes generados)
*   @param  integer $ilSemana       Semana correspondiente
*   @param  integer $ilNumProceso   Nmero de proceso en ejecuci�
*   @param  integer $ilPeriodo      Nmero de periodo en ejecuci�
* 	@return void
*/
function fLiquidaInvent(&$db, $rsTarjas, $dlFecCierre, $dlFechaLiq, $ilSemana, $ilNumProces, $ilPeriodo)
{
    global $gdFecInicio, $gaOpt;
    global $ilSemana;
    global $slUsuario;
...
//
	...
	//#fah 12/02/2010 +tmp_CantEmbar
	//#fah 30/06/2010 incluir identificado de item pagable
    $slSql = "SELECT com_CodReceptor,  det_CodItem, act_UniMedida, tmp_LiqNumero,
		    tmp_CodProductor, tmp_CantEmbar,
		    par_valor2 AS tmp_pagable,
                     ...
                     sum(IF(com_RefOperat < tmp_UltSemana ,det_CantEquivale * pro_Signo, 000000.00)) AS xxx_SalCantidad
		     ...
		     JOIN genparametros on par_clave = 'ACTGRU' AND par_secuencia =  act_grupo
              ...
              WHERE pro_codProceso = 5   AND
                    com_RefOperat BETWEEN " . $ilSemInic . " AND ". $ilSemana .
                    " AND com_FecContab >=  '" . $gdFecInicio . "'
		    AND act_inventariable = 1
              GROUP BY 1,2,3,4,5,6,7
              ORDER BY 1,2 ";                           //   fah Feb/07/07  (tmp_ultsemana)
//                     " AND com_FecContab BETWEEN '" . $gdFecInicio . "' AND '" . $dlFecCierre . "'
    ...
    // #fah12/02/2010    Rubros Virtuales que se aplican en  liquidacion como parte de INV
    $alRubVirt = $db->getAll("SELECT par_valor1 AS par_Valor1,
				par_valor2 AS par_Valor2,
				par_valor3 AS par_Valor3
			     FROM genparametros WHERE par_clave = 'LGREX'  AND par_valor1 <> 0");
    $db->SetFetchMode(ADODB_FETCH_BOTH);
    //echo "<br>\nrubros virtuales" ; print_r($alRubVirt);
    while (!$rsLiquida->EOF )
    {
//print_r($rsLiquida->fields);
//echo "<br><br><br>";
    	if ($ilProductor != $rsLiquida->fields['com_CodReceptor']) {
			// fah Feb/07/07 Aplicacion de politacas para Inventario
			// TODO: Parametrizar la cuenta .
			//      Seleccionar la primera politica vigente
            $sSql= "SELECT IFNULL(pol_Valor,0), pol_politica
                        FROM conpoliticas
                        WHERE pol_Cuenta = '" . "@@@" . "' AND
                              pol_Auxiliar = " .$rsLiquida->fields['com_CodReceptor'] . " AND
                              pol_Vigencia >= '" . $dlFecCierre . "' ORDER BY pol_Vigencia DESC LIMIT 1";
            $db->SetFetchMode(ADODB_FETCH_BOTH);
            $rsPol = $db->Execute($sSql);  // Obtener la politica de pago
            if(!$rsPol) fErrorPage('',"NO SE PUDO DEFINIR POLITICA  DE COBRO DE INVENTARIO PARA PRODUCTOR" . " - " . $rsLiquida->fields['com_CodReceptor'] , true, false);
            $rsPol->MoveFirst();
            if ($rsPol->EOF){
                $blProcesarInv = true;
                $flValor = 0;
                ...
                $flValor =$rsPol->fields[0];    // TODO:   aplicar unvalor maximo a cobrar / pagar en cada caso.
                if($rsPol->fields[1] == "N" ) $blProcesarInv = false;; // No procesa Inventario
                unset ($rsPol);
            }
    	    $ilSecue = 1;
    	    $ilRegNumero = false;
    	    $ilRegNumeroLN = false;
    	    $ilTexto = 'Liquidacion Sem ' . $ilSemana . (!$blProcesarInv ? " Sin aplicar Inventario por politica de Cobro predefinida " : " " );
            fAgregaComprobante($db, 'LI', $rsLiquida->fields['tmp_LiqNumero'], 0, $dlFechaLiq, $dlFechaLiq,
                                ...
                                $ilTexto,  0, 1, 0, 0,$ilSemana, 5, 1,
                                $ilNumProces, 593, $slUsuario, $ilPeriodo, date("Y-m-d"));
            $ilRegNumero = $db->Insert_ID();
            //--- Correccion para cerrar documentos de Inventario   TODO: modificar  el numprocoso por la liquidacion
	    $db->Execute( "UPDATE concomprobantes set com_numproceso = " . $ilSemana . ", " .
				    " com_perOperativo  = " . $ilSemana .
							    " WHERE  com_tipocomp in ('EP', 'DV') "  .
									    " AND com_codreceptor = " . $rsLiquida->fields['tmp_CodProductor'] .
									    " AND com_refoperat <= " . $ilSemana .
									    " AND com_numproceso <=0 " );            // fah Feb/07/07 (com_peroperativo)
            $ilProductor = $rsLiquida->fields['com_CodReceptor'];
	    /* #fah 12/02/2010             Rubros Virtuales									  */
	    $ilSecue = -100;
	    foreach($alRubVirt as $k => $v){
		$alDetalle['det_RegNumero']     = $ilRegNumero;
				$flMultip = NZ($v["par_Valor3"],0);

                $alDetalle['det_Secuencia']     = $ilSecue++;
                $alDetalle['det_CodItem']       = NZ($v["par_Valor1"],0);
                $alDetalle['det_CanDespachada'] = round($rsLiquida->fields['tmp_CantEmbar'] *  ($flMultip ? $flMultip : 1),2); // El tercer parametro es un multiplicador de cantidad
                $alDetalle['det_UniMedida']     = 1;
                $alDetalle['det_CantEquivale']  = $alDetalle['det_CanDespachada'];
                $alDetalle['det_CosTotal']      = 0;   // REv
                $alDetalle['det_ValTotal']      = round($v["par_Valor2"] * $alDetalle['det_CanDespachada'],2);
                $alDetalle['det_RefOperativa']  = 0;
                $alDetalle['det_Estado']        = 1;
                $alDetalle['det_CosUnitario']   = 0;
                $alDetalle['det_ValUnitario']   = NZ($v["par_Valor2"],0);
                $alDetalle['det_Destino']       = 0;
		//echo "<br>";print_r($alDetalle);
		//echo "<br>";print_r($v);
                fInsDetalleInv($db, $alDetalle);
	    }
	    $ilSecue=1;
	}
        if ($blProcesarInv) {       // fah Feb/07/07        Manejo de politicas de cobro para Inventarios
            $alDetalle = false;
    ...
    //-----
//	if ($ilCodItem == "25041") echo "<br>inicia: " . $flCanti;
...
//echo "<br>2 ";
                if ($flCanti == 0) $flPrecUnit =0;
                  ...
                  $flPrecUnit = fTraePrecioItem($db, $rsLiquida->fields['det_CodItem'], $gaOpt["lisCobLiq"]);
                }
//echo "<br>3 ";
                if ($flCanti < 0 ) { //                           Si el productor entrega m� cajas de las retiradas en bodega
// #fah30/06/2010   Si el item NO es pagable  // #fah23/07/10
//jvl para que sea pagable en ACT_GRU en Par_Valor2 se debe setear a la categoria con 1 y sera pagado
//jvl para que acumule en invprocesos el comprobante ej LN en campo efeacumula debe estar seteado a 1 Y SIGNO -1 EFECTO 1
		    if ( $rsLiquida->fields['tmp_pagable'] == 0){ 
			      $flPrecUnit = 0;
			      $flCanti = 0;
		    } else {
...
//echo "<br>4 ";
//                              Pagar solo si hay saldo cobrado a favor del prod.
			      if ($flSalCant  > 0)
//echo "<br>5 ";
                        /*if ($flSalCant <>0) $flPrecUnit = round( $flSalValor / $flSalCant, 2); //@TODO: parametrizar el modo de pago de Inventario
                        else $flPrecUnit = 0;*/

			$flPrecUnit = fTraePrecioItem($db, $rsLiquida->fields['det_CodItem'], $gaOpt["lisPagLiq"]); //#fah14/06/2010 : Lista de precios a pagar
...
//if ($ilCodItem == "25041") echo "<br> Termina: " . $flCanti . " // " . $flSalCant;
                $alDetalle['det_RegNumero']     = $ilRegNumero;
                $alDetalle['det_Secuencia']     = $ilSecue++;
     ...
     echo "<br> $ilCodItem / $flSaldo // $flCanti " .  " ::" . abs($flSaldo) . " ===" . abs($flCanti);  // Descomentado JVL
//          Agregar un comprobante LN por lo que no se pague al productor (saldo negativo no justficado por historia)

		$flPrecUnit = fTraePrecioItem($db, $rsLiquida->fields['det_CodItem'], $gaOpt["lisPagLiq"]);  //#fah10/06/09 Hacer que el precio a pagar sea el de lista, no el promedio //#fah14/06/2010 : Lista de precios a pagar

		if ( $flSaldo < 0 && abs($flSaldo) > abs($flCanti)) {
                    if (!$ilRegNumeroLN) { //                   A�dir el comprobante si no se lo ha hecho previamente
...
//                      el tipo de operaci� aritm�ica a efectuar y el tipo de variable que maneja. Aplicando procedi-
//                      mientos diferentes si se trata de variable de Tarja, de productor o de Embarque, etc.
//                      En cada nuevo productor se genera la cabecera de liquidaci�.
    $rsTarjas->MoveFirst();
...
//---- Definir si la liquidacion tiene una sola cabecera (LL) y un invdetalle y un condetalle �son  dos cabeceras (LI, LQ)
            fAgregaComprobante($db, 'LQ', $trj->TAD_LIQNUMERO, 0, $dlFechaLiq, $dlFechaLiq,
                                $dlFechaLiq, 29, $trj->TAC_EMBARCADOR, '.' ,
                                'Liquidaci� Sem ' . $ilSemana , 0, 1, 0, 0, $ilSemana, 5, 1,
                                $ilNumProces, 593, $slUsuario, $ilPeriodo, date("Y-m-d"));
            $ilNumReg_lq = $db->Insert_ID();
            $rsRubGen->MoveFirst();
            $flAcum = 00;
            $flMini = 0; // @TODO: PArametrizar                                               Valor minimo por liquidacion #fah29/07/09
            while ($rub = $rsRubGen->FetchNextObject()) {
                $flCanti = 0;
                $flValor= 0.00;
		$flDeflt = NULL;
		$flDeflt = $db->getOne("SELECT des_valor FROM liqdefaults WHERE des_semana = " . $ilSemana .
				    " AND des_productor = " . $ilProductor . " AND des_rubro = " . $rub->RUB_CODRUBRO );  // #fahMay/07/09 - Obtener Valor default del rubro para esa semana
//echo "long: " . strlen( $flDeflt);
		if (strlen( $flDeflt) == 0) $flDeflt = -1;
//echo "<br> *rbro: " .  $trj->TAD_LIQNUMERO . " / " . $rub->RUB_DESCLARGA . " / " . $rub->RUB_INDDBCR . " def: " . $flDeflt;
                if ($rub->RUB_INDCANTIDAD) $flCanti = $trj->TAD_CANTEMBARCADA;
                if ($rub->RUB_INDDBCR == 1 || ($rub->RUB_INDDBCR == -1 && $flAcum >= $flMini) || $rub->RUB_ORDCALCULO < 20) {//    Si son Ingresos � Egresos y el  acumulado > Minimo
                    $flValor = (($flDeflt >=0)) ? $flDeflt : fResuelve($db, $rub, $trj, $gdFecInicio, $dlFecCierre);
                    if ($gbDbg)  echo "<br> Valor a aplicar:  ". $flValor ;
...
**/
	    ...
	    $flDeflt = $db->getOne("SELECT des_valor FROM liqdefaults WHERE des_semana = " . $ilSemana .
				" AND des_productor = " . $ilProductor . " AND des_rubro = " . $rub->RUB_CODRUBRO );  // #fahMay/07/09 - Obtener Valor default del rubro para esa semana
            if (strlen( $flDeflt) ==0) $flDeflt = NULL;
	    if ($rub->RUB_INDCANTIDAD) $flCanti = $trj->TAD_CANTEMBARCADA;
            if($rub->RUB_OPERACION != "A") { //Variable de Tarja
                if ($rub->RUB_INDDBCR == 1 || ($rub->RUB_INDDBCR == -1 && $flAcum >= $flMini) || $rub->RUB_ORDCALCULO < 20) {//    Si son Ingresos � Egresos y el  acumulado > Minimo
                   $flValor = ($flDeflt >=0) ? $flDeflt : fResuelve($db, $rub, $trj, $gdFecInicio, $dlFecCierre);	     // #fahMay/07/09   SI existe un valor Default, lo aplica, caso contrario lo calcula
                    if ($rub->RUB_ORDCALCULO >=20  && ($rub->RUB_INDDBCR == -1 && ($flAcum - $flValor) < $flMini)) $flValor = $flAcum - $flMini; //  Si el Descuento sobrepasa el minimo
                    }
                else  $flValor = 0;
...
function fResuelve(&$ds, &$rub, &$trj, $gdFecInicio, $dlFecCierre)
{
    ...
    global $gbDbg;
   if ($gbDbg)  {
	echo "<br><br>***************************************************************** " . $rub->RUB_CODRUBRO . "/ ". $rub->RUB_DESCLARGA . "*******************************<br>" ;
	print_r($rub);
   }
 ...
 if ($gbDbg) echo " 4 " ;
	...
	if ($gbDbg) { echo " 8 " ; print_r($flOper);    }
//echo " 9 " ;
	if ($rub->RUB_OPERACION == "*") {
	    $flCalc= $flOper[1] * $flOper[2];
	    if ($gbDbg) { echo " 9 " ; print_r($flOper); echo $flCalc;   }
	    return $flCalc;
    }
    else {
	if ($gbDbg) { echo " 10" ; print_r($flOper);    }
...
function fTrae(&$db, $NombVar, &$rs) {
    global $ilSemana, $gbDbg;
    switch ($NombVar){
        case 'TCAJEMBA':
            return NZ($rs->TAD_CANTEMBARCADA,0);
            break;
        case 'TCAJRECH':
            return NZ($rs->TCAJRECH,0);
            break;
        case 'TCAJCAID':
            return NZ($rs->TCAJCAID,0);
            break;
        case 'PVALOFIC':
            return NZ($rs->PVALOFIC,0);
            break;
        case 'PDIFPREC':
            return NZ($rs->PDIFPREC,0);
            break;
        case 'PBONPREC':
            return NZ($rs->PBONPREC,0);
            break;
        ...
        case 'TCAJRETI':
            ...
            break;
        case 'TVALUNIT':
            return NZ($rs->TVALUNIT,0);
            break;
        case 'TBONUNIT':
            $flVal = NZ($rs->TADEUNIT,0);
        ...
        case 'TADEUNIT':
            ...
            break;
        case 'TVALOFIC':
            return round(NZ($rs->TAD_CANTEMBARCADA,0) * NZ($rs->TVALUNIT,0),2);
            break;
        case 'TVALPROD':
            return round(NZ($rs->TAD_CANTEMBARCADA,0)  * (NZ($rs->TVALUNIT,0) - NZ($rs->TADEUNIT,0)),2);
            break;
        case 'TVALADEL':
            $flVal = NZ($rs->TADEUNIT,0);
            ...
            break;
        case 'TBASIMPO':
            // #fah02/09/09 return NZ($rs->TAD_CANTEMBARCADA) * (NZ($rs->TVALUNIT) + fTrae($db, 'TBONUNIT', $rs)); // Base Impon: Cant * ( Prec. ofic + Bono)
            if ($gbDbg) echo "<br> BI:" . $rs->TBASIMPO;
            return round(NZ($rs->TBASIMPO,0),2);
            break;
        case 'TVALBONO':
            $flVal = NZ($rs->TADEUNIT);
...
*       @param      integr      $pProd      C�igo del productor
*       @param      string      $pCuent     C�igo de la cuenta
*       @param      date        $pDesde     Fecha de Inicio para el c�culo
*       @param      date        $pHAsta     Fecha de Cierre para el c�culo
*       @param      bool        $pPolit     Indica si se aplica politica o no
*       @param      bool        $pDbCr      Indica si se trata de cuenta deudora o acreedora
*       @return     float                   Saldo de la cuenta.
*/
function fTraeSaldo( &$db, $pProd, $pCuent,$pDesde, $pHasta, $pPolit=NULL, $pDbCr=-1)
{
    global $gdFecInicio, $gbDbg;
    global $ilSemana;
    if (!isset($pHasta)) $pHasta= date('Y-m-d');
    if (is_numeric($pHasta)) $pHasta=str2date($pHasta, 'ymd');
    if (substr($pCuent,0,1)== '1') $ilTipo = 1;        // define un multiplicador segun naturaleza de Cuenta (1=Deudora, -1=Acreedora) para tener siempre un saldo positivo que analizar
    else $ilTipo = -1;
    /*@rev esl	16/nov/2011	Cambio realizado para que el query traiga los datos por la fecha especificada en el detalle o por las fechas default*/
    $lSql= "SELECT sum(det_Valdebito - det_ValCredito) * (" . $ilTipo . ") as tmp_saldo ".
                "FROM concomprobantes JOIN condetalle on det_Regnumero = com_RegNUmero AND (
											    det_FecCheque BETWEEN '". $pDesde . "' AND '" . $pHasta . "' " .
											"OR det_FecCheque = '2020-01-01'
											 OR det_FecCheque = '2999/12/31'
                                                                                         OR det_FecCheque = '0000-00-00'
                                                                                         OR det_FecCheque = '2999/12/31'
											 )" .
...
//      Seleccionar la primera politica vigente
	...
	$sSql= "SELECT IFNULL(pol_Valor,0), pol_politica
                    ...
                    WHERE pol_Cuenta = '" . $pCuent . "' AND
                          pol_Auxiliar = " .$pProd . " AND
			  pol_semana  <= ". $ilSemana . " AND
			  pol_Vigencia >= '" . $pHasta . "'
			  order by pol_semana desc limit 1
                          ";
        $db->SetFetchMode(ADODB_FETCH_BOTH);
        ...
        $rs->MoveFirst();
        if ($rs->EOF)return $flSaldo;			// Productor sin politica de cobro, aplica el saldo total
        $flValor =$rs->fields[0];
	switch ($rs->fields[1]){
	    case "O":  				// Valor obligatorio mientras haya saldo
		if ($flValor == 0 || $flValor > $flSaldo) $flValor =  $flSaldo;
		break;
	    case "V":				// Valor obligatorio sin importar el saldo
		break;
	    case "X":				// Multiplo de Otra variable
		break;

/*
        if($rs->fields[1] == "O" || $rs->fields[1] == "V" ) return $flValor =$rs->fields[0]; // si existe una politica "obligatoria" aplicarla sin mas.
        unset ($rs);
        if ($flValor < 0 ) return 0;					// Valor negativo
        if ($flValor == 0 || $flValor > $flSaldo) return $flSaldo; 	//Si el valor de la politica de pago supera al saldo
		 */
	}
	if ($gbDbg) echo " ts2:" . $flValor . " ";
        return $flValor;

    }
    if ($gbDbg) echo " ts3:" . $flValor . " ";
    return $flSaldo;
}
...
*       @param      int         $NumTar         Nmero de Tarja
*       @param      int         $TarjaSec       Secuencia de Tarja
*       @param      int         $CodRub         C�igo de rubro
...
{
    if (strlen($ValTot) < 1 or is_null($ValTot)) $ValTot = 0;
    $Val = 0;
    $ValTot = NZ($ValTot,0);
...
/**     Trae el precio de un item, segun la lista solicitada
*       @access     public
*       @param      obj      $db            	Conexion a BD
*       @param      int      $pItem      Codigo del item
*       @param      int      $plist	 Codigo de lista a plicar (default:3)
*       @return     void
**/
function fTraePrecioItem($db, $pItem, $pList=3 ){
    $pList = 3;
    $flPrecUnit = NZ(fDBValor($db, 'invprecios', 'pre_preUnitario', 'pre_CodItem = ' . $pItem . ' AND pre_LisPrecios =' . $pList));
    if ($flPrecUnit == 0)  {
		$slItem = NZ(fDBValor($db, 'conactivos', 'concat(act_descripcion, " ", act_descripcion1) as descr', 'act_CodAuxiliar = ' . $pItem));
		echo 'NO SE HA DEFINIDO PRECIO DE LIQUIDACION PARA EL ITEM &nbsp;&nbsp;' . $pItem . ' - '. $slItem . '<br>';
    }
    return $flPrecUnit;
}
...
*   @param  integer     $pNumero    Nmero del documento, proceso, etc
*   @param  datetime    $pFecha     Fecha de Inicio
*   @param  string      $pEvento    Acci� que dispara la bitacora
...
$ilProductor=fGetParam('pPrd',false);//             C�igo de productor a ejecutar, si es falso, se aplica a todo el lote
$slMensaje  =false;
...
if (!$ilAno) $slMensaje .= " ANIO DE PROCESO INVALIDO<BR>";
if ($slMensaje) fErrorPage('',$slMensaje, true, false);
unset($rs);
set_time_limit(20*60);
if ($ilEstadoProc >  7) $ilEstadoProc = 7;
if ($ilEstadoProc <= 0) $ilEstadoProc = 2;
$gbDbg = fGetParam("pAdoDbg", false);
...
$gaOpt= $db->GetAssoc("SELECT par_valor1, par_valor2 FROM genparametros WHERE par_clave = 'LGLIPR'");  // #fah20/01/2011 Parametro de lista de precios a aplicar al cbro y pago de carton
//print_r($gaOpt);

if ($ilEstadoProc ==2 )  fEliminaLiq($db, "com_Numproceso = " . $ilNumProces, "liq_Numproceso = " . $ilNumProces, "tad_LiqProceso = " . $ilNumProces);
if ($ilEstadoProc >1 ) fSelDatosProc($db, $ilNumProces, $ilProductor);
...
switch ($ilEstadoProc) {
     case 1:
        echo "PASO $ilEstadoProc.-  LIBERACION DE TARJAS PARA PROCESAMIENTO: </font></td></tr><tr><td>";
        fLiberaProc($db);
    ...
    case 5:
        ...
        fDelComprobPrev($db, $ilNumProces, "'LQ', 'IG', 'LP', 'DC', 'EC'", $CondProd);
