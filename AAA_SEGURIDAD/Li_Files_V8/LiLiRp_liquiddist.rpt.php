...
*   @rev	esl 08/04/2011  VALIDAR CUANDO EL REPORTE SE EJECUTA DESDE UNA EMPRESA CORPORATIVA O UNA INDIVIDUAL
*/
...
function fCabeceradeLiq(&$rpt, &$group){
    ...
    /** @REV ESL	08/04/2011
     *  PARA VALIDAR CUANDO EL REPORTE SE EJECUTA DESDE UNA EMPRESA CORPORATIVA O UNA INDIVIDUAL
    */
    if ($group->lastRec['CIA'] != 0 ) {
	$slSql="select concat(par_descripcion, ifnull(par_valor1,'')) as dat
		from ". $group->lastRec['CIA'] .".genparametros 
	...
	//    echo "REC";
	//    print_r($rs->fields);
			$rpt->putTextWrap(190, $rpt->pdf->y+20 , 450, 10, $slEmpre,  12);
			$rpt->pdf->y -=12;
			$rpt->putTextWrap(50, $rpt->pdf->y , 100, 8, "RUC: " . $slRuc,  0);
			$rpt->putTextWrap(350, $rpt->pdf->y , 400, 8, "DIRECCION: " . $slDirec,  0);
			$rpt->pdf->y -=16;
