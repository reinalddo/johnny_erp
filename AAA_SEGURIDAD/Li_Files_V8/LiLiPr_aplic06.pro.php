...
/**
/**
*   Genera una tabla temporal con los totales por liquidacion segun la estructura enviada como parametro
...
$gdFecInicio='2013/12/31';                                // Fecha Inicial Para todos los procesos
...
if (!$ilAno) $slMensaje .= " YEAR DE PROCESO INVALIDO<BR>";
...
switch ($ilEstadoProc) {
 ...
 // Automatizacion de Proceso de liq negativas
        ...
        break;
    case 7:
        echo "<br>PASO $ilEstadoProc.-  GENERACION DE LF:</font></td></tr><tr><td>";
        fSelRubrosFin($db);
        fSelTotalesLiq($db, $ilNumProces, 20, 1);
        $rs =& fSelDatosContIng($db, $ilNumProces, 30, 'LF', 1);    // JVL 141004 se cambio IC x LF
        ...
        $agPla['TAP'] = $rec->PRO_AUXILIAR2;   // JVL NZ(fDBValor($db, 'genparametros', 'par_valor2+0', "par_clave='CUCAJA'"), 0);
        $agPla['AAP'] = $agPla['TAP'];
