...
*   @rev	esl 26/07/09    Usar moneda para liquidacion definida en parametros generales de empresa.
*   @rev	esl 10/06/2011	Agregar nueva pagina cuando el valor es 0
*   @rev	esl 04/jul/2012 Corregir parametro que almacena el codigo de moneda, ahora leer de EGMON
...
function before_group_COD (&$rpt, &$group) {
    ...
    if ($ilPag > 1 ) {
	    ...
	    // esl 04/jul/2012 Corregir parametro que almacena el codigo de moneda EGMON
	    $slCodMon= $db->GetOne("SELECT par_valor1 FROM genparametros WHERE par_clave='EGMON'")	;
...
$rep->setDefaultColPro('format', "9:4:,:."); //     Default Format for columns  total long:decimal places:thousands sep:decimal sep:
