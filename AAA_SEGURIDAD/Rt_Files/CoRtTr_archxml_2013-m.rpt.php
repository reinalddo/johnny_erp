...
function &fDefineVentas(&$db, $pQry = false) {
    ...
    $alSql = "	SELECT	tid.tab_txtdata2  AS tpIdCliente,
			...
			ROUND(SUM(CASE WHEN iva1.par_valor3 IS NOT NULL THEN 
					CASE WHEN 
					(SELECT 1
					FROM condetalle a
					WHERE a.det_RegNUmero = con.det_RegNUmero
					AND a.det_CodCuenta = iva1.par_Valor3) = 1 
					AND iva1.par_Valor3 <> con.det_CodCuenta
					THEN con.det_ValCredito ELSE 0 END					
					ELSE 0 END),2) 		AS baseNoGraIva,
			ROUND(SUM(CASE WHEN iva2.par_valor3 IS NOT NULL THEN 
					CASE WHEN 
					(SELECT 1
					FROM condetalle a
					WHERE a.det_RegNUmero = con.det_RegNUmero
					AND a.det_CodCuenta = iva2.par_Valor3) = 1 
					AND iva2.par_Valor3 <> con.det_CodCuenta					
					THEN con.det_ValCredito
					WHEN
					(SELECT COUNT(1)
					FROM condetalle a
					WHERE a.det_RegNUmero = con.det_RegNUmero) = 2
					THEN con.det_ValCredito
					ELSE 0                                         
                                        END					
					ELSE 0 END),2) 		AS baseImponible,
			ROUND(SUM(CASE WHEN iva3.par_valor3 IS NOT NULL THEN 
					CASE WHEN 
					(SELECT 1
					FROM condetalle a
					WHERE a.det_RegNUmero = con.det_RegNUmero
					AND a.det_CodCuenta = iva3.par_Valor3) = 1 
					AND iva3.par_Valor3 <> con.det_CodCuenta					
					THEN con.det_ValCredito ELSE 0 END					
					ELSE 0 END),2) 		AS baseImpGrav,
	...
	FROM concomprobantes com
		...
		JOIN conactivos act ON act.act_CodAuxiliar = inv.det_CodItem
                JOIN condetalle con ON con.det_RegNUmero = com.com_RegNumero
                LEFT JOIN genparametros iva1 ON iva1.par_clave = 'CTIVA' AND iva1.par_Secuencia = 0
		LEFT JOIN genparametros iva2 ON iva2.par_clave = 'CTIVA' AND iva2.par_Secuencia = 1
		LEFT JOIN genparametros iva3 ON iva3.par_clave = 'CTIVA' AND iva3.par_Secuencia = 2		
	...
	WHERE  com.com_TipoComp = 'FA' AND " . 
            $condicion = str_replace("fechaRegistro", "com.com_FecContab", $pQry) .
    /* NUEVO QUERY PARA A�ADIR NOTAS DE CR�DITO------------ */
     "	UNION ALL SELECT tid.tab_txtdata2  AS tpIdCliente,
			per_Ruc AS idCliente,
                        pro.per_parterelacionada AS parteRelVtas,                        
			IFNULL((SELECT par_valor1 FROM genparametros WHERE par_clave = 'DIMCOM' AND par_secuencia = 2), 2) AS tipoComprobante,
			COUNT(DISTINCT com.com_numcomp) 		AS numeroComprobantes,
			...
			ROUND(SUM(CASE WHEN iva3.par_valor3 IS NOT NULL THEN 
					...
					ELSE 0 END),2) 		AS montoIva,
			CAST(0 AS DECIMAL(14,2)) AS montoIce,
			CAST(0 AS DECIMAL(14,2)) AS valorRetIva,
			CAST(0 AS DECIMAL(14,2)) AS valorRetRenta
	FROM concomprobantes com
		JOIN condetalle con ON con.det_RegNUmero = com.com_RegNumero
		LEFT JOIN genparametros iva1 ON iva1.par_clave = 'CTIVA' AND iva1.par_Secuencia = 0
		LEFT JOIN genparametros iva2 ON iva2.par_clave = 'CTIVA' AND iva2.par_Secuencia = 1
		...
		LEFT JOIN genparametros pid  ON pid.par_clave = 'TIPID' AND pid.par_secuencia = per_tipoID
		LEFT JOIN fistablassri tid   ON tid.tab_codtabla = '2-2' AND tid.tab_codigo = 2 AND tid.tab_txtData  = pid.par_valor2
	WHERE  com.com_TipoComp = 'NC' AND " . 
            $condicion = str_replace("fechaRegistro", "com.com_FecContab", $pQry) .
    /* NUEVO QUERY PARA A�ADIR NOTAS DE D�BITO------------ */
    "	UNION ALL SELECT tid.tab_txtdata2  AS tpIdCliente,
			per_Ruc AS idCliente,
                        pro.per_parterelacionada AS parteRelVtas,                        
			IFNULL((SELECT par_valor1 FROM genparametros WHERE par_clave = 'DIMCOM' AND par_secuencia = 3), 2) AS tipoComprobante,
			COUNT(DISTINCT com.com_numcomp) 		AS numeroComprobantes,
			...
			ROUND(SUM(CASE WHEN iva1.par_valor3 IS NOT NULL THEN 
					...
					AND a.det_CodCuenta = iva1.par_Valor3) = 1 
					AND iva1.par_Valor3 <> con.det_CodCuenta
					THEN con.det_ValDebito ELSE 0 END					
					ELSE 0 END),2) 		AS baseNoGraIva,
			ROUND(SUM(CASE WHEN iva2.par_valor3 IS NOT NULL THEN 
					CASE WHEN 
					(SELECT 1
...
////echo  "<br>"  . $alSql;
     ...
     */
    $slSql2 = "
	SELECT MID(LPAD(CAST(com_numcomp AS CHAR(12)),12,'0'), 1, 3)  AS codEstab,
                ROUND(SUM(case com_tipoComp when 'FA' then det_valCredito when 'NC' then det_valCredito * -1 else det_valDebito end),2) AS ventasEstab
	FROM concomprobantes com
		LEFT OUTER JOIN genparametros lib ON lib.par_Clave = 'CLIBRO' AND lib.par_Valor4 = 'ATS2' AND  lib.par_secuencia  = com_libro
		LEFT OUTER JOIN invdetalle inv ON inv.det_RegNUmero = com.com_RegNumero
		LEFT OUTER JOIN conactivos act ON act.act_CodAuxiliar = inv.det_CodItem
		LEFT OUTER JOIN condetalle con ON con.det_RegNUmero = com.com_RegNumero
	WHERE " . #fah2015-03-30   	com.com_TipoComp = 'FA'  AND
            $condicion = str_replace("fechaRegistro", "com.com_FecContab", $pQry) .
            " and com_tipoComp in ('FA', 'NC', 'ND') " .
            " and not exists (select 1 from genparametros iva where iva.par_clave = 'CTIVA' and iva.par_valor3 = con.det_CodCuenta) " .
            " GROUP BY 1 ";
    $rs2 = fSQL($db, $slSql2);

    $slSql3 = "
	SELECT 	ROUND(SUM(case com_tipoComp when 'FA' then det_valCredito when 'NC' then det_valCredito * -1 else det_valDebito end),2) AS ventasEstab
	FROM concomprobantes com
		LEFT OUTER JOIN genparametros lib ON lib.par_Clave = 'CLIBRO' AND lib.par_Valor4 = 'ATS2' AND  lib.par_secuencia  = com_libro
		LEFT OUTER JOIN invdetalle inv ON inv.det_RegNUmero = com.com_RegNumero
		LEFT OUTER JOIN conactivos act ON act.act_CodAuxiliar = inv.det_CodItem
...
$totalV = fDBValor($db, "concomprobantes com INNER JOIN condetalle con ON con.det_RegNUmero = com.com_RegNumero ", "ROUND(SUM(case com_tipoComp when 'FA' then det_valCredito when 'NC' then det_valCredito * -1 else det_valDebito end),2) AS ventasTotal ", 
                        " ". $condicion = str_replace("fechaRegistro", "com.com_FecContab", $pQry) . 
                        " and com_tipoComp in ('FA', 'NC', 'ND') " .
                        " and not exists (select 1 from genparametros iva where iva.par_clave = 'CTIVA' and iva.par_valor3 = con.det_CodCuenta) ");
