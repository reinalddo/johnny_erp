...
$sSql = "SELECT
   com_RegNumero AS 'codigo',
    ...
    concat(left(com_Concepto,30), ' / ', det_glosa) AS 'descripcion',
    det_ValDebito  AS 'debito',
    det_ValCredito AS 'credito',
    '' saldo,
    det_ValDebito + det_ValCredito  AS 'saldof',
    'NO' AS 'act',
    det_NumCheque as 'cheque'
    ,det_secuencia
    ,cue_Descripcion cuenta
...
if($rs->EOF){
    ...
    if (!$Tpl->is_cached('CoTrTr_Mayor.tpl')) {
            }
    
            $Tpl->display('CoTrTr_Mayor.tpl');
}
?>