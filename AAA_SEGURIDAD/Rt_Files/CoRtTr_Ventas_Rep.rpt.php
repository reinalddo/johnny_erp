 ...
 */
ini_set("memory_limit","5000M");		// #fah2015-03-01    Soportar procesos largos
ini_set('max_execution_time', 500);		// #fah2015-03-01    Soportar procesos largos
...
$sSql = "SELECT   c.com_TipoComp,
		  ...
		  i.det_ValTotal,
		  i.det_porceDesc,
		  i.det_DescTotal as Dscto, 
		  i.det_RefOperativa,
		  i.det_Estado,
		  i.det_cosunitario, 
		  i.det_valunitario,
		  i.det_Destino,
        s.tsd_PorcentajeBI AS pIVA, 
 		  (CASE act_IvaFlag WHEN 0 THEN (det_ValTotal - det_DescTotal) ELSE 0 END) AS base0,
		  (CASE act_IvaFlag WHEN 0 THEN 0 ELSE (det_ValTotal - det_DescTotal) END) AS baseIva,
		  (CASE act_IvaFlag WHEN 0 THEN 0 ELSE ((det_ValTotal - det_DescTotal)*(tsd_PorcentajeBI/100)) END) AS montoIva,
		  (CASE act_IvaFlag
		     WHEN 0 THEN (CASE act_IvaFlag WHEN 0 THEN (det_ValTotal - det_DescTotal) ELSE 0 END)
		     ELSE (CASE act_IvaFlag WHEN 0 THEN 0 ELSE (det_ValTotal - det_DescTotal) END) + (CASE act_IvaFlag WHEN 0 THEN 0 ELSE ((det_ValTotal - det_DescTotal)*(tsd_PorcentajeBI/100)) END)
		  END) AS totalFac,
...
IF(i.`det_Secuencia` >= 2, '', (SELECT valorRetRenta FROM v_retencionVTA rt WHERE rt.documento = c.com_NumComp AND rt.com_tipocomp = 'RT'))	AS	  valorRetRenta
   ...
   JOIN gentasacabecera t ON c.com_FecContab BETWEEN t.tsc_FecInicia AND t.tsc_FecVence  
   JOIN gentasadetalle s ON t.tsc_ID = s.tsd_ID 
