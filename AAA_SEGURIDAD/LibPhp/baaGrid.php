...
*----------------------------------------------------------*
* version 1.7   Allow Function() in setGPCol()             *
*----------------------------------------------------------*
* version 1.8   Add sybase support                         *
************************************************************
      ...
      ******************************************/
      ...
      function _query() {
               ...
               case DB_MSSQL :
                    ...
                    break;
               case DB_SYBASE :
                    $res = @sybase_query($this->query, $this->conn);
                    if (!$res) {
                         $this->_handleError(mssql_get_last_message());
                         return false;
                    }
      ...
      function _fetch_row($rc) {
               ...
               case DB_SYBASE:
                    return sybase_fetch_row($this->cursor); break;
               case DB_MSSQL:
      ...
      function _getFieldNames(&$fa, &$fn) {
               ...
               case DB_SYBASE:
                    $fn = sybase_num_fields($this->cursor);
                    for ($i=0; $i<$fn; $i++)  {
                         $fobj = sybase_fetch_field($this->cursor,$i);
                         $fa[] = $fobj->name;
                         if (strpos(strtolower($fobj->type),'date')!==false)
                             $this->datefield[$i] = 1;
                    }
                    break;
      ...
      function _printTotals() {
               ...
               for ($c = 0; $c < $this->fieldCount; $c++) {
                   ...
                   if (isset($this->spans[$c])) {
                    ...
                    echo "<th";
                    if (isset($theClass[$c])) echo " class=\"{$theClass[$c]}\"";
                    echo "$atxt >";
                    if (isset($this->totals[$c])) {
                       if (isset($this->decpl[$c]))
      ...
      function _parseFields($cell,$row, $col) {
               ...
               if (($p = strpos($cell,'('))!==false) {
                    if (function_exists(substr($cell,0,$p))) {
                        $cell = eval("return(".$cell.");");
                        if (isset($this->decpl[$col]))
                           $cell = number_format($cell, $this->decpl[$col]);
                    }
               }
               return $cell;
      }
