...
class ezPdfReport
{
	...
	var $slQry='';           //para el querystring de excel
/**
*   Report Initiator
*   @access  private
...
*   @return void
*/
    function ezPdfReport($pRs, $pPage="letter", $pOrie="portrait", $pFont="./fonts/Helvetica", $pSize="10",$slQry1=null) {
...
//header("Content-disposition: inline; ");
	...
	//$goGrid = new baaGrid ($this->slQry, "DB_ADORS", null);
	$goGrid = new baaGrid ($this->slQry, null, $db);
...
//	$goGrid->debug();
	...
	echo "<br>";
        ...
        $this->rs->movefirst();
        foreach($lacols as $col => $val) 
		{//              Populate the columns from recordset info
          $olf = $this->rs->FetchField($i);
          $slt = $this->rs->MetaType($olf->type);
		  echo $slt.' '.$col.' '.$val;
		  echo "<br>";
		...
		die();
	if (is_array($rs)) $acols=array_keys($rs); //   array of column names
	if (is_array($this->groups))  $agrps=array_keys($this->groups);  //   array of group names
	reset($acols);
	foreach ($acols as $colname) 
	{
	...
	//echo dirname(__FILE__) . '/PHPExcel/Classes/PHPExcel.php';
	$objPHPExcel = new PHPExcel();
	$objReader = PHPExcel_IOFactory::createReader('Excel2007');
	$objPHPExcel = $objReader->load(dirname(__FILE__) . '/PHPExcel/prueba.xlsx');
	...
	//echo dirname(__FILE__) . '/PHPExcel/prueba1.xlsx';
	//die();
	$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
	//$objWriter->save('php://output');
	$objWriter->save(dirname(__FILE__) . '/PHPExcel/prueba1.xlsx');
	$archivo=dirname(__FILE__) . '/PHPExcel/prueba1.xlsx';
