...
//        if (!isset($this->rptOpt['maxWidth']))       $this->rptOpt['maxWidth'] =       800;
        ...
        if (!isset($this->titleOpts['T']['fontSize'])) $this->titleOpts['T']['fontSize']=$this->fontSize +2;     // si no se ha definido el tamaño para Titulo
        if (!isset($this->titleOpts['S']['fontSize'])) $this->titleOpts['S']['fontSize']=$this->fontSize;      // si no se ha definido el tamaño para SubTitulo
        if (!isset($this->titleOpts['C']['fontSize'])) $this->titleOpts['C']['fontSize']=$this->fontSize -2;      // si no se ha definido el tamaño para Condicion
...
//          If there is an acumulator that breaks in any group or in a specific group, resets it to zero
            if (($this->columns[$col]->acumBreaker == false && $ilbreaks) ||
                 ($ilbreaks && $this->groups[$this->columns[$col]->acumBreaker]->break == true) )
                $this->columns[$col]->acumValue = 0;
            $j = count($data);
            $flag = false;
...
//echo "<br>" . $grp . " - " . $col . " : " . $this->groups[$grp]->sums[$col];
                if (isset($rec[$col]) && is_numeric($rec[$col]) ) $this->groups[$grp]->sums[$col] += $rec[$col];
                }
            }
    }
...
//    else
//      $this->pdf->ezStream(); // no funciona en IE
...
*/
    ...
    function view($sgTitulo="REPORTE", $fname ){
...
/*
           fAbrirWin("'.$fname.'", "'. $wname .'", pOpc, 900, 800)
*/
            window.frames["fr2"].location.replace("'. $fname . '")

            }
        //--></script>
        </head>
        <body onLoad="go_now()"; >
        <a href="'.$fname.'" accesskey="h">HAGA CLICK AQUI</a> SI NO RECIBE EL DOCUMENTO EN PANTALLA".
        <iframe name="fr2" frameborder="1"  margin:1; style="WIDTH: 100%; HEIGHT: 650px"
          source="">
        </iframe>
...
*/
    function hideColumns() {
        foreach($this->columns as $col) {
            if (!isset($this->colHead[$col->name])) $this->columns[$col->name]->visible=false;
        }
    }
/**
...
*/
    function rptHeader(&$obj ){
        ...
        if (!$this->stop) {
            foreach ($this->columns as $col) {
                $alBlankLine[$col->name] = ' ' ;
                if(isset($this->colHead[$col->name])) {     //                          if a exists a header with this name ...
                    $this->columns[$col->name]->visible = true;
                    $ilTabWidth = $ilTabWidth + $this->colOpt[$col->name]['width'] + $this->rptOpt['colGap']; // compute the table width
                   }
...
*/
        if ($ilTabWidth > $this->pdf->ez['pageWidth'] ) {  // si el tamaño de la tabla > que la pagina
...
*/
    function rptFooter(){
        if ($this->printFooter) {
        	...
        	$x = $this->pdf->ez['pageWidth'] - $this->pdf->ez['rightMargin'] - $this->textWidth(6, "Pág. XXX de XXX");
...
function putText($pText, $pSize=false, $pLead = false, $pFnam = false, $pJust = 'left', $pLeft = false, $pAlef=0){
        ...
        $this->pdf->y -=2;  //                      Saltar un espacio mínimo
//die();
}
/**
...
function putTextWrap($x, $y, $pWidth, $pSize=false, $pText="",  $pLead = 0, $pJust = 'left', $pAng = 0, $pTest=0){
        ...
        $ilHeight = $this->pdf->getFontHeight($pSize) + $pLead; // add a spacing
         while (strlen($pText)){
            ...
            $y -=$ilHeight;  //                      Saltar un espacio mínimo
        }
        return $y;
}
/**
