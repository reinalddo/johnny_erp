...
class Minify {
  ...
  function _minify($string, $type = TYPE_JS) {
	$olMini = new Minify($type);
    //return $type === TYPE_JS ? Minify::minifyJS($string) :  	//#fah
    //    Minify::minifyCSS($string);							//#fah
	return $type === TYPE_JS ? $olMini->minifyJS($string) :
        $olMini->minifyCSS($string);
  }
  ...
  function combine($minify = true) {
    ...
    foreach($this->files as $file) {
      ...
      if ($this->type === TYPE_CSS && MINIFY_REWRITE_CSS_URLS) {
		...
		$msg .=    ( $minify ? " minified": " NO minified") ."\n" ;
        $combined[] = $minify ? $slHead . file_get_contents($file) :
					  $slHead . implode("",file($file)) ;
      }
    }
//	echo "\n\n\nPRE  CONTENIDO minify: $minify :\n" . $combined . "____" . implode("\n", $combined);
    //$combined = $minify ? Minify::_minify(implode("\n", $combined), $this->type) :
    //    implode("\n", $combined);
	$combined = implode("\n", $combined);
	if ($minify ){
		$jsmin = new JSMin($combined);
		$combined =  $jsmin->jsminify();
	}
//echo "CCCCONTENIDO:\n" . $combined;
...
//echo "path: " . MINIFY_BASE_DIR.
//	"<br>file: " . $file .
//	 "<br>realpath: " . realpath(MINIFY_BASE_DIR.'/'.$file) .
//	 "<br>realpath BR: " . realpath(MINIFY_BASE_DIR) .
//	 "<br>res :" . $filepath .
//	 "<br>"
//	 ;
