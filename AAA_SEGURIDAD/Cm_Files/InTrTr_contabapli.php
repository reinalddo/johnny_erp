 ...
 *	@return		void
 *	@rev	JVL	28/07/12	Excluir los items no inventariables de la contabilizacion
 *	@rev	JVL	31/10/13	Excluir los items no inventariables de la contabilizacion
 **/
...
//    if (strlen($pTip) <> 0) {  // Si hay parametro de Tipo de comprobante, armar condicion 
//	$slCond .= strlen($slCond) >0 ? " AND " :"";
//	$slCond .= " com_tipocomp = '" . $pTip . "'";
//    }


    $slSql = "SELECT com_regnumero 
	FROM invprocesos p
	    JOIN concomprobantes c ON pro_codproceso = 1
	    WHERE " . $slCond;
    $rs = $db->execute($slSql);
