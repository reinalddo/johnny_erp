...
//        $rpt->pdf->line($x, $y1, $x+$l , $y1);   // JVL Lineas de pie de pagina elabora, aprueba, etc.
    ...
    $slLabel= "ARE:";
    $flDy = $rpt->putTextAndLabel($rpt->leftBorder - 30 , $ilY + 15 , $slCon , '.', 350, 30, 0,0,0,'left' ); // JVL 
...
// JVL se subio $iYY de 83 a 100 para que exista distancia entre el cuadro de detalle y datos de pago
    $rpt->pdf->addText($x, $ilY - 90,12,"BANK DETAILS");
    $rpt->pdf->addText($x, $ilY - 100,12,"PLEASE MAKE PAYMENT TO:",$angle=0,$wordSpaceAdjust=0);
...
//  `per_Apellidos`,   `per_Nombres`,   `per_Ruc`,   `per_PersonaContacto`,   `per_Direccion`,   `per_Ciudad`,

    $stBen = $rsB->PER_APELLIDOS . " " . $rsB->PER_NOMBRES;    
    $stDir = $rsB->PER_PERSONACONTACTO . " " . $rsB->PER_DIRECCION ;
    $stEcu = $rsB->PER_CIUDAD;
    
    $rpt->pdf->addText($x+5, $ilY - 120,9, "BENEFICIARY" . " : " );
    $rpt->pdf->addText($x+80, $ilY - 120,11, $stBen );
    $rpt->pdf->addText($x+5, $ilY - 130,9, "ADDRESS" . " : " );
    $rpt->pdf->addText($x+80, $ilY - 130,10,$stDir ,$angle=0,$wordSpaceAdjust=0);
    $rpt->pdf->addText($x+80, $ilY - 140,10,$stEcu ,$angle=0,$wordSpaceAdjust=0);
//    Bloque de Beneficiario
//    $blSql = Array();
//    $iYY +=10;
//    $rpt->pdf->addText($x+40,$ilY-$iYY,6,$blSql ,$angle=0,$wordSpaceAdjust=0);

//BLOQUE DE DATOS BANCARIOS DEL BENEFICIARIO/*
    $iYY = 140;       // JVL debe ir acorde a ultimo $rpt
    ...
    while(!$rst->EOF) {
    	...
    	$beneval = $rec->BNV;
    	$iYY +=10;
	   $rpt->pdf->addText($x+5,$ilY-$iYY,9,$benedes . " : ",$angle=0,$wordSpaceAdjust=0);
	   $rpt->pdf->addText($x+80,$ilY-$iYY,9,$beneval ,$angle=0,$wordSpaceAdjust=0);
