...
//        $rpt->pdf->line($x, $y1, $x+$l , $y1);   // JVL Lineas de pie de pagina elabora, aprueba, etc.
	...
	if ($strpos($group->lastRec['CON'],'#VFE')=== FALSE) { 
		$slCon = strtoupper(strstr($group->lastRec['CON']));
	} else {
...
//    $rpt->pdf->addText($x+95,30,8,"Contabilidad",$angle=0,$wordSpaceAdjust=0);
    ...
    $iYY = 83;
    $rpt->pdf->addText($x , $ilY - 70,10, "BANK DETAILS");
...
//*/    
    ...
    $rpt->pdf->addText($x+120,35,8,"The funds that correspond to this invoice must be on our account by " . $group->lastRec['FVE'],$angle=0,$wordSpaceAdjust=2);
    $rpt->pdf->addText($x+125,30,8,"** Please note that the amount is payable net of all bank charges **",$angle=0,$wordSpaceAdjust=2);
