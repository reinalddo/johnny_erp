<?php
/*
*   InTrTr_karprodsemsum_deu.rpt.php: Kardex de productores DEUDORES DE CARTON RESUMIDO SEMANAL
*   @author     Fausto Astudillo
*   @param      integer		pQryTar  Condici� de bsqueda
*   @output     contenido pdf del reporte.
*   @todo       Generalizar el reporte para todos
*   @rev	Jun/13/07	Correccion para MySQl 5: tamano de campos en tablas temporales
*/
...
function &fDefineQry(&$db, $pQry=false){
   $slSql = "SELECT 
                ...
                SUM(if (com_tipocomp = 'LI' and det_cantequivale < 0, det_cantequivale * pro_signo, 0)) AS 'PAG',
                ...
                SUM((det_costotal * pro_signo)) AS 'SCO',
    ...
    $slSql .= " GROUP BY CODRE, RECEP, CODIT, ITEM  HAVING SAL >0 ORDER  BY RECEP, ITEM ";
...
$rep->columns['SAL']->type="N";             // This column is an acumulator
$rep->columns['HIS']->type="N";             // This column is an acumulator
$rep->columns['VHI']->type="N";             // This column is an acumulator
...
$rep->addGrp('RECEP');                         //
    ...
    $rep->addResumeLine('RECEP','-', 'SUBTOTAL  ',0);
        $rep->setAggregate('RECEP',0, 'SCO','S');  // Change aggregate to '-' (nothing)  for column 'C2' in resumline 1
...
$glMarTxt='';
$rep->run();
$rep->view($rep->title, $rep->saveFile("KDX_DEUD_"));
?>
