<?php
/*
*      @rev    JVL 03/07/2013  Inclusion de campos adicionales para Orden de Compra
 **/
...
//Class_Initialize Event @220-1FA6EF35
    function clsGridInTrTr_topline()
    {
        ...
        $slRestr = $_SESSION['restr'][$gsModulId]['TIP'];  //#fahJun/10/09 Restriccion de Emisor
		if (strlen($slRestr) > 1){
            $this->pTipoComp->ds->SQL .= " AND  cla_tipocomp in (" . $slRestr  . ")" ;  // #fahJun/10/09
        }
        $slAplic = "";
...
//Class_Initialize Event @70-7B995C26
    function clsRecordInTrTr_comp()
    {
        ...
        if($this->Visible)
        {
            ...
            "UNION " .*/   // Emisor no puede ser un OBjeto
            ...
            (strlen($_SESSION['restr']['InTrTr']['ACC']) >= 1 ) ? " per_codauxiliar in ( " . $_SESSION['restr']['InTrTr']['ACC'] . ") " : "" .
            "";
...
// JVL BEGIN Campos Adicionales de Factura de Exportacion PAYMENT
            $this->com_payment = new clsControl(ccsListBox, "com_payment", "Forma Pago", ccsInteger, "", CCGetRequestParam("com_payment", $Method));
            $this->com_payment->DSType = dsTable;
            list($this->com_payment->BoundColumn, $this->com_payment->TextColumn, $this->com_payment->DBFormat) = array("par_Secuencia", "par_Descripcion", "");
            $this->com_payment->ds = new clsDBdatos();
            $this->com_payment->ds->SQL = "SELECT *  " .
"FROM genparametros";
            $this->com_payment->ds->Parameters["expr55"] = 'PTERM';
            $this->com_payment->ds->wp = new clsSQLParameters();
            $this->com_payment->ds->wp->AddParameter("1", "expr55", ccsText, "", "", $this->com_payment->ds->Parameters["expr55"], "", false);
            $this->com_payment->ds->wp->Criterion[1] = $this->com_payment->ds->wp->Operation(opEqual, "par_Clave", $this->com_payment->ds->wp->GetDBValue("1"), $this->com_payment->ds->ToSQL($this->com_payment->ds->wp->GetDBValue("1"), ccsText),false);
            $this->com_payment->ds->Where = $this->com_payment->ds->wp->Criterion[1];
            $this->com_payment->Required = false;
// JVL END Campos Adicionales de Factura de Exportacion
// JVL BEGIN Campos Adicionales de Factura de Exportacion BUSINESS
            $this->com_business = new clsControl(ccsListBox, "com_business", "Tipo Negocio", ccsInteger, "", CCGetRequestParam("com_business", $Method));
            $this->com_business->DSType = dsTable;
            list($this->com_business->BoundColumn, $this->com_business->TextColumn, $this->com_business->DBFormat) = array("par_Secuencia", "par_Descripcion", "");
            $this->com_business->ds = new clsDBdatos();
            $this->com_business->ds->SQL = "SELECT *  " .
"FROM genparametros";
            $this->com_business->ds->Parameters["expr66"] = 'TBUSI';
            $this->com_business->ds->wp = new clsSQLParameters();
            $this->com_business->ds->wp->AddParameter("1", "expr66", ccsText, "", "", $this->com_business->ds->Parameters["expr66"], "", false);
            $this->com_business->ds->wp->Criterion[1] = $this->com_business->ds->wp->Operation(opEqual, "par_Clave", $this->com_business->ds->wp->GetDBValue("1"), $this->com_business->ds->ToSQL($this->com_business->ds->wp->GetDBValue("1"), ccsText),false);
            $this->com_business->ds->Where = $this->com_business->ds->wp->Criterion[1];
            $this->com_business->Required = false;
// JVL END Campos Adicionales de Factura de Exportacion
// JVL BEGIN Campos Adicionales de Factura de Exportacion KGS NETO, BRUTO, BENEFICIARIO y VIAJE
            $this->com_kgsneto = new clsControl(ccsTextBox, "com_kgsneto", "Peso Neto", ccsText, "", CCGetRequestParam("com_kgsneto", $Method));
            $this->com_kgsneto->Required = true;
            $this->com_kgsbruto = new clsControl(ccsTextBox, "com_kgsbruto", "Peso Bruto", ccsText, "", CCGetRequestParam("com_kgsbruto", $Method));
            $this->com_kgsbruto->Required = true;
            $this->lbbeneficiary = new clsControl(ccsLabel, "lbbeneficiary", "lbbeneficiary", ccsText, "", CCGetRequestParam("lbbeneficiary", $Method));
            $this->com_beneficiary = new clsControl(ccsListBox, "com_beneficiary", "Com beneficiary", ccsInteger, "", CCGetRequestParam("com_beneficiary", $Method));
            $this->com_beneficiary->DSType = dsSQL;
            list($this->com_beneficiary->BoundColumn, $this->com_beneficiary->TextColumn, $this->com_beneficiary->DBFormat) = array("cod", "nomb", "");
            $this->com_beneficiary->ds = new clsDBdatos();
//          JVL - Query de Beneficiary categoria = 70 -- y de Solicitante categoria 103
            $this->com_beneficiary->ds->SQL = 
            "SELECT per_codauxiliar as cod , concat(per_Nombres, \" \", per_apellidos) as nomb " .
            "FROM conpersonas INNER JOIN concategorias on conpersonas.per_codauxiliar = concategorias.cat_codauxiliar AND cat_Categoria = 103 order by 2";
//          JVL - Se modifico la vista vop_ordenes para que apunte a 10_frutiboni y traiga los viajes a OPEX
            $this->lbembarque = new clsControl(ccsLabel, "lbembarque", "lbembarque", ccsText, "", CCGetRequestParam("lbembarque", $Method));
            $this->com_embarque = new clsControl(ccsListBox, "com_embarque", "Com embarque", ccsInteger, "", CCGetRequestParam("com_embarque", $Method));
            $this->com_embarque->DSType = dsSQL;
            list($this->com_embarque->BoundColumn, $this->com_embarque->TextColumn, $this->com_embarque->DBFormat) = array("cod", "nomb", "");
            $this->com_embarque->ds = new clsDBdatos();
            $this->com_embarque->ds->SQL = 
            "SELECT per_codauxiliar as cod , concat(per_Nombres, \" \", per_apellidos) as nomb " .
            "FROM conpersonas INNER JOIN concategorias on conpersonas.per_codauxiliar = concategorias.cat_codauxiliar AND cat_Categoria = 81 order by 2";
//            $this->com_embarque->ds->SQL = 
//            "SELECT vor_ID AS cod , vor_Descripcion AS nomb FROM vop_ordenes WHERE vor_estado = 1 ORDER BY nomb DESC ";
// JVL END Campos Adicionales de Factura de Exportacion
...
"FROM genparametros";
            ...
            if(!$this->FormSubmitted) {
                ...
                if(!is_array($this->com_payment->Value) && !strlen($this->com_payment->Value) && $this->com_payment->Value !== false)
                $this->com_payment->SetText(9999);
                if(!is_array($this->com_business->Value) && !strlen($this->com_business->Value) && $this->com_business->Value !== false)
                $this->com_business->SetText(9999);
                if(!is_array($this->com_kgsneto->Value) && !strlen($this->com_kgsneto->Value) && $this->com_kgsneto->Value !== false)
                $this->com_kgsneto->SetText(0);
                if(!is_array($this->com_kgsbruto->Value) && !strlen($this->com_kgsbruto->Value) && $this->com_kgsbruto->Value !== false)
                $this->com_kgsbruto->SetText(0);
                if(!is_array($this->com_beneficiary->Value) && !strlen($this->com_beneficiary->Value) && $this->com_beneficiary->Value !== false)
                $this->com_beneficiary->SetText(0);
                if(!is_array($this->com_embarque->Value) && !strlen($this->com_embarque->Value) && $this->com_embarque->Value !== false)
                $this->com_embarque->SetText(0);
...
//Validate Method @70-2C462403
    function Validate()
    {
        ...
        $Validation = ($this->com_payment->Validate() && $Validation);		// JVL Adiciona campo com_payment
        $Validation = ($this->com_business->Validate() && $Validation);
        $Validation = ($this->com_kgsneto->Validate() && $Validation);
        $Validation = ($this->com_kgsbruto->Validate() && $Validation);
        $Validation = ($this->com_beneficiary->Validate() && $Validation);
        $Validation = ($this->com_embarque->Validate() && $Validation);
...
//CheckErrors Method @70-612C24D1
    function CheckErrors()
    {
        ...
        $errors = ($errors || $this->com_payment->Errors->Count());			// JVL Adicional campo com_payment
        $errors = ($errors || $this->com_business->Errors->Count());
        $errors = ($errors || $this->com_kgsneto->Errors->Count());
        $errors = ($errors || $this->com_kgsbruto->Errors->Count());
        $errors = ($errors || $this->lbbeneficiary->Errors->Count());
        $errors = ($errors || $this->com_beneficiary->Errors->Count());
        $errors = ($errors || $this->lbembarque->Errors->Count());
        $errors = ($errors || $this->com_embarque->Errors->Count());
...
//InsertRow Method @70-02BF5213
    function InsertRow()
    {
        ...
        $this->ds->com_payment->SetValue($this->com_payment->GetValue());   		// JVL com_payment
        $this->ds->com_business->SetValue($this->com_business->GetValue());
        $this->ds->com_kgsneto->SetValue($this->com_kgsneto->GetValue());
        $this->ds->com_kgsbruto->SetValue($this->com_kgsbruto->GetValue());
        $this->ds->com_beneficiary->SetValue($this->com_beneficiary->GetValue());
        $this->ds->com_embarque->SetValue($this->com_embarque->GetValue());
...
//UpdateRow Method @70-6A2BF9A3
    function UpdateRow()
    {
        ...
        $this->ds->com_Concepto->SetValue($this->com_Concepto->GetValue());
        $this->ds->com_Libro->SetValue($this->com_Libro->GetValue());
        $this->ds->com_payment->SetValue($this->com_payment->GetValue());
        $this->ds->com_business->SetValue($this->com_business->GetValue());
        $this->ds->com_kgsneto->SetValue($this->com_kgsneto->GetValue());
        $this->ds->com_kgsbruto->SetValue($this->com_kgsbruto->GetValue());
        $this->ds->com_beneficiary->SetValue($this->com_beneficiary->GetValue());
...
//Show Method @70-2E9C2339
    function Show()
    {
        ...
        $this->com_payment->Prepare();
        $this->com_business->Prepare();
        $this->com_beneficiary->Prepare();
        $this->com_embarque->Prepare();
            ...
            if($this->Errors->Count() == 0)
            {
                ...
                else if($this->ds->next_record())
                {
                    ...
                    if(!$this->FormSubmitted)
                    {
                        ...
                        $this->com_payment->SetValue($this->ds->com_payment->GetValue());
                        $this->com_business->SetValue($this->ds->com_business->GetValue());
                        $this->com_kgsneto->SetValue($this->ds->com_kgsneto->GetValue());
                        $this->com_kgsbruto->SetValue($this->ds->com_kgsbruto->GetValue());
                        $this->com_beneficiary->SetValue($this->ds->com_beneficiary->GetValue());
                        $this->com_embarque->SetValue($this->ds->com_embarque->GetValue());
        ...
        if($this->FormSubmitted || $this->CheckErrors()) {
            ...
            $Error .= $this->com_payment->Errors->ToString();
            $Error .= $this->com_business->Errors->ToString();
            $Error .= $this->com_kgsneto->Errors->ToString();
            $Error .= $this->com_kgsbruto->Errors->ToString();
            $Error .= $this->lbbeneficiary->Errors->ToString();
            $Error .= $this->com_beneficiary->Errors->ToString();
            $Error .= $this->lbembarque->Errors->ToString();
            $Error .= $this->com_embarque->Errors->ToString();
        ...
        global $Tpl;
	global $gsModulId;
	$slFecComp = CCFormatDate($this->com_FecContab->getValue(), split(',',"yyyy,/,m,/,d"));
        $slFecHoy  = date("Y/m/d");
        $ilDias=floor(abs(strtotime($slFecHoy) - strtotime($slFecComp))/86400);
	if (isset($_SESSION['restr'][$gsModulId]['DUR']) ) $ilRest= $_SESSION['restr'][$gsModulId]['DUR'] ;
	else $ilRest = 1;
	if (fGetParam("pAppDbg", false)) {
			echo "<br><br> Fecha Contab: "  . $slFecComp . "  :  "; print_r($this->com_FecContab->getValue());
			echo "<br><br> Fecha Conv: " ; echo $slFecComp;
			echo "<br><br> Fecha Hoy: " ; echo $slFecHoy . " / " . strtotime($slFecHoy) .  " / " . strtotime($slFecComp);
			echo "<br><br> Fecha Conv: " ; echo CCFormatDate($this->com_FecContab->getValue(), split(',',"d,/,m,/,yyyy"));
			echo "<br><br> PERFIL: " ; print_r( $_SESSION);
			echo "<br><br> DIAS: " .$ilDias  . " /  RESTRICCION $gsModulId:" . $ilRest;
	} 		
...
//        fHabilitaCCS($pPagina, 'DUP', $goTrans->f('cla_Duplicable')); // siempre habilitar duplicacicones en funcion del ususario y transacc
        ...
        //$ldFecComp = str2date($this->com_FecContab->Value1[1] . )
//$dias_diferencia = $segundos_diferencia / (60 * 60 * 24); 
        $this->Button_Insert->Visible = !$this->EditMode && $this->InsertAllowed;
        $this->Button_Update->Visible = $this->EditMode && $this->UpdateAllowed;
        $this->Button_Delete->Visible = $this->EditMode && $this->DeleteAllowed;
		if (abs($ilDias) < $ilRest){  /// Prueba
			if (fGetParam("pAppDbg", false)) echo " PERMITIDO $ilEstado  dias: $ilDias restric:$ilRest";
			fHabilitaBotonesCCS(false, $aOpc, $ilEstado );
			fHabilitaBotonesCCS(false, 'DUP', $goTrans->f('cla_Duplicable') );
		}
		else{
			if (fGetParam("pAppDbg", false)) echo " BLOQUEADO $ilEstado dias: $ilDias restric:$ilRest";
			fHabilitaBotonesCCS(false, $aOpc, false );
			fHabilitaBotonesCCS(false, 'DUP', $goTrans->f('cla_Duplicable') );
			$this->Errors->addError("SOLO PARA CONSULTA!! (Ud no puede modificar este comprobante)");
			$this->com_EstOperacion->setValue(-5);
		}
        //----------
        ...
        $this->com_Valor->Show();
        $this->com_Libro->Show();
        $this->com_payment->Show();
        $this->com_business->Show();
        $this->com_kgsneto->Show();
        $this->com_kgsbruto->Show();
        $this->lbbeneficiary->Show();
        $this->com_beneficiary->Show();
        $this->lbembarque->Show();
        $this->com_embarque->Show();
...
//Class_Initialize Event @70-B479E5A5
    function clsInTrTr_compDataSource()
    {
        ...
        $this->com_payment = new clsField("com_payment", ccsInteger, "");
        $this->com_business = new clsField("com_business", ccsInteger, "");
        $this->com_kgsneto = new clsField("com_kgsneto", ccsText, "");
        $this->com_kgsbruto = new clsField("com_kgsbruto", ccsText, "");
        $this->lbbeneficiary = new clsField("lbbeneficiary", ccsText, "");
        $this->com_beneficiary = new clsField("com_beneficiary", ccsInteger, "");
        $this->lbembarque = new clsField("lbembarque", ccsText, "");
        $this->com_embarque = new clsField("com_embarque", ccsInteger, "");
...
//Open Method @70-23DA9141
    function Open()
    {
        ...
        $this->SQL = "SELECT    com_TipoComp, 
                ...
                cla_Formulario, cla_Informe,        cla_TipoEmisor,     cla_TipoReceptor,   com_payment,
                com_business,   com_kgsneto,			 com_kgsbruto,			com_beneficiary, 	  com_embarque,
                conpersonas.*,  concat_ws(' ',per_Apellidos, per_Nombres) AS txtReceptor  " .
        "FROM (concomprobantes INNER JOIN genclasetran ON concomprobantes.com_TipoComp = genclasetran.cla_tipoComp)
        LEFT JOIN conpersonas ON concomprobantes.com_CodReceptor = conpersonas.per_CodAuxiliar";
...
//SetValues Method @70-F4D799B9
    function SetValues()
    {
        ...
        $this->com_payment->SetDBValue(trim($this->f("com_payment")));
        $this->com_business->SetDBValue(trim($this->f("com_business")));
        $this->com_kgsneto->SetDBValue($this->f("com_kgsneto"));
        $this->com_kgsbruto->SetDBValue($this->f("com_kgsbruto"));
        $this->com_beneficiary->SetDBValue(trim($this->f("com_beneficiary")));
        $this->com_embarque->SetDBValue(trim($this->f("com_embarque")));
...
//die();
        ...
        $this->cp["com_payment"] = new clsSQLParameter("ctrlcom_payment", ccsInteger, "", "", $this->com_payment->GetValue(), 9999, false, $this->ErrorBlock);
        $this->cp["com_business"] = new clsSQLParameter("ctrlcom_business", ccsInteger, "", "", $this->com_business->GetValue(), 9999, false, $this->ErrorBlock);
        $this->cp["com_kgsneto"] = new clsSQLParameter("ctrlcom_kgsneto", ccsText, "", "", $this->com_kgsneto->GetValue(), 0, false, $this->ErrorBlock);
        $this->cp["com_kgsbruto"] = new clsSQLParameter("ctrlcom_kgsbruto", ccsText, "", "", $this->com_kgsbruto->GetValue(), 0, false, $this->ErrorBlock);
        $this->cp["com_beneficiary"] = new clsSQLParameter("ctrlcom_beneficiary", ccsInteger, "", "", $this->com_beneficiary->GetValue(), 0, false, $this->ErrorBlock);
        $this->cp["com_embarque"] = new clsSQLParameter("ctrlcom_embarque", ccsInteger, "", "", $this->com_embarque->GetValue(), 0, false, $this->ErrorBlock);
        ...
        $this->SQL = "INSERT INTO concomprobantes ("
             ...
             . $this->ToSQL($this->cp["com_payment"]->GetDBValue(), $this->cp["com_payment"]->DataType) . ", "
             . $this->ToSQL($this->cp["com_business"]->GetDBValue(), $this->cp["com_business"]->DataType) . ", "
             . $this->ToSQL($this->cp["com_kgsneto"]->GetDBValue(), $this->cp["com_kgsneto"]->DataType) . ", "
             . $this->ToSQL($this->cp["com_kgsbruto"]->GetDBValue(), $this->cp["com_kgsbruto"]->DataType) . ", "
             . $this->ToSQL($this->cp["com_beneficiary"]->GetDBValue(), $this->cp["com_beneficiary"]->DataType) . ", "
             . $this->ToSQL($this->cp["com_embarque"]->GetDBValue(), $this->cp["com_embarque"]->DataType) . ", "
...
//Update Method @70-A829AA99
    function Update()
    {
        ...
        $this->cp["com_payment"] = new clsSQLParameter("ctrlcom_payment", ccsInteger, "", "", $this->com_payment->GetValue(), "", false, $this->ErrorBlock);
        $this->cp["com_business"] = new clsSQLParameter("ctrlcom_business", ccsInteger, "", "", $this->com_business->GetValue(), "", false, $this->ErrorBlock);
        $this->cp["com_kgsneto"] = new clsSQLParameter("ctrlcom_kgsneto", ccsText, "", "", $this->com_kgsneto->GetValue(), "", false, $this->ErrorBlock);
        $this->cp["com_kgsbruto"] = new clsSQLParameter("ctrlcom_kgsbruto", ccsText, "", "", $this->com_kgsbruto->GetValue(), "", false, $this->ErrorBlock);
        $this->cp["com_beneficiary"] = new clsSQLParameter("ctrlcom_beneficiary", ccsInteger, "", "", $this->com_beneficiary->GetValue(), "", false, $this->ErrorBlock);
        $this->cp["com_embarque"] = new clsSQLParameter("ctrlcom_embarque", ccsInteger, "", "", $this->com_embarque->GetValue(), "", false, $this->ErrorBlock);
        ...
        $this->SQL = "UPDATE concomprobantes SET "
             ...
             . "com_payment=" . $this->ToSQL($this->cp["com_payment"]->GetDBValue(), $this->cp["com_payment"]->DataType) . ", "
             . "com_business=" . $this->ToSQL($this->cp["com_business"]->GetDBValue(), $this->cp["com_business"]->DataType) . ", "
             . "com_kgsneto=" . $this->ToSQL($this->cp["com_kgsneto"]->GetDBValue(), $this->cp["com_kgsneto"]->DataType) . ", "
             . "com_kgsbruto=" . $this->ToSQL($this->cp["com_kgsbruto"]->GetDBValue(), $this->cp["com_kgsbruto"]->DataType) . ", "
             . "com_beneficiary=" . $this->ToSQL($this->cp["com_beneficiary"]->GetDBValue(), $this->cp["com_beneficiary"]->DataType) . ", "
             . "com_embarque=" . $this->ToSQL($this->cp["com_embarque"]->GetDBValue(), $this->cp["com_embarque"]->DataType) . ", "
...
$gsModulId = "CoTrTr";
switch(fGetParam("pMod", "CoTrTr")){  			//#fah 11/02/2010   Estaba "pModu" por "pModul"
	case "IN":
		$gsModulId = "InTrTr";
		break;
	case "CO":
