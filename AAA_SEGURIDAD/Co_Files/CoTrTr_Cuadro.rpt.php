...
$sSql = "select   con.com_FecContab,
		  ...
		  IFNULL(fis.valRetAir,0)+IFNULL(fis.valRetAir2,0)+IFNULL(fis.valRetAir3,0) AS ret,
		  ifnull(fis.baseImponible,0)+ifnull(fis.baseImpGrav,0)+ifnull(fis.montoIva,0)-(IFNULL(fis.valRetAir,0)+IFNULL(fis.valRetAir2,0)+IFNULL(fis.valRetAir3,0)) as total,
		  case 
...
$sSql .= " UNION
	   SELECT NULL AS com_FecContab, NULL AS com_CodEmpresa, NULL AS numComprobante, NULL AS codProv, bita.bit_idAux AS  idProvFact, CONCAT(per.per_Apellidos,' ',per.per_Nombres) AS nombreProveedor
		...
		, 'INGRESADA EN BITACORA' AS concepto, bit_valor AS valFactura, NULL AS iva,null as ret, NULL AS total, 'EN TRANSITO' AS estado, NULL AS cpp_saldo, NULL AS usuario
	  FROM bitacora bita
