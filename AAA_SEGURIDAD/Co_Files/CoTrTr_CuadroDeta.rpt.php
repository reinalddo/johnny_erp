...
$sSql = "select   con.com_FecContab,
	  ...
	  left join v_condetxpag vcxp on
		  vcxp.det_codcuenta LIKE (SELECT `genparametros`.`par_Valor1` FROM `genparametros` WHERE (`genparametros`.`par_Clave` = 'CUCXP'))
...
if($rs->EOF){
    ...
    if (!$Tpl->is_cached('CoTrTr_CuadroDeta.tpl')) {
            }
            $Tpl->display('CoTrTr_CuadroDeta.tpl');
}
?>