...
<?php
//print_r($_SESSION);
$pQryCom=fGetParam("pQryCom", false); 
if (strlen($pQryCom) > 0){
     $slCondicion = 	  " WHERE " . $pQryCom ;
} else $slCondicion = 	  " WHERE " . fGetParam("pQry", "com_RegNumero=-9999");
$qry_rs1 =
  "SELECT com_RegNumero AS 'REG',  
	  ...
	  FROM concomprobantes JOIN condetalle on (det_RegNumero = com_RegNumero)
	      ...
	      LEFT JOIN concuentas ON (cue_codcuenta = det_codcuenta) " . $slCondicion;
...
$rs2 = $db->execute($qry_rs2) or die("Error en SQL");
$rs2->MoveFirst();
...
<body style="border: 0px none; margin-left: 0cm; margin-top: 0px; margin-bottom: 0px;">
      <?
  ...
  <table width="97%"  border="0" style="border-collapse: collapse; position:absolute; /*top:22cm;*/ left:0cm; border: 1px thin black; " cellpadding="0" cellspacing="0">
