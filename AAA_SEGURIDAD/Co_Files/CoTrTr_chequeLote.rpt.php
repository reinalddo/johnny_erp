...
function &fDefineQry(&$db, $pQry=false){
    ...
    $alSql[] = "select com_numcomp AS 'COM', com_feccontab ,
						...
						when 1 then concat(day(com_feccontab),' de Enero del ',year(com_feccontab))
						when 2 then concat(day(com_feccontab),' de Febrero del ',year(com_feccontab))
						when 3 then concat(day(com_feccontab),' de Marzo del ',year(com_feccontab))
						when 4 then concat(day(com_feccontab),' de Abril del ',year(com_feccontab))
						when 5 then concat(day(com_feccontab),' de Mayo del ',year(com_feccontab))
						when 6 then concat(day(com_feccontab),' de Junio del ',year(com_feccontab))
						when 7 then concat(day(com_feccontab),' de Julio del ',year(com_feccontab))
						when 8 then concat(day(com_feccontab),' de Agosto del ',year(com_feccontab))
						when 9 then concat(day(com_feccontab),' de Septiembre del ',year(com_feccontab))
						when 10 then concat(day(com_feccontab),' de Octubre del ',year(com_feccontab))
						when 11 then concat(day(com_feccontab),' de Noviembre del ',year(com_feccontab))
						when 12 then concat(day(com_feccontab),' de Diciembre del ',year(com_feccontab))
					    end as 'FEC',
...
function fDirReporte(&$db,$com_TipoComp){
  $sqlString=" select cla_Cheque FROM genclasetran WHERE cla_tipocomp = '".$com_TipoComp."' and cla_indcheque = 1";
  
 /* SELECT * FROM 14_durexporta.`genclasetran` WHERE cla_indcheque = 1 ;
  SELECT cla_Cheque FROM 14_durexporta.`genclasetran` WHERE cla_tipocomp = 'BP';*/
  $rs= fSQL($db, $sqlString);
...
//            $slFecha=  date("F j, Y", fText2Fecha($rs->fields['fecha']));
    ...
    $rpt->putTextWrap(85, $y, 400, 10, strtoupper($sltxt),  $pLead = 0, $pJust = 'left', $pAng = 0, 0);//210
...
//    echo $slCant . " " . $gfValor . " " . $flValor ."<br>";
    $y -=  20; //22
    $rpt->putTextWrap(60, $y, 430, 10, $slCant,  $pLead = 0, $pJust = 'left', $pAng = 0, 0); //205
    $y -= 37;
    $rpt->putTextWrap(18, $y, 300, 10, "Guayaquil,  " . $slFecha,  $pLead = 0, $pJust = 'left', $pAng = 0, 0);//195 
//    $group->lastRec['NOM'];
    $y = $rpt->pdf->y -30;
...
$com_TipoComp=$_GET["com_TipoComp"];
if(isset($com_TipoComp)  && strlen(trim($com_TipoComp))>0)
{
	// llamar a codifgo de comprobantes
	$rs=fDirReporte($db,$com_TipoComp);
	foreach($rs as $k => $row) {
		$array= $row["cla_Cheque"] ;
	}
	if (!$array) fErrorPage('','EL TIPO DE COMPROBANTE NO ESTA ESTABLECIDO PARA ESTE TIPO DE REPORTE ', true,  false);
	//$array = adodb_getall($rs);
	//var_dump($array);
    header('Location: ../Co_Files/'.$array.'?pQryCom='.$slQry);
	
	
}
else { echo(" escoja el tipo de comprobante");}

//echo  $_POST["{com_FecTrans_Name}"]  ;
//echo $_GET["com_TipoComp"];
/*  JVL 140828 */
