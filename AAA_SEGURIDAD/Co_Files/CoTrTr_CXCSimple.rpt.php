...
$pCue_CodCuenta = fGetParam('Cue_CodCuenta','');
...
$sSql = "SELECT	 
        ...
        WHERE det_codcuenta = ".$pCue_CodCuenta;
...
while ($r = $rs->fetchRow()){
       $agValor[$r['det_idauxiliar']][$r['dias']] += $r['det_ValDebito'];
       $agSaldos[$r['det_idauxiliar']][$r['dias']] += $r['txt_valor'];
    }
...
$sSql .= " GROUP BY det_numdocum,
		    det_Codcuenta,
	            cue_Descripcion,
                    ...
                    txt_nombre
	   HAVING sum(det_valdebito - det_ValCredito) <> 0";
$sSql .= " ORDER BY txt_nombre, com_FecContab";
