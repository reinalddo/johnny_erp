<?php
/*
*   Balance de Comprobacion en formato xls 
*   @date	09/02/12    
*   @author     esl
*   Formato para Dinaser
*/
...
//echo $slEsquema;
    ...
    // esl 09/02/12	Termogas - incluir auxiliares de rentas aunque no se saque el reporte a nivel de detalle de auxiliar.
    $aSql[] ="SELECT
		    ...
		    (SUM(round(valcr,2) + 00000000000.00) - SUM(round(valdb,2) + 00000000000.00)) AS SAB,
		    ...
		    per_FecFinal AS PERI
			  ...
			  join conperiodos on per_NumPeriodo = ". $ilPeriodo." AND per_Aplicacion= 'CO'
                WHERE (cue_posicion + if(det_idauxiliar>0,1,0)  <= ". $ilNivel ." )/*Posicion de la cuenta puede mostrar auxiliares*/
	        ".$slEsquema ." 
                 GROUP BY 1,2,3,4,5 
		".  $slSaldoCondicion .
		" ORDER by cuenta, case ifnull(per_codAnterior,'') when '' then concat('',det_idauxiliar) else per_codAnterior end ,/*det_idauxiliar,*/3, 5 ";
...
$tplFile = '../Co_Files/CoAdEf_balcomColumn_SubTotal.tpl';
...
$Tpl->assign("Path", $_SERVER["SCRIPT_NAME"]);
