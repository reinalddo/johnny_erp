...
function before_header(&$rpt, &$hdr){
    $ilTxtSize=10;  //
    $ilLeading=0;  //
    include_once ("RptHeader.inc.php");   // JV entrar a quitar rayas
  }
/**
*   Texto acbecera 
*   @access public
...
function before_group_COM (&$rpt, &$group) {
    ...
    include ("ComHeader2.inc.php");  // esto hay que entrara a quitar rayas
    $rpt->rptOpt['onlyHeader']=false;    // true jv
    $rpt->rptOpt['showHeadings']=2;   // 2  jv
    $rpt->rptOpt['showLines']=3;      // 3  jv
    $alBlankLine=array();
    $alBlankline[]['COM']=' ';
