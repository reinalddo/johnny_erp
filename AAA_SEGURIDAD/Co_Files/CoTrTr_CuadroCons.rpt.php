...
/*    Reporte - Cuadro de Cuentas por Pagar
     ...
     @rev	esl	14/01/2011	no mostrar el reporte para las empresas que no sean consolidadas
 */
...
/* ****************************************************************************
   esl 14/01/2011 Empresas individuales no ven los reportes consolidados
******************************************************************************* */
$sSql= "SELECT e.*,  CASE 
...
$sSql .= " order by empresa, nombreProveedor, secuencial,fechaEmision";
...
if($rs->EOF){
    ...
    if (!$Tpl->is_cached('CoTrTr_CuadroCons.tpl')) {
            }
            $Tpl->display('CoTrTr_CuadroCons.tpl');
}
