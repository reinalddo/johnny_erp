...
function fContabPres($pRNum){
	...
	$trSql = "SELECT
		...
		li.par_valor3 as LIB
		    ...
		    LEFT JOIN genTransacDetal on tra_Id = ".$pTraId."
...
//	    $fNum+=1;
	    if ($tra->tra_Cuota > 0) $lastrec['det_glosa'] = "Cuota No: ".$tra->tra_Cuota;
...
function fGrabaComp($TComp){
    ...
    $slSqlper="SELECT per_PerContable, per_estado,
		      ...
		      WHERE per_Aplicacion ='CO' AND '". $fecComp ."' between per_FecInicial and per_FecFinal";
    ...
    if (!fAgregaComprobante($db, $TComp, 0, 0, $fecComp, $fecComp,
                ...
                $arrData["tra_Valor"], 1, $arrData["tra_Motivo"], 0, $arr["per_Semana"], 5, 1, 0,
 ...
 *	opcionalmente un tipo de comprobante
 ----------------------------------------------------------------------------------------------------------------------------**/
	...
	case 1:		// Aplicar a un comprobante
		...
		//fContabPres(fGetparam('pReg', false));
