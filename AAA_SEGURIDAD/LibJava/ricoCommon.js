	...
	init : function() {
			...
			for (var i=0; i<elements.length; i++) {
  			...
  			var src = elements[i].src;
				if (src.toLowerCase().indexOf("ricocommon.js") == -1) continue;
				src = src.substring(0, src.lastIndexOf('/')+1);
				this.jsDir = src;
				this.cssDir= src.replace(/LibJava\/$/,'css/');
				this.imgDir= src.replace(/LibJava\/$/,'Images/');
				break;
			}
		}
}
...
var RicoUtil = {
   ...
   getInnerText: function(el) {
     ...
     var l = cs.length;                 // fah, cambio deposicion
     var str = "";
     for (var i = 0; i < l; i++) {
       switch (cs[i].nodeType) {
         case 1: //ELEMENT_NODE
           str += (cs[i].tagName.toLowerCase()=='img') ? cs[i].src : this.getInnerText(cs[i]);
           break;
         case 3: //TEXT_NODE
           str += cs[i].nodeValue;
           break;
