...
*   @rev esl	10-Oct-2012	Agregar en funcion fSelectImpuesto() en clausula where condicion para que filtre los que tienen estado 1 y solo muestre impuestos activos
*   @rev esl	18-Oct-2012	Cambiar formato de fechas al seleccionar los datos de autorizacion, mostrar en formato dd-MMM-yy
*
**/
	...
	olAct.onLoad  =  function () {
		...
		olRpc.onLoad  =  function () {
	    	...
	    	if (data.datos.length > 0){
				...
				// slFecha = fConvFecha(pFecha, "dd/MMM/yy", "yyyy-mm-dd"); esl convertir fecha
				//@rev esl	18-Oct-2012	Cambiar formato de fechas al seleccionar los datos de autorizacion, mostrar en formato dd-MMM-yy
				
				fiscompras.tmp_FecEmision.value = fConvFecha(fiscompras.tmp_FecEmision.value, "dd-mm-yyyy", "dd/MMM/yy");
				fiscompras.tmp_FecCaduc.value   = fConvFecha(fiscompras.tmp_FecCaduc.value, "dd-mm-yyyy", "dd/MMM/yy");
				
				
				
				//print(fiscompras.tmp_FecEmision.value);
...
**/
	...
	olAct.procesa(false, slSql, '', '');  // fah Nov/27/09
}
...
function fSelectImpuesto(oCtrl) {
    ...
    gAutosuggestCont=0;
	//@rev esl	07-Sep-2012	Agregar en funcion fSelectImpuesto() en clausula where condicion para que filtre los que tienen estado 1 y solo muestre impuestos activos
	slSql = escape("SELECT concat(tab_Codigo,  ' .-  ', UCASE(tab_Descripcion)),  tab_Codigo,  tab_porcentaje,  + round( " + fParseDec(pCalcu) + " *  tab_porcentaje +  / 100 , 2)" +
					"FROM fistablassri " +
				    "WHERE tab_Estado = 1 and tab_CodTabla = '" + pTabla + "' AND tab_Descripcion LIKE ");
    slCond = escape("");
