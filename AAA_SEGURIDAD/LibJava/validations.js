...
// Sustirumos la siguuiente funcion, pues no trabaja bien
/*
...
*/
/*
*	Analiza cada elemento INPUT o SELECT para determinar si alguno cambio, entonces devuelve true o false
*/
function isFormModified(theform,hidden_fields,ignore_fields){
	if(hidden_fields==null){hidden_fields="";}
	if(ignore_fields==null){ignore_fields="";}
	...
	if(!scan(elements, hidden_fields, ignore_fields)) {
		elements=document.getElementsByTagName('SELECT');
		return scan(elements, hiddenFields, ignoreFields);
	}
	return false;
}

/*
*	Funcion de proceso de ratstreo de elementos de un formulario
*/
var gaModificados= Array(); // 	array de objectos modificados
var gaNumModif=0;//             Numero de elementos modificados
function scan(elements, hiddenFields, ignoreFields) {
	for(i=0;i<elements.length;i++){
		var changed=false;
		var name=elements[i].name;
		fields += ", " + (elements[i].name);
		if(!isBlank(name) && elements[i].type != "button"){
			var type=elements[name].type;
			if(!ignoreFields[name]){
				if(type=="hidden"&&hiddenFields[name]){changed=isChanged(elements[name]);}
				else if(type=="hidden"){changed=false;}
					 else {
					 	changed=isChanged(elements[name]);
						if (changed) {
						gaModificados[gaNumModif] = elements[name]
					   	gaNumModif += 1;
					  	}
				 	}
			}
