...
Rico.LiveGrid.prototype = {
  ...
  initialize: function( tableId, visibleRows, totalRows, url, options, ajaxOptions ) {
    ...
    this.options = {
      ...
      beforeRowHandler : false, /* function name to call when a row gains the focus */
      afterRowHandler  : false, /* function name to call when a row losses the focus */
      ...
      specDecimal      : {type:'number', prefix:'', decPlaces:2, className:'numCol'},
      specDecimal4     : {type:'number', prefix:'',  decPlaces:4, className:'numCol'},
      specDecimal6     : {type:'number', prefix:'',  decPlaces:6, className:'numCol'},
      specDollar       : {type:'number', prefix:'$', decPlaces:2, className:'numCol'},
      specEuro         : {type:'number', prefix:'&euro;', decPlaces:2, className:'numCol'},
      specPercent      : {type:'number', suffix:'%', decPlaces:2, multiplier:100, className:'numCol'},
  ...
  createTables: function(visibleRows,hdrtableId) {
    ...
    if (this.options.headingRow > headerRows.length) alert ("Error en definicion de la propiedad 'headingRow'"); //fah
    ...
    for (r=0; r<headerRows.length; r++) {
      ...
      if (r==this.options.headingRow) {
        ...
        for ( var i = 0 ; i < this.headerColCnt; i++ )
          colWidths.push(headerCells[i].style.width ? headerCells[i].style.width : headerCells[i].offsetWidth); // fah
      }
    }
...
//    this.spacerDiv.style.height=hdrHt+'px';
    this.spacerDiv.style.width=totw + 5 +'px';  // fah
//    this.outerDiv..style.width=totw + 5 +'px';  // fah
  ...
  rowMouseOver: function(e) {
    ...
    if (this.lastEditedRow >= 0) return true;
    var r=Event.element(e);
    if (this.useIEevents==false)
      r=RicoUtil.getParentByTagName(r,this.options.rowTag);
    if (!r) return;
    if(this.lastEditedRow < 0)   this.rowHighlight(r.rowIndex);
    else if (this.lastEditedRow >= 0 ){
             var blOK = true;
             if (this.options.beforeSaveHandler)
                blOK=this.options.beforeSaveHandler(this, r.rowIndex - this.headerRowCnt);
             if (blOK)
                this.saveRow(r, "u", r.rowIndex - this.headerRowCnt);
        if (this.options.beforeRowHandler)      // fah: register the functionto manage
            this.options.beforeRowHandler(this, r.rowIndex - this.headerRowCnt);
    }
  },
  ...
  rowMouseOut: function(e) {
    ...
    if (this.lastEditedRow ==-1)    this.rowUnhighlight(r.rowIndex);
    var blOK=true;
    if (this.options.afterRowHandler)      // fah: register the functionto manage
        blOk = this.options.afterRowHandler(this, r.rowIndex - this.headerRowCnt);
    if (this.lastEditedRow >= 0 ){
        if (blOK && this.options.beforeSaveHandler )
            blOK = this.options.beforeSaveHandler(this, r.rowIndex - this.headerRowCnt);
        if( blOK ) this.saveRow(this, "u", r.rowIndex - this.headerRowCnt);
       }
  },
...
Rico.TableCell.prototype = {
  initialize: function(fCell,sCell,objColumn) {
    ...
    if (this.column.format.type == "number"){   //fah
        fCell.style.textAlign="right";          //fah
        sCell.style.textAlign="right";          //fah
    }
    ...
    slFunctionName = "window." + this.column.fieldName + "_onClick";                       //fah
    if (eval(slFunctionName))  {                                                    //fah
        this.clickHandler = eval(slFunctionName +".bindAsEventListener(this)");     // fah
        Event.observe(this.cell,"click",  this.clickHandler , false);                    //fah
    }
...
Rico.TableColumn.prototype = {
  ...
  createDataMenu: function(r) {
    ...
    if (this.sortable) {
      ...
      gridMenu.addMenuItem(RicoTranslate.getPhrase("Ascending"), this.sortAsc.bind(this), true);
      gridMenu.addMenuItem(RicoTranslate.getPhrase("Descending"), this.sortDesc.bind(this), true);
    }
