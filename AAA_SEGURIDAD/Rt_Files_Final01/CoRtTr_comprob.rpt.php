...
function &fDefineQry(&$db, $pQry=false)
{
    ...
    $alSql = Array();
    $alSql[] = "/*ESTA ES LA RETENCION */SELECT 	ID as ID,
			idProvFact COD,
			concat(pv2.per_Apellidos, ' ', pv2.per_Nombres) NOM,
			UPPER(date_format(fechaEmiRet1,'%d-%M-%y')) FEC,
			year(fechaRegistro) PER,
			ifNULL(pv2.per_Ruc,'********') as RUC,
			pv2.per_Direccion as Direcc,
			concat(ifNULL(pv2.per_Direccion,' '), '  / Telf:', ifNULL(pv2.per_Telefono1,' ')) as DIR,
