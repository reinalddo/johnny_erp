...
*   @rev	esl 25/11/11	Incluir las compras de fruta dentro de anexo de compras
*/
...
function &fDefineCompras(&$db, $pQry=false){
	...
	FROM fiscompras
			...
			/* WHERE tipoTransac = 1 */ /*CAMBIO PARA FRUTIBONI - ESL 25/NOV/2011 */
			WHERE tipoTransac IN (1,8 /*Transaccion 8 es compra de fruta que tambien se incluye*/ )
...
function &fDefineExport(&$db, $pQry=false){
   ...
   $alSql .=" UNION
		...
		WHERE dau_estado = 1 ";
	$condicion = str_replace("YEAR(fechaRegistro)","dau_aniocarga",$pQry);
	$condicion = str_replace("MONTH(fechaRegistro)","dau_mescarga",$condicion);
	if (strlen($pQry) > 0 ) $alSql .= " AND "  . $condicion ;
