	...
	getEditor: function(cache) {
		...
		if (this.editor == null || !cache) {
			...
			myField.validateValue = function(value) {
				...
				if (myField.value === undefined || myField.value === null) val = "";
				if (originalFunc.call(this, val) === false) return false;
...
Ext.extend(Ext.ux.netbox.core.Field, Ext.util.Observable, {
	...
	getAvailableOperatorById: function(id) {
		...
		else return (this.getAvailableOperators()[index]);
	},
	...
	createEditor: function(operatorId) {
		...
		if (!this.isAvailableValuesAvailable()) {
			...
			editor = new Ext.ux.netbox.core.AvailableValuesEditor(this.getAvailableValues(), {
				remote: this.isStoreRemote(),
				forceReload: this.isForceReload(),
	...
	emptyNotAllowedFn: function(value) {
		if (!value) return (this.emptyNotAllowed);
		if (!value.length) return (this.emptyNotAllowed);
		return (true);
	},
...
Ext.extend(Ext.ux.netbox.core.ElementaryFilter, Ext.ux.netbox.core.Filter, {
	...
	getFilterObj: function() {
		return ({
			...
			operatorId: this.getOperator().getId(),
	...
	getElementaryFiltersByFieldId: function(fieldId) {
		if (this.getField().getId() == fieldId) return ([this]);
		return ([]);
	},
	isValid: function() {
		if (this.values !== undefined && this.getOperator().validate(this.values) === true) return true;
		else return false;
	}
});
...
Ext.extend(Ext.ux.netbox.core.CompositeFilter, Ext.ux.netbox.core.Filter, {
	...
	getElementaryFilterById: function(id) {
		...
		if (this.getRightSide() != null) return (this.getRightSide().getElementaryFilterById(id));
		return null;
	},
	getElementaryFiltersByFieldId: function(fieldId) {
		var toReturn = this.getLeftSide().getElementaryFiltersByFieldId(fieldId);
		if (this.getRightSide() != null) toReturn = toReturn.concat(this.getRightSide().getElementaryFiltersByFieldId(fieldId));
		return toReturn;
	}
});
...
Ext.ux.netbox.core.FieldManager = function(config) {
	...
	this.fields = new Ext.util.MixedCollection(false,
	function(field) {
		return (field.getId())
	});
	if (config !== undefined) {
...
Ext.extend(Ext.ux.netbox.core.FieldManager, Ext.util.Observable, {
	...
	createFieldFromCfg: function(fieldCfg) {
		...
		case "xdatefield":
		case "xdatetime":
		case "datetime":  // @TODO: Configurar campo xdAtetime
		...
		if (fieldCfg.defaultValues !== undefined) field.setDefaultValues(fieldCfg.defaultValues);
		if (fieldCfg.forceReload !== undefined) field.setForceReload(fieldCfg.forceReload);
		if (fieldCfg.caseSensitive !== undefined) field.setCaseSensitive(fieldCfg.caseSensitive);
		if (fieldCfg.validate !== undefined) field.setValidateFn(fieldCfg.validate);
		return (field);
	},
	...
	removeField: function(field) {
		if (this.fields.containsKey(field.getId())) {
			if (this.fireEvent("beforeFieldRemoved", field) !== false) {
				if (this.fields.removeKey(field.getId())) this.fireEvent("fieldRemoved", field);
			}
		}
	},
	removeAll: function() {
...
Ext.ux.netbox.core.FilterModel = function(config) {
	...
	this.filter = null;
	if (config instanceof Ext.ux.netbox.core.FieldManager) this.fieldManager = config;
	else this.fieldManager = new Ext.ux.netbox.core.FieldManager(config);
	this.fieldManager.on("beforeFieldRemoved", this.onBeforeFieldRemoved, this);
	if (config.logicalOperator === undefined) this.logicalOperator = Ext.ux.netbox.core.CompositeFilter.AND;
	else this.logicalOperator = config.logicalOperator;
}
Ext.extend(Ext.ux.netbox.core.FilterModel, Ext.util.Observable, {
	onBeforeFieldRemoved: function(field) {
		if (this.getElementaryFiltersByFieldId(field.getId()).length > 0) return (false);
	},
	...
	_addFilter: function(elementaryFilter) {
		if (this.getFilter() == null) this.filter = elementaryFilter;
		else this.filter = new Ext.ux.netbox.core.CompositeFilter(this.getFilter(), this.logicalOperator, elementaryFilter);
	...
	getFilterObj: function(evenInvalid, additionalFilterObj, additionalLogicalOper) {
		...
		if (additionalLogicalOper === undefined) additionalLogicalOper = Ext.ux.netbox.core.CompositeFilter.AND;
		var filter = this.getFilter();
		var filterToExport = null;
		if (this.getFilter() !== null) if (filter === null) filterToExport = additionalFilter;
		else {
		...
		if (filterToExport != null) return (this._encodeFilter(filterToExport));
		else return null;
	},
	...
	getElementaryFilterById: function(id) {
		if (this.getFilter() != null) return (this.getFilter().getElementaryFilterById(id));
		return (null);
	},
	getElementaryFiltersByFieldId: function(fieldId) {
	...
	removeElementaryFilterById: function(filterId) {
		...
		if (this.getFilter() == null) throw ("Unable to remove the elementaryFilter with id " + filterId + ". The elementaryFilter doesn't exist.");
	...
	getAllElementaryFilters: function(filter) {
		if (filter === undefined) filter = this.getFilter();
	...
	setLogicalOperator: function(logicalOperator) {
		if (logicalOperator === Ext.ux.netbox.core.CompositeFilter.OR || logicalOperator === Ext.ux.netbox.core.CompositeFilter.AND) this.logicalOperator = logicalOperator;
	},
...
Ext.extend(Ext.ux.netbox.core.DynamicFilterModelView, Ext.grid.EditorGridPanel, {
	...
	valueText: 'Valor',
	comboText: 'Seleccione un campo',
	logicOpeAndText: 'Cumplir todos',
	logicOpeOrText: 'Cumplir al menos uno',
	...
	onFilterAdded: function(filterModel, filter) {
		var filterRecord = [];
		filterRecord.push(['', filter.getField(), filter.getOperator().getId(), filter.getValues(), filter, filter.getId()]);
	...
	createLogicOpeCombo: function() {
		var logicOpeStore = new Ext.data.SimpleStore({
			fields: ['label', 'value'],
			data: [[this.logicOpeAndText, Ext.ux.netbox.core.CompositeFilter.AND], [this.logicOpeOrText, Ext.ux.netbox.core.CompositeFilter.OR]]
		});
	...
	setLogicOpeCombo: function() {
		...
		if (filter instanceof Ext.ux.netbox.core.CompositeFilter) this.logicOpeCombo.setValue(filter.getLogicalOperator());
	},
	...
	chgLogicOpe: function(combo, record, index) {
		...
		this.getFilterModel().each(function(filter) {
	...
	updateOperatorStore: function(e) {
		if (e.column == 2) {
			...
			for (var i = 0; i < field.getAvailableOperators().length; i++) {
				operators.push([field.getAvailableOperators()[i].getId(), field.getAvailableOperators()[i].getLabel().escHtml()]);
			}
...
Ext.extend(Ext.ux.netbox.core.StaticFilterModelView, Ext.form.FormPanel, {
	...
	operatorChanged: function(filter, operator) {
		...
		var editorComponent = elementaryFilterCfg.getEditorComponent();
		if (operatorCombo.getValue() != operator.getId()) operatorCombo.setValue(operator.getId());
		var panelNum = this.getEditorPanelNumber(field);
		var editor = filter.getOperator().getEditor(false);
		this.changeEditor(editor, elementaryFilterCfg);
		var formField = elementaryFilterCfg.getEditor().field;
...
Ext.ux.netbox.core.ElementaryFilterCfg.prototype = {
	...
	setEditor: function(editor) {
		this.editor = editor;
		if (Ext.isSafari) this.editor.field.anchor = "90%";
		else this.editor.field.anchor = "98%";
		this.editor.field.labelSeparator = '';
		if (this.labelAlign == "top") {
			if (Ext.isIE) this.editor.field.fieldLabel = "<pre> </pre>";
			else {
...
Ext.ux.netbox.core.AvailableValuesEditor = function(store, config) {
	...
	var mode = 'local';
	if (config.remote == true) mode = 'remote';
	...
	if (config.forceReload) {
		this.fieldCombo.on("beforequery",
		function(qe) {
			qe.combo.lastQuery = null;
		});
	}
	if (config.caseSensitive) this.caseSensitive = true;
...
Ext.extend(Ext.ux.netbox.core.AvailableValuesEditor, Ext.ux.netbox.FilterEditor, {
	...
	getValue: function() {
		...
		if ((val.length > 0 && val[0] !== "") && toReturn.length == 0) return (this.originalValue);
...
Ext.extend(Ext.ux.netbox.string.StringListOperator, Ext.ux.netbox.core.Operator, {
	createEditor: function(operatorId) {
		var editor = new Ext.ux.netbox.core.AvailableValuesEditor(this.getField().getAvailableValues(), {
			remote: this.isStoreRemote(),
			forceReload: this.isForceReload(),
			multiSelect: true,
			caseSensitive: this.isCaseSensitive()
...
Ext.ux.netbox.string.StringField = function(id, label) {
	...
	op.addValidateFn(noEmptyAllowed);
	this.addOperator(op);
	this.setDefaultOperator(op); // fah
...
Ext.extend(Ext.ux.netbox.string.StringField, Ext.ux.netbox.core.Field, {
	...
	doesntContainsText: "no Contiene",
	startsWithText: "Inicia con",
	endsWithText: "Termina con",
	stringListText: "incluye a  ",
	stringNotListText: "excluye a "
});
...
Ext.extend(Ext.ux.netbox.number.NumberRangeOperator, Ext.ux.netbox.core.Operator, {
	...
	toText: 'Hasta',
	includeText: 'Entre',
	bothFromAndToNotEmpty: "'Desde' y 'Hasta' deben tener valor",
...
Ext.extend(Ext.ux.netbox.number.NumberField, Ext.ux.netbox.core.Field, {
	createEditor: function(operatorId) {
		var editor = new Ext.ux.netbox.core.TextValuesEditor(new Ext.form.NumberField({
			decimalPrecision: 10
		}));
		return editor;
	}
...
Ext.extend(Ext.ux.netbox.date.DateOperator, Ext.ux.netbox.core.Operator, {
	createEditor: function(operatorId) {
		...
		if (splittedFormat.length > 1) {
			...
			editor = new Ext.ux.netbox.date.DateTextEditor(dateTimeField, {
				format: this.format
			});
		} else {
...
Ext.extend(Ext.ux.netbox.date.DateRangeOperator, Ext.ux.netbox.date.DateOperator, {
	...
	createEditor: function(operatorId) {
		...
		var editor = new Ext.ux.netbox.date.DateRangeEditor(field, {
			format: this.format
		});
		field.on("editingcompleted", editor.completeEdit, editor);
		return editor;
	...
	getTextFieldConfig: function() {
		return ({
			plugins: [new Ext.ux.netbox.InputTextMask(this.calculateMask(), true)]
		});
	},
...
Ext.ux.netbox.date.DatePeriodOperator = function() {
	...
	this.periodStore = new Ext.data.SimpleStore({
		fields: ['value', 'label'],
		data: [["LAST_QUARTER", this.quarterText], ["LAST_HOUR", this.hourText], ["LAST_DAY", this.dayText], ["LAST_WEEK", this.weekText], ["LAST_MONTH", this.monthText], ["LAST_YEAR", this.yearText]]
	});
	var validateFn = function(value) {
		if (this.getField().emptyNotAllowedFn(value) !== true) {
			return (this.getField().emptyNotAllowedFn(value));
		}
		if (value[0].value !== "LAST_QUARTER" && value[0].value !== "LAST_HOUR" && value[0].value !== "LAST_DAY" && value[0].value !== "LAST_WEEK" && value[0].value !== "LAST_MONTH" && value[0].value !== "LAST_YEAR") {
...
Ext.extend(Ext.ux.netbox.core.RangeField, Ext.form.TriggerField, {
	...
	getValue: function() {
		if (this.menu !== undefined) this.rangeValue = this.menu.getValue();
		return (this.rangeValue);
	},
	setValue: function(val) {
		valueFrom = val[0] !== undefined ? val[0] : {
			value: '',
			label: ''
		};
		valueTo = val[1] !== undefined ? val[1] : {
			value: '',
			label: ''
		};
...
Ext.extend(Ext.ux.netbox.core.RangeMenu, Ext.menu.Menu, {
	...
	getValue: function() {
		var result = [{
			value: this.editorFrom.getValue(),
			label: this.editorFrom.getValue()
		},
		{
			value: this.editorTo.getValue(),
			label: this.editorTo.getValue()
		}];
		return result;
	},
	isValid: function() {
...
Ext.ux.netbox.core.LocalStoreFilterResolver = function(filterModel, mapping) {
	...
	if (mapping == undefined) {
		this.mapping = {
			NUMBER_EQUAL: {
				fn: this.filterNumberEqual,
				scope: this
			},
			NUMBER_NOT_EQUAL: {
				fn: this.filterNumberDifferent,
				scope: this
			},
			NUMBER_GREATER: {
				fn: this.filterNumberGreater,
				scope: this
			},
			NUMBER_GREATER_OR_EQUAL: {
				fn: this.filterNumberGreaterOrEqual,
				scope: this
			},
			NUMBER_LESS: {
				fn: this.filterNumberLess,
				scope: this
			},
			NUMBER_LESS_OR_EQUAL: {
				fn: this.filterNumberLessOrEqual,
				scope: this
			},
			NUMBER_RANGE: {
				fn: this.filterNumberRange,
				scope: this
			},
			STRING_EQUAL: {
				fn: this.filterStringEquals,
				scope: this
			},
			STRING_DIFFERENT: {
				fn: this.filterStringDifferent,
				scope: this
			},
			STRING_CONTAINS: {
				fn: this.filterStringContains,
				scope: this
			},
			STRING_DOESNT_CONTAIN: {
				fn: this.filterStringDoesntContains,
				scope: this
			},
			STRING_STARTS_WITH: {
				fn: this.filterStringStartsWith,
				scope: this
			},
			STRING_ENDS_WITH: {
				fn: this.filterStringEndsWith,
				scope: this
			},
			STRING_LIST: {
				fn: this.filterList,
			...
			STRING_NOT_IN_LIST: {
				fn: this.filterNotInList,
				scope: this
			},
			DATE_EQUAL: {
				fn: this.filterDateEqual,
				scope: this
			},
			DATE_GREATER: {
				fn: this.filterDateGreater,
				scope: this
			},
			DATE_GREATER_OR_EQUAL: {
				fn: this.filterDateGreaterOrEqual,
				scope: this
			},
			DATE_LESS: {
				fn: this.filterDateLess,
				scope: this
			},
			DATE_LESS_OR_EQUAL: {
				fn: this.filterDateLessOrEqual,
				scope: this
			},
			DATE_RANGE: {
				fn: this.filterDateRange,
				scope: this
			},
			DATE_PERIOD: {
				fn: this.filterDatePeriod,
				scope: this
			}
		};
	}
...
Ext.ux.netbox.core.LocalStoreFilterResolver.prototype = {
	...
	filterDatePeriod: function(record, value, column) {
		...
		var upper = new Date();
		upperValue = {
			label: upper.format('Y-m-d H:i:s'),
			value: upper.format('Y-m-d H:i:s')
		};
		var lower;
		...
		var lowerValue = {
			label: lower.format('Y-m-d H:i:s'),
			value: lower.format('Y-m-d H:i:s')
		};
	...
	filter: function(record, id, filterObj) {
		if (filterObj == undefined) filterObj = this.filterModel.getFilterObj();
		if (filterObj == null) return (true);
		if (filterObj.operatorId != undefined) {
			...
			if (ret === true && filterObj.logicalOperator === Ext.ux.netbox.core.CompositeFilter.OR) return true;
			if (ret === false && filterObj.logicalOperator === Ext.ux.netbox.core.CompositeFilter.AND) return (false);
...
Ext.extend(Ext.ux.netbox.core.QuickFilterModelView, Ext.util.Observable, {
		...
		if (column == -1 || this.getField(grid, column) == null) {
			...
			for (var i = 0; i < availableOperatorsId.length; i++) {
				...
				for (var j = 0; j < filtersList.length; j++) {
					if (filtersList[j].getOperator().getId() === operator.getId() && Ext.util.JSON.encode(filtersList[j].getValues()) === Ext.util.JSON.encode(values)) isToAdd = false;
				}
				if (operator.validate(values) !== true) isToAdd = false;
				if (isToAdd) itemsArray.push(filterItem);
			}
			if (itemsArray.length > 0) {
				this.quickFilterItem.setSubMenu(new Ext.menu.Menu({
					items: itemsArray
				}));
				return true;
			} else {
	...
	getValues: function(grid, row, column, fieldId, operatorId) {
		...
		if (this.fieldsOptions) {
			for (var i = 0; i < this.fieldsOptions.length; i++) {
				if (this.fieldsOptions[i].id === fieldId && this.fieldsOptions[i].getterFn) {
					...
					if (this.fieldsOptions[i].getterScope) getterScope = this.fieldsOptions[i].getterScope;
				}
			}
		}
		if (!getterFn) {
		...
		if (!getterScope) getterScope = this;
	...
	getValuesOther: function(tableValue, fieldId, operatorId, grid, row, column) {
		...
		var record = grid.getStore().getAt(row);
		var label = rendererFn(tableValue, {},
		record, row, column, grid.getStore());
		return ([{
			label: label,
			value: tableValue
		}]);
	...
	getValuesDate: function(tableValue, fieldId, operatorId, grid, row, column) {
		...
		var operator = field.getAvailableOperatorById(operatorId);
		return ([{
			label: tableValue.format(operator.getFormat()),
			value: tableValue.format('Y-m-d H:i:s')
		}]);
	},
	setFilter: function(grid, row, column, fieldId, operatorId) {
		...
		if (!this.duplicatedElementaryFiltersAllowed) {
			...
			for (var i = 0; i < elementaryFilters.length; i++) {
				if (elementaryFilters[i].getOperator().getId() === operatorId && Ext.util.JSON.encode(elementaryFilters[i].getValues()) === Ext.util.JSON.encode(values)) {
					addFilter = false;
					break;
				}
			}
	...
	removeFilterIsToShow: function(grid, row, column) {
		...
		if (filters.length > 0) {
			...
			for (var i = 0; i < filters.length; i++) {
				...
				var iconCls = '';
				if (filters[i].isValid() !== true) iconCls = 'x-icon-invalid';
				var filterItem = {
					iconCls: iconCls,
					text: label,
			...
			this.removeFilterItem.setSubMenu(new Ext.menu.Menu({
				items: itemsArray
			}));
			return true;
		} else {
		...
		if (this.store === undefined) {
			this.store = new Ext.data.Store({
				...
				reader: new Ext.data.JsonReader({
					...
					fields: ['prefId', 'prefName', 'prefDesc', {
						name: 'isDefault',
						type: 'boolean'
					}]
				})
...
Ext.ux.netbox.PreferenceManagerView = function(config) {
	...
	this.on('show', this.loadRemotePref, this, {
		single: true
	});
};
Ext.extend(Ext.ux.netbox.PreferenceManagerView, Ext.menu.Menu, {
	...
	createStableItems: function() {
		if (this.items.getCount() == 0) {
			this.add({
				...
				handler: this.addPreference,
				scope: this
			},
			{
				text: this.manageText,
				tooltip: this.manageTooltipText,
				handler: this.showManageDialog,
				scope: this
			},
			'-');
		}
	...
	resetMenu: function() {
		...
		for (var i = this.items.getCount() - 1; i >= 0; i--) {
			if (this.items.get(i).removable === true) {
				if (this.items.get(i).getEl() && this.items.get(i).getEl().isMasked()) this.items.get(i).getEl().unmask();
	...
	loadRemotePrefAsync: function() {
		...
		for (var i = 0; i < this.prefStore.getTotalCount(); i++) {
			...
			var iconCls = '';
			if (rec.get('isDefault') == true) iconCls = 'x-icon-checked';
			this.add({
				id: rec.get('prefId'),
	...
	managePreference: function() {
		...
		if (record) this.showAddDialog(record.get('prefId'), record.get('prefName'), record.get('prefDesc'), record.get('isDefault'));
	},
	...
	showAddDialog: function(prefId, prefName, prefDesc, isDefault) {
		...
		if (prefId != '') this.addDialog.setTitle(this.modifyText);
		else this.addDialog.setTitle(this.addText);
		this.addDialog.show();
	},
	...
	imageRenderer: function(value, metadata, record, rowIndex, colIndex, store) {
		if (value == true) return ('<img class="x-menu-item-icon x-icon-checked" src="' + Ext.BLANK_IMAGE_URL + '"/>');
	},
...
Ext.ux.netbox.core.FilterHeaderPlugin.prototype = {
	...
	updateColumnHeadings: function(view) {
		...
		for (var i = 0,
		len = view.cm.config.length; i < len; i++) {
