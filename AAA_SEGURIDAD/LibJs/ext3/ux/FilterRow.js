...
Ext.ux.grid.FilterRow = Ext.extend(Ext.util.Observable, {
  ...
  renderFields: function() {
    this.eachFilterColumn(function(col) {
      ...
      //var editor = col.editor || col.filter.getField();       // #fah22/06/2011
      var editor = col.filter.getField();
...
Ext.ns("app.gen");
/*
 *  Create a global preconfigured Singleton to use with any grid.
 *  This plugin have better performance that default because "waits" last typed character to send the ajax request.
 */
app.gen.preConfigFilterRow =  Ext.extend(Ext.ux.grid.FilterRow,{  //new Ext.ux.grid.FilterRow({
    autoFilter: false
    ,minChars:  3
    ,minDelay:  400
    ,lastTime:  0
    ,listeners: {
        change: function(pData) {
            this.filterData = pData
            if (Ext.EventObject.ENTER == Ext.EventObject.keyCode)
              this.filterApply();
        }
    }
    ,filterApply: function(){
        var olPar = this.grid.getStore().lastOptions || {};
        var alFilter=[];
        this.lastTime=0;
        if (!olPar.params) olPar.params={};
        olPar.params.headFilter = [];
        var i = 0;
        olPar.start = this.filterData.start || 0;
        var cm = this.grid.getColumnModel()

        for (var slFld in this.filterData)  {
           if (this.filterData[slFld].length >0) {
               //var olCol = cm.getColumnById(slFld);
               var olCol = cm.config[cm.config.findIndexByCol(slFld, "name")];
               if (olCol != undefined){

               alFilter[i] = {field:slFld
                  ,data:{
                      type: 	olCol.type || "string"
                      ,value: this.filterData[slFld]|| ""
                      }}
               i += 1 //olPar.params.filter;
               }
           }
        }
        if (alFilter.length > 0) olPar.params.headFilter = alFilter; //Ext.util.JSON.encode(alFilter);
        this.grid.getStore().load(olPar);
    }
});
