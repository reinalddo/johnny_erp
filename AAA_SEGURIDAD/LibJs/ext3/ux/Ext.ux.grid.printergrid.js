							...
							if ( jval>(this.grid.getColumnModel().config.length-1) ){
									...
									if ( tmpchkhidden ) {
										...
										if ( this.excludefields.indexOf('' + numcolref + ',')>=0 ){
											...
											//switch (this.grid.store.getAt(0).fields.items[numcolref-1].type){  #fah
											switch (this.grid.store.getAt(0).fields.items[numcolref].type){
											    case "string":
											    case "date":
											...
											if (this.usenameindex==false){
												...
												if (this.grid.store.getAt(0).fields.items[numcolref].type == 'date'){
