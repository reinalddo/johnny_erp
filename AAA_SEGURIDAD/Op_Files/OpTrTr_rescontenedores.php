<?php
/*
*   Resumen de cantidades embarcadas por contenedor
...
function &fDefineQry(&$db, $pQry=false){
   ...
   $sql = " select txp_NombBuque, txp_refoperativa, txp_Contenedor, cnt_CantDeclarada,
            sum(txp_cantneta) as tmp_Cantidad
            from v_opetarjexpand join liqcajas on caj_codcaja = txp_codcaja
            left join opecontenedores on cnt_embarque = txp_refOperativa and cnt_serial = txp_contenedor
            where " . $gsCond. "
            group by 1,3
            order by 1,3
            " ;
...
$pSem = fGetParam('emb_SemInicio', false);
//$pEmb = fGetParam('pEmb', false);
$pEmb = fGetParam('emb_CodVapor', false);
...
$tplFile = 'OpTrTr_rescontenedores.tpl';
//rs2html($rs);
$rs->MoveFirst();
$aDet =& SmartyArray($rs);
//obsafe_print_r($aDet);
