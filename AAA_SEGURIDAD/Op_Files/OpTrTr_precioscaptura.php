...
*   @rev    fah 13/May/08   Se define el flete como on atributo del detalle de tarja
**/
...
switch ($gsSqlId){
    case ("pro"): //           Precios  a  nivel Productor
        ...
        WHERE txp_refOperativa = {pEmb} AND {filter}
    ...
    case ("mar"): //           Precios  a  nivel de marca
        ...
        "SELECT txp_Semana,
            ...
            sum(txp_CantNeta * txp_PrecUnit ) as tmp_Valor
        FROM v_opetarjexpand 
        WHERE txp_refOperativa = {pEmb} AND txp_Embarcador = {pProd} AND txp_CodProducto = {pPrdc} AND  {filter}
...
if (fGetParam("init", false) != false) {
  ...
  // @TODO: Hacer que la semana sea un parametro enviado al datasource no a este script
    $loadJSFile = "../Op_Files/OpTrTr_precioscaptura"; // Script a cargar en loadgrid.php
        ...
        function beforeGetData(){
            ...
            switch ($gsSqlId){           
                case "pro":
                    ...
                    $goGrid->setFieldOpt('txp_Producto', array("header"=>"PRODUCTO", "hidden"=>0, "width"=>35));
                ...
                case "mar": //           Precios  a  nivel Marca
                    ...
                    $goGrid->setFieldOpt('txp_CajDescrip', array("header"=>"CAJA", "hidden"=>0, "width"=>50));
                    $goGrid->setFieldOpt('txp_Fecha',   array("header"=>"FECHA", "hidden"=>1, "width"=>50));
                    $goGrid->setFieldOpt('txp_NumTarja', array("header"=>"TARJA NUM", "hidden"=>1, "width"=>50));
                    break;
                case "tar": //           Precios  a  nivel Detalle
                    ...
                    $goGrid->setFieldOpt('txp_NumTarja', array("header"=>"TARJA NUM", "hidden"=>0, "width"=>50));
                    break;
                case "dia": //           Precios  a  nivel Diario
                    $goGrid->setFieldOpt('txp_Semana', array("header"=>"SEMANA", "hidden"=>1));
                    $goGrid->setFieldOpt('txp_RefOperativa', array("header"=>"ID EMB.", "hidden"=>1));
                    $goGrid->setFieldOpt('txp_NombBuque', array("header"=>"VAPOR", "hidden"=>1, "width"=>30));
                    $goGrid->setFieldOpt('txp_Embarcador', array("header"=>"COD.PRODUCTR", "hidden"=>1));        
            ...
            $goGrid->setFieldOpt('tmp_Flete',    array("header"=>"FLETE",  'width'=>15, "hidden"=>0, "editor"=>$alNumbField));
