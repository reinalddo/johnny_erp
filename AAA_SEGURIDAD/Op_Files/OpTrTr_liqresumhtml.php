<?php
/*
*   Generar una tabla RESUMIDA con los resultados de liquidacion, para hacer seguimiento de facturacion de productores
*   @author fah
*   @package Operaciones
*   @subpackage Transacciones
*   @version    
*/
...
$pSem = fGetParam("pro_Semana", -1);
$pEmb = fGetParam("pEmb", -1);
$pDbg = fGetParam("pAdoDbg", 0);
$db->DB->debug = $pDbg;
$disp='Content-Disposition: inline; filename="RESUMEN_LIQUIDACIONE_SEM_' . $pSem .'"';
header($disp);
//--------------------------------------------------------------------------------------------------------------------
//                          Procesamiento
//--------------------------------------------------------------------------------------------------------------------
...
set_time_limit (0) ;
*/
$gsSqlText = "call spLiqGetResumenFac($pSem)";
...
//	$goGrid = new baaGrid ($gsSqlText, DB_MYSQL);
	$goGrid->setTableAttr('border="1" padding="4" title="RESUMEN DE LIQUIDACIONES" nowrap align="center" cellspacing="1" width="'. $ilTabW . '"');
	$goGrid->showErrors(); // just in case
	$goGrid->setHeaderClass('CobaltColumnTD');
	$goGrid->setRowClass('CobaltDataTD,CobaltAltDataTD');
    $goGrid->setHeadings("CODIGO, NOMBRE, CAJAS,VALOR FRUTA,BONO,
                         NETO A PAGAR,SEMANA,ESTADO,ZONA,COMENTARIO");
