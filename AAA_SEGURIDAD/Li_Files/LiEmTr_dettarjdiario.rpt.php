...
// Grupos de Cabecera
   $slSqlH ="
	...
	`txp_Productor`,
	 ...
	 `txp_CajDescrip`,
	 `txp_CajAbrevia`,
	 IFNULL(txp_PaletInfo,'') as txp_PaletInfo,
	    `txp_Calidad`,
	   `txp_Peso`,
	   `txp_Largo`,
	    `txp_NumDedos`,
	 ...
	 IFNULL(txp_CantRechazada,0) as txp_CantRechazada,
	 IFNULL(txp_CantCaidas,0) as txp_CantCaidas,
	 `txp_DifUnitario`,
	 `txp_PrecUnit`,
	 `txp_LiqNumero`,
	 `txp_IndFlete`,
	 IFNULL(txp_CodOrigen,0) as txp_CodOrigen
	
   FROM v_opetarjexpand WHERE $pQry
   ORDER BY txp_PtoDescripcion, txp_NombBuque, txp_Producto, txp_Fecha, txp_Productor
   " ;				//@TODO Condicion de filtrado
...
$tplFile = 'LiEmTr_dettarjdiario.tpl';
