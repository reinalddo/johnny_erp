...
switch ($ilEstadoProc) {
 ...
 // Automatizacion de Proceso de liq negativas
        ...
        break;
    case 7:
        echo "<br>PASO $ilEstadoProc.-  CONTABILIZACION DE LIQUIDACION:</font></td></tr><tr><td>";
        fSelRubrosFin($db);
        fSelTotalesLiq($db, $ilNumProces, 20, 1);
        $rs =& fSelDatosContIng($db, $ilNumProces, 30, 'LF', 1);
        $agPla['CCI'] = $rec->PRO_CODCUENTA2;
        $agPla['TCI'] = $rec->PRO_AUXILIAR2;
        $agPla['ACI'] = $rec->PRO_AUXILIAR2;
        $agPla['CAP'] = $rec->PRO_CODCUENTA2;  // NZ(fDBValor($db, 'genparametros', 'par_valor1',   "par_clave='CUCAJA'"), false);
        $agPla['TAP'] = $rec->PRO_AUXILIAR2;   // NZ(fDBValor($db, 'genparametros', 'par_valor2+0', "par_clave='CUCAJA'"), 0);
        $agPla['AAP'] = $agPla['TAP'];
...
// Automatizacion de Proceso de liq negativas
// Ya no es necesario porque el LF asienta el valor negativo.
/*
        $ilEstadoProc = 12;
		fLimpiezaPrev($db,$ilEstadoProc, $ilNumProces, $ilNumProduc);
