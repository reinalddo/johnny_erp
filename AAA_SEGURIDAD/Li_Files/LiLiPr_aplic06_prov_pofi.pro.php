...
<?php
/** Proceso Post Liquidacion, Generar comprobante de provision por Precio oficial mas descuentos, omite posicion ordinal = 2
...
//            echo str_pad(' ', 40*6, '&nbsp;') . $slProductor . str_repeat('&nbsp;',$p) . $lastrec['det_tipocomp'] . " - " . $ilNumComp .  "   Cheque: " . ($ilNumChq ) . "<br>";
            echo "&nbsp;&nbsp;&nbsp;" .
                $slProductor . "</td><td>" . $lastrec['det_tipocomp'] . " - " . $ilNumComp . "</td><td> " .
...
//echo "<br>1";
            ...
            $slSql = "REPLACE INTO conenlace(enl_tipo, enl_ID, enl_opCode, enl_secuencia, enl_tipocomp, enl_numcomp, enl_usuario)
                            VALUES ('LQ'," . $lastrec['det_numcomp']. "," . 6 . "," . 0 . ",'" .
...
//@TODO:  HAcer columnas a elegir dinamicas, abasadas en parametro de BD (liqprocesos).
    $slSql ="SELECT var_tipopago as TP,
		    ...
		    SUM(CASE WHEN  var_tipopago = 'TR' AND RUB_POSORDINAL  in (1,3,4,5,10) THEN tmp_valtotal ELSE 0 END)  AS 'TR',
...
switch ($ilEstadoProc) {
    ...
    case 6:
        ...
        echo "<table width='90%' align='center' ><tr><td >NOMBRE</td ><td >COMPROB. NRO.</td ><td >TIPO PAGO</td ><td >VALOR</td ></tr ><tr ><td >";
        ...
        $agPla['TAP'] = "";
        $agPla['AAP'] = "";
        if(!fContabiliza2($db, $rs, "PAGO DE LIQUIDACION-P.O., SEM. " . $ilSemana)) $ilEstadoProc = -1;
