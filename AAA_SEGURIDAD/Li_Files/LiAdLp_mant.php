...
"FROM genparametros";
            ...
            $this->s_TipoItem = new clsControl(ccsListBox, "s_TipoItem", "s_TipoItem", ccsInteger, "", CCGetRequestParam("s_TipoItem", $Method));
...
"FROM genparametros";
            ...
            $this->s_Grupo = new clsControl(ccsListBox, "s_Grupo", "s_Grupo", ccsInteger, "", CCGetRequestParam("s_Grupo", $Method));
...
"FROM genparametros"; 
            ...
            $this->s_Subgrupo = new clsControl(ccsListBox, "s_Subgrupo", "s_Subgrupo", ccsInteger, "", CCGetRequestParam("s_Subgrupo", $Method));
            $this->s_Subgrupo->DSType = dsTable;
            list($this->s_Subgrupo->BoundColumn, $this->s_Subgrupo->TextColumn, $this->s_Subgrupo->DBFormat) = array("par_Secuencia", "par_Descripcion", "");
            $this->s_Subgrupo->ds = new clsDBdatos();
            $this->s_Subgrupo->ds->SQL = "SELECT *  " .
"FROM genparametros"; 
            $this->s_Subgrupo->ds->Parameters["expr999"] = 'ACTSGR';
            $this->s_Subgrupo->ds->wp = new clsSQLParameters();
            $this->s_Subgrupo->ds->wp->AddParameter("1", "expr999", ccsText, "", "", $this->s_Subgrupo->ds->Parameters["expr999"], "", false);
            $this->s_Subgrupo->ds->wp->Criterion[1] = $this->s_Subgrupo->ds->wp->Operation(opEqual, "par_Clave", $this->s_Subgrupo->ds->wp->GetDBValue("1"), $this->s_Subgrupo->ds->ToSQL($this->s_Subgrupo->ds->wp->GetDBValue("1"), ccsText),false);
            $this->s_Subgrupo->ds->Where = $this->s_Subgrupo->ds->wp->Criterion[1];

            //$this->s_Grupo->HTML = true;
            $this->s_Descripcion = new clsControl(ccsTextBox, "s_Descripcion", "Descripci&oacute;n", ccsText, "", CCGetRequestParam("s_Descripcion", $Method));
...
//Validate Method @232-715FB007
    function Validate()
    {
        ...
        $Validation = ($this->s_Subgrupo->Validate() && $Validation);
...
//CheckErrors Method @232-32C670AE
    function CheckErrors()
    {
        ...
        $errors = ($errors || $this->s_Subgrupo->Errors->Count());
...
//Show Method @232-DFE33919
    function Show()
    {
        ...
        $this->s_Subgrupo->Prepare();
        $this->s_Descripcion->Prepare();
        ...
        if($this->FormSubmitted || $this->CheckErrors()) {
            ...
            $Error .= $this->s_Subgrupo->Errors->ToString();
            $Error .= $this->s_Descripcion->Errors->ToString();
...
//Class_Initialize Event @2-D8F59DC6
    function clsEditableGridlispredetal()
    {
        ...
        $this->act_CodAnterior = new clsControl(ccsTextBox, "act_CodAnterior", "Cod. Alterno", ccsText, "");
...
//Initialize Method @2-396EE9CE
    function Initialize()
    {
        ...
        $this->ds->Parameters["urls_Subgrupo"] = CCGetFromGet("s_Subgrupo", "");
        $this->ds->Parameters["urls_Descripcion"] = CCGetFromGet("s_Descripcion", "");
    }
...
//Prepare Method @2-514F7AB5
    function Prepare()
    {
        ...
        $this->wp->AddParameter("5", "urls_Subgrupo", ccsInteger, "", "", $this->Parameters["urls_Subgrupo"], "", false);
        $this->wp->AddParameter("6", "urls_Descripcion", ccsText, "", "", $this->Parameters["urls_Descripcion"], "", false);       
        $this->wp->AddParameter("7", "urls_Descripcion", ccsText, "", "", $this->Parameters["urls_Descripcion"], "", false);       
        $this->wp->AddParameter("8", "urls_Descripcion", ccsText, "", "", $this->Parameters["urls_Descripcion"], "", false);       
        ...
        $this->wp->Criterion[5] = $this->wp->Operation(opEqual, "act_Subgrupo", $this->wp->GetDBValue("5"), $this->ToSQL($this->wp->GetDBValue("5"), ccsInteger),false);
        $this->wp->Criterion[6] = $this->wp->Operation(opContains, "act_Descripcion", $this->wp->GetDBValue("6"), $this->ToSQL($this->wp->GetDBValue("6"), ccsText),false);
        $this->wp->Criterion[7] = $this->wp->Operation(opContains, "act_CodAuxiliar", $this->wp->GetDBValue("7"), $this->ToSQL($this->wp->GetDBValue("7"), ccsText),false);      
        $this->wp->Criterion[8] = $this->wp->Operation(opContains, "act_CodAnterior", $this->wp->GetDBValue("8"), $this->ToSQL($this->wp->GetDBValue("8"), ccsText),false);
        $this->Where = $this->wp->opAND(false, $this->wp->opAND(false, $this->wp->opAND(false, $this->wp->opAND(false, $this->wp->opAND(false, /*$this->wp->opOR(true, $this->wp->Criterion[1], $this->wp->Criterion[2]),*/ $this->wp->Criterion[3]), $this->wp->Criterion[4]), $this->wp->Criterion[5]),$this->wp->opOR(true, $this->wp->opOR(false, $this->wp->Criterion[6], $this->wp->Criterion[7]), $this->wp->Criterion[8]))); 
...
//Open Method @2-93BBCBD7
    function Open()
    {
        ...
        "invprecios.pre_CodItem = conactivos.act_CodAuxiliar AND (pre_LisPrecios = '{$this->Parameters["urls_LisPrecios"]}' OR pre_LisPrecios IS NULL)) LEFT JOIN genunmedida ON " .
        ...
        $this->SQL = "SELECT act_CodAnterior, act_CodAuxiliar, act_Descripcion, act_Descripcion1, invprecios.*, uni_Descripcion, uni_Abreviatura, cat_Categoria, cat_Activo  " .
        "FROM ((conactivos LEFT JOIN invprecios ON " .
        "invprecios.pre_CodItem = conactivos.act_CodAuxiliar AND (pre_LisPrecios = '{$this->Parameters["urls_LisPrecios"]}' OR pre_LisPrecios IS NULL)) LEFT JOIN genunmedida ON " .
        "conactivos.act_UniMedida = genunmedida.uni_CodUnidad) INNER JOIN concategorias ON " .
        "conactivos.act_CodAuxiliar = concategorias.cat_CodAuxiliar";        
...
//Update Method @2-1951984D
    function Update()
    {
        ...
        $this->SQL = "INSERT INTO invprecios_historico(pre_CodItem, pre_LisPrecios, pre_PreUnitario, pre_fecha)  " .
        "VALUES(" . $this->SQLValue($this->cp["pre_CodItem"]->GetDBValue(), ccsInteger) . ", " . $this->SQLValue($this->cp["pre_LisPrecios"]->GetDBValue(), ccsInteger) . ", " . $this->SQLValue($this->cp["pre_PreUnitario"]->GetDBValue(), ccsText) . ", now() ) ";
