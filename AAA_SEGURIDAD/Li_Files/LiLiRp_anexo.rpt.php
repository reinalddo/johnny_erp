...
*   @param      string		pQryLiq  Condicion de busqueda
*   @output     contenido pdf del reporte.
*   @rev	fah Jun/12/07	Correccion para MySQl 5:  Longitud de campos en tablas temporales
*   @rev	fah Ene/16/08	Parametrizacion de la semana inicial para liquidaciones (800)
*   @rev	fah Jul/22/09	Omitir detalles con cantidad cero (aunque tengas valor numerico)
*   @rev	jvl Oct/30/14	Cambio de los ordenes de los campos solicitado x Banadecsa
*/
...
function &fDefineQry(&$db, $pQry=false){
    ...
    $ilSemInic = NZ(fDBValor($db, 'genparametros', 'par_valor2', "par_clave='LIINIC'"),0);//               Trae el Parametro de periodo minimo de liquidacion
    ...
    $alSql[] =  "CREATE INDEX i_tmppr ON tmp_productor(tmp_codProductor)";
    ...
    $alSql[] =  "CREATE  /*TEMPORARY*/ TABLE tmp_data
                SELECT tmp_nombre AS NOM, com_CodReceptor AS PRO, pro_semana AS SEM,
                    act_grupo AS GRU,
                    concat(det_CodItem, '  ', left(act_descripcion,28), ' ',left(act_descripcion1,5)) AS DIT,
                    ...
                    000000000.0000 AS 'C08',
                    000000000.0000 AS 'V08',
              ...
              WHERE pro_codProceso = 5   AND com_RefOperat  >= " . $ilSemInic . " AND com_RefOperat  < pro_Semana AND det_cantEquivale  <> 0
              GROUP BY 1,2,3,4,5  ORDER BY 1,2,3 ";
// @TODO: parametrizar la semana de inicio del liquidaciones (800)
    ...
    $alSql[] ="INSERT INTO tmp_data
                SELECT tmp_nombre AS NOM, com_CodReceptor AS PRO, pro_semana AS SEM,
                    ...
                    SUM(CASE WHEN  pro_orden = 8 AND act_grupo = '210' THEN (det_CantEquivale * pro_Signo) ELSE 0 END)  AS 'C08',
                    SUM(CASE WHEN  pro_orden = 8 AND act_grupo = '210' THEN (det_ValTotal * pro_Signo)     ELSE 0 END)  AS 'V08',
                    SUM(det_CantEquivale * pro_Signo)   AS 'CXX',
                    SUM(det_ValTotal * pro_Signo)       AS 'VXX'
              FROM (((((invprocesos  join concomprobantes ON com_TipoComp = cla_TipoTransacc)
                     JOIN tmp_productor ON tmp_codProductor = com_CodReceptor)
    ...
    $alSql[] ="SELECT NOM, PRO, GRU, DIT, SEM,
                    ...
                    SUM(IF(C08 > 0, C08,0)) AS CPA,
                    SUM(IF(C07 < 0, V07,0)) AS VCO,
                    SUM(IF(C08 > 0, V08,0)) AS VPA
            FROM tmp_data
            GROUP BY 1,2,3,4,5
            HAVING  (SUM(C01) <> 0 AND
                    SUM(V01) <> 0 ) OR
                    SUM(C02) <> 0 OR
                    SUM(V02) <> 0 OR
                    SUM(C03) <> 0 OR
...
function before_group_PRO (&$rpt, &$group) {
    ...
    $sltxt = "SELECT com_tipocomp, com_numcomp
                    ...
                    WHERE   com_tipocomp in (SELECT cla_TipoTransacc FROM invprocesos WHERE pro_codProceso = 5 AND pro_signo <> 0  ) and com_refOperat = " . $group->lastRec['SEM'] . " and
    ...
    if ($rs) {
        ...
        $rpt->pdf->y -= 8;
// JVL x Reporte a Productor        $rpt->pdf->y=$rpt->putTextAndLabel(10, $rpt->pdf->y, $sltxt, "DOCS: " ,  500,30) + 6;
...
//$rep = new ezPdfReport($rs, "A4", "portrait", "Courier", 8);
//$rep = new ezPdfReport($rs, "A4", "portrait", "Times", 8); Jvl no
//rep = new ezPdfReport($rs, array(29,15.8), "landscape", "Helvetica", 6); Jvl casi
$rep = new ezPdfReport($rs, array(21.7,14),"portrait", "Courier", 8);
$rep->subTitle="LIQUIDACION DE MATERIALES ";
//$rep->subTitle="-";
//$rep->condition=fGetParam('pCond', '');
$rep->titleOpts=array('T'=>array('font'=>$rep->font, 'fontSize'=>$ilFontSize+2,  'justification'=>'center', 'leading'=>$ilFontSize),
                      'S'=>array('font'=>$rep->font, 'fontSize'=>$ilFontSize ,   'justification'=>'center', 'leading'=>$ilFontSize+7),
...
$rep->margins = array(0.5,0.5,0.5,0.5);
echo "TIPO: " . fGetParam('pTipo', 'XX');
switch (fGetParam('pTipo', 'XX')){
    case 'H': //      Formato incluye 'historia' del productos, saldo anterior y final
        $rep->colHead = array(
            'DIT'=>'   ITEM ',
//            'C01'=>'SALDO ANTER.',
//                    'V01'=>'VAL ANT',
...
//                    'V06'=>'VAL CAI',
            'C07'=>'COBRO/ PAGO',
            'PUN'=>'PREC. UNIT',
            'V07'=>'TOTAL COBRO/PAGO',
//                    'CCO'=>'CANT. COBRADA',
//            'CPA'=>'CANT. PAGADA',         //JVL
//            'VPA'=>'VALOR. PAGADO',        //JVL
//                    'VCO'=>'VALOR. COBRADO',
//            'CXX'=>'SALDO FINAL'
//                    'VXX'=>'SAL VAL'
            );
            break;
    case "I":           //      Solo INVENTARIO
        $rep->colHead = array(
            'DIT'=>'   ITEM ',
            'C02'=>'EGRESO BODEGA',
            'C03'=>'DEVOLU CIONES',
...
$rep->setDefaultColPro('format', "10:4:.:"); //     Default Format for columns  total long:decimal places:thousands sep:decimal sep:
...
$rep->columns['C04']->format="10:2:.:";
$rep->columns['C07']->format="10:2:.:";
$rep->columns['V07']->format="10:2:.:";
$rep->columns['CCO']->format="10:2:.:";
$rep->columns['CPA']->format="10:2:.:";
$rep->columns['VCO']->format="10:2:.:";
$rep->columns['VPA']->format="10:2:.:";
$rep->columns['VXX']->format="10:2:.:";
switch (fGetParam('pTipo', 'XX')){
    case 'H':  //      Formato que incluye 'historia' del productos, saldo anterior y final
        $rep->columns['C01']->visible=true;
        $rep->columns['C07']->visible=true;
        $rep->columns['V07']->visible=true;
        $rep->columns['CCO']->visible=false;
        $rep->columns['CPA']->visible=false;
        $rep->columns['VCO']->visible=false;
        $rep->columns['VPA']->visible=false;
        echo "HISTORICO";
    break;
    case"I":
        $rep->columns['C01']->visible=false;
...
$rep->colOpt['DIT']['width']=160;
$rep->colOpt['PUN']['width']=45;
...
$rep->groups['general']->fontSize=8;                 // set font size for this group (has no efect yet, because ezTable sets fontsize for entire table)
    ...
    $rep->addResumeLine('general','-', 'SUMA GENERAL', 0);         // Add a resume line for group 'general' using sums in all columns bt default
        $rep->groups['general']->linesBefore=0;      // JVL Antes estaba 2
        /*
      ...
      */
// Jvl  31 Oct 2014     $rep->setAggregate('general',0, 'V07','S');
// Jvl  31 Oct 2014      $rep->setAggregate('general',0, 'VPA','S');
// Jvl  31 Oct 2014      $rep->setAggregate('general',0, 'VCO','S');
...
$gfValor=0;
$rep->run();
$rep->view($rep->title, $rep->saveFile("LQF_ANEXO_"));
?>
