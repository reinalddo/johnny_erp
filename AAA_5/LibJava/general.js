...
*   @created    12/Abr/03
*   @rev				fah				10/12/07		Hacer que tecla hacia abajo lleve al primer campo de la fila. Para ello debe venir como
...
function fAbrirObjWin(pUrl, oWin, pWin, pOpc, pAnch, pLarg)
{
    ...
    sOpcDef = 'status=yes,titlebar=no,toolbar=no,menubar=no,scrollbars=yes, alwaysRaised=1,dependant'                            // Opciones Default
...
*   @param  object  f campo destino en la fila
*   @return void
*/
function fKeyUp(e, f)
{
// alert(e);
	...
	ilFila = fEstaFila(tg);
	if (f)	slcampo = f;				// #fah-10/12/07
	else 	slcampo = tg.name.substr(0, tg.name.lastIndexOf("_"))
	...
	//slDestino=slcampo + "_" + ilDesplaza;	  // #fah-12/10/07
...
//alert (e.keyCode)
	...
	if(!f.length) slcampo=f                   // if default field not defined, focus to same field in next line ( if empty)
	if (kc != 38 && kc != 40) return;
	with (tg.form) {
		ilFilas = fNumFilas(tg);
