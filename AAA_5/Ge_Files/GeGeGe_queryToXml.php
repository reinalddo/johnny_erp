...
*   @rev	fah	22/08/08	Soportar la definicion del sqlid via variable global $gsSql y no solamente via GET/POST
*/
function &fAplicaCondicion(&$db){
	global $gsSqlText, $gsSql, $appStatus, $giTotalRecs, $giPageRecs;
	$id=fGetParam('id', false);
	if ($id) {
		$alQry=  $_SESSION[$id];
	}
	else $alQry=  (fGetParam('pQry', false)) ? fGetParam('pQry', false) : $gsSqlText ;
	if (!$id &&  $alQry === false ) {
...
if ($rsDb){
	...
	$slMensaje = 'ERROR EN LA CONSULTA ' . $gsSql;
