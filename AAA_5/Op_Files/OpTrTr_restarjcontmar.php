...
//$style_pr = ".nosaltar {page-break-inside:void}";
...
$pNavi   = fGetParam('pNavi', false);
$pFruta   = fGetParam('pFruta', false);
...
if ($pNavi == -9999 || $pNavi == ' TODOS') $pNavi = false;
if ($pFruta == -9999 || $pFruta == ' TODOS') $pFruta = false;
...
$gsCond .= (strlen($gsCond )>0 ? " AND " : "") . (($pNavi != false )? " cnt_naviera  = '" . $pNavi ."'" :" true ");
$gsCond .= (strlen($gsCond )>0 ? " AND " : "") . (($pFruta != false )? " txp_CodProducto  = '" . $pFruta ."'" :" true ");
...
if (fGetParam("pRpt", "C") != "C")	
   $tplFile = 'OpTrTr_restarjcontma2.tpl';  // Reporte con nombres
else
   $tplFile = 'OpTrTr_restarjcontmar.tpl';  	// Reporte con Codigos unicamente
...
$Tpl->assign("style_sc", "body {font-size:x-small}");
