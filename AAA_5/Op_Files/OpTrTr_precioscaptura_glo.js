...
*/  
    ...
    var store = new storeX({
        url: 'OpTrTr_precioscaptura_glo.php',
    ...
    grd2 = new grd2class({
        ...
        ,bbar: new Ext.PagingToolbar({
            ...
            , {
                ...
                ,handler: fRecargaDetalles.createDelegate(this, ["mar",rowNum] , true)
            }, {
                //pressed: false
                //,enableToggle:true
                text: 'Dias'
                ...
                ,cls: 'x-btn-text-icon details' 
                ,handler: fRecargaDetalles.createDelegate(this, ["dia",rowNum], true)
            }, {
                //pressed: false
                //,enableToggle:true
                text: 'Tarjas'
                ,tootTip:"Cantidades embarcadas por Tarja"
                ,cls: 'x-btn-text-icon details' 
                ,handler: fRecargaDetalles.createDelegate(this, ["tar",rowNum], true)
            },'-',{
                //pressed: false
                //,enableToggle:true
                text: 'Div.Tarja'
                ,tootTip:"Divide una detalle de tarja en dos "
                ,cls: 'x-btn-text-icon details' 
                ,handler: fDivideTarja.createDelegate(this, ["tar",rowNum], true)
            },{
                //pressed: false
                //,enableToggle:true
                text: 'CERRAR'
                ,cls: 'x-btn-text-icon details' 
                ,handler: fCerrarGrid.createDelegate(this, [gridId], true)
            }]
        })
        ,view: new Ext.grid.GroupingView({
            forceFit:false
...
function fInitExpander(){
    ...
    function expandedRow(obj, record, body, rowIndex){
        ...
        id2 = "g-" + rowIndex
        this.id=id;
        fVerDetalles(id2, "tar", id, rowIndex)
    }	 
}
function fCampos(){
    return [
...
function fColumns2(){
	...
	Ext.grid.GroupSummary.Calculations['totalCost'] = function(v, record, field){
        ...
        ,{id:"tmp_Cantidad",
            ...
            sortable: true,
            summaryType:'sum',
            style:"background-color: gray; color:blue",
            renderer: fRendQuantity,
            summaryRenderer: fRendQuantity,                
            dataIndex: 'tmp_Cantidad'}
	    ,{id:"tmp_PrecUnit",
...
function fInitGridPrec(pPlugIns){
    ...
    strProd = new xg.strPrecios({
	    id:"strProd"
	    ,url: 'OpTrTr_precioscaptura_glo.php?op=pro'
...
function fDivideTarja(pBotn, pNN, pTipo, pRowNum){
        var olRec=Ext.getCmp("gridPrecProd").store.getAt(pRowNum).data
        var olRec = grd2.selModel.selection.record;
        var olCant1 = new Ext.form.NumberField ({ 
            //xtype: "numberfield"
            id: "ilCant1"
            ,value: 0
            ,fieldLabel:"Cantidad 1"
            ,tabIndex: 1
            ,allowBlank: false
        })
        var olPrec1 = new Ext.form.NumberField({//xtype: "numberfield"
            id: "flVal1"
            ,value: 0
            ,fieldLabel:"Prec.Unitario 1"
            ,tabIndex: 2
            ,allowBlank: false
        });
        olCant1.setValue(olRec.data.tmp_Cantidad);
        olPrec1.setValue(olRec.data.tmp_PrecUnit);
        var olWinDiv= new Ext.Window({
            title: 'DIVISION DE TARJA' + " " + olRec.data.txp_NumTarja + "-" + olRec.data.txp_Secuencia +
                   ' con ' + olRec.data.tmp_Cantidad  + " cjs"
            ,id:'winDivide'
            ,items:[{
                xtype:'form'
                ...
                ,items:[{       // Columna de cantidades
                    columnWidth:0.5
                    ,defaults: {labelHide:false, labelWidth:60, width:40}
                    ,layout:'form'
                    ,items: [olCant1,{
                        xtype: "numberfield"
                        ,id: "ilCant2"
                        ,value:0
                        ,fieldLabel:"Cantidad 2"
                        ,tabIndex: 3
                    }]
                    },{     // Columna deprecios
                    columnWidth:0.5
                    ,layout:'form'
                    ,defaults: {labelWidth:60, width:40}                    
                    ,items:[olPrec1,{
                        xtype: "numberfield"
                        ,id: "flVal2"
                        ,value:0
                        ,fieldLabel:"Prec.Unitario 2"
                        ,tabIndex: 4
                    }]
                }]
                ,buttons: [{xtype: "button"
                        ,text:"Aplicar" 
                        ,handler: fDividir.createDelegate(olWinDiv, [olRec])
                }]
            }]
        /*,beforeShow: function(pRec){
            Ext.getCmp("ilCant1").setValue = pRec.tmp_Cantidad;
            debugger;
            return true;
            //Ext.getCmp("ilVal1").value = olRec.tmp_PrecUni;
        }.createDelegate(this, [olRec])
        */
        })/*.show();*/
	/*olWinDiv.on("show", function(){
	    return true
	})*/
	olWinDiv.show(); 
    }
function fDividir(pTar){
    var ilC1 = Ext.getCmp("ilCant1").getValue() ;
    var ilC2 = Ext.getCmp("ilCant2").getValue()  ;
    var ilP1 = Ext.getCmp("flVal1").getValue()  ;
    var ilP2 = Ext.getCmp("flVal2").getValue()  ;
    if ((ilC1+ilC2) == pTar.data.tmp_Cantidad) {	// Aplicar solo si esta cuadrada la cantidad original con la suma de divisiones
	olDat = Ext.Ajax.request({
	    url: 'LiEmTr_tarjdivide.php'
	    ,callback: function(pOpt, pStat, pResp){
    		if (true == pStat){
    		}
	    },success: function(pRes,pPar){
    		Ext.getCmp("winDivide").close()  ;
            grd2.store.reload()
	    },failure: function(pObj, pRes){
    		Ext.Msg.alert('Status', 'No se pudo aplicar la division');
	    },params: {tad_NumTarja: pTar.data.txp_NumTarja
            ,tad_Secuencia: pTar.data.txp_Secuencia
            ,txt_Cant1: ilC1
            ,txt_Cant2: ilC2
            ,txt_Prec1: ilP1
            ,txt_Prec2: ilP2
	...
	Ext.Msg.alert('Status', 'Debe cuadrar las cantidades con la tarja original');
    }
}
