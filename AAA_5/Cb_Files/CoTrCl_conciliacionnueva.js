...
Ext.onReady(function(){
    ...
    */
    ...
    name="Car";
    app.cl.gridNuevo = new Ext.ux.AutoGridPanel({
        title:''
        ,plugins: [filtersDet]
        //,id:'gridConciliacion'
        ,height: 200
        ...
        ,bbar: new Ext.PagingToolbar({
		   ...
		   ,handler: fVerGridDetalle
		    ...
		    //,toggleHandler: toggleDetails
		    ,handler: function(){
			app.cl.gridNuevo.getSelectionModel().clearSelections();
		}}*/
        ]})
    ...
    //if(!Ext.getCmp("gridConciliacion")){
        ...
        olPanel.add(app.cl.gridNuevo);
    //}
    
    
    app.cl.gridNuevo.store.load({params: {meta: true, start:0, limit:25, sort:'com_FecContab', dir:'DESC'}});
    Ext.getCmp(gsObj).doLayout();
    
    /*app.cl.gridNuevo.getSelectionModel().on('rowselect', function(sm, rowIdx, r) {
                //debugger;
                //var olRec=app.cl.gridNuevo.getSelectedRecord().node;
                var ilTipo = r.data.com_TipoComp;
                var ilNum = r.data.com_NumComp;
                var sm2 = app.cl.gridNuevo.getSelectionModel().getSelected();
    ...
    app.cl.gridNuevo.on('rowclick', function(sm, rowIdx, e) {
                //debugger;
                //var olRec=app.cl.gridNuevo.getSelectedRecord().node;
                var r = app.cl.gridNuevo.getStore().getAt(rowIdx);
                var ilTipo = r.data.com_TipoComp;
                var ilNum = r.data.com_NumComp;
                var sm2 = app.cl.gridNuevo.getSelectionModel().getSelected();
                if ('0' == r.data.det_EstLibros ){
    ...
    /*app.cl.gridNuevo.getSelectionModel().on('rowdeselect', function(sm, rowIdx, r) {
