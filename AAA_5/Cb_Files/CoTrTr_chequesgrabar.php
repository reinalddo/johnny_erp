...
include_once("CoTrTr_archivobanco.php");//../work_w_abr_01/
...
function fGrabaConfirmacion(){
    ...
    //Actualiza campos para confirmar
    /*$limite = $agPar['tot']."-";
    $ind = 0;
    while ($ind < $limite){
        $sSql = "update concheques_cab
                    set Confirmado=1, fecConfir=CURRENT_TIMESTAMP, usuConfir='".$_SESSION['g_user']."'
                    where IdBatch=".$agPar['batch'][$ind]."";
    ...
    /*//Inserta Cabecera
    $sSql = "insert concheques_cab (tipo,fecha,Observacion,Origen,Destino,usuario,Confirmado,fecConfir,usuConfir) values (";
    $sSql .= "2,CURRENT_TIMESTAMP,'".$agPar['obs']."','".$agPar['origen']."','".$agPar['destino']."','".$_SESSION['g_user']."'";
    $sSql .= ",1,CURRENT_TIMESTAMP,'".$_SESSION['g_user']."'";
    $sSql .= ")";
    //echo $sSql;
