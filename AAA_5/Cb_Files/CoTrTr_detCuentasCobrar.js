...
*  
*  proceso de Autogrid
*   consulta de cuentas por cobrar en un rango de fechas determinado
*   parametros:
*       pAuxil  = codigo de auxiliar de persona
...
Ext.onReady(function(){
    ...
    var gridViewCtaCob = new Ext.grid.GridView({ 
       //forceFit: true, 
    ...
    Ext.namespace("app", "app.CtaCo");		     // Iniciar namespace cart
    ...
    var storeCtaCo = new storeX({
	    proxy: new Ext.data.HttpProxy({
            method:"GET",
            url: (!sgLoadUrl)? 'CoTrTr_detCuentasCobrar.php' : sgLoadUrl  
        }),
    ...
    */
	...
	{type: 'string',  dataIndex: 'txt_Beneficiario'},
    ...
    name="Cnt";
    gridCtaCo = new Ext.ux.AutoGridPanel({
        id:'cajaCtaCobrar'
        ,title:''
	,plugins: [filters]
        ,height: 300
        ,width: 800
        ,cnfSelMode: 'rsms'//'csm'  //CnfSelMode: propiedad para definir el tipo de selección de datos==> csm(CheckSelectionMode), csm(CellSelectionMode), rsms(RowSelectionMode Single), rsm(RowSelectionMode Multiple)
        ,loadMask: true
        ,stripeRows :true
        ,autoSave: true
        ...
        ,bbar: new Ext.PagingToolbar({
            ...
            ,{
                ...
                handler: function(){basic_printGrid2(gridCtaCo, 'Cuentas por cobrar');}
            }]
        })
    ...
    Ext.getCmp(gsObj).add(gridCtaCo);
   
    //Ext.getCmp('paneles').doLayout();
    Ext.getCmp(gsObj).doLayout();
    this.gridCtaCo.store.load({params: {meta: true, start:0, limit:100, sort:'com_FecContab', dir:'DESC'}});// @TODO: load method must be applied over a dinamic referenced object, not 'this.grid1' referenced
     
    gridCtaCo.getView().refresh();
