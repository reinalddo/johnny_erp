...
function fgetParamatro(){
    global $modulo;
    $modulo = fGetParam("pmodulo","BiTrTr"); //Si no se especifica otro modulo el default va a ser el de transacciones-bitacora
    //echo($modulo);
    //print_r($_SESSION);
    ...
    //echo "<br/>".$_SESSION["atr"][$modulo][$agPar['pKey']]."---".$agPar['pKey'];
    
    if ("user" != $agPar['pKey']){
        $bool = $agPar['pBool'];
        if (1 == $bool){
            $valor = $_SESSION["atr"][$modulo][$agPar['pKey']];
            if (1 == $valor)
                $valor = true;
            else
                $valor = false;
            //$valor = ((is_null($_SESSION["atr"][$modulo][$agPar['pKey']]) || 0 == $_SESSION["atr"][$modulo][$agPar['pKey']]) ? false : $_SESSION["atr"][$modulo][$agPar['pKey']]);
        }else{
            $valor = (is_null($_SESSION["atr"][$modulo][$agPar['pKey']]) ? 0 : $_SESSION["atr"][$modulo][$agPar['pKey']]);
        }
