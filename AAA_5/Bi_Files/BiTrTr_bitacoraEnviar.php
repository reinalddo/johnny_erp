...
if (fGetParam("init", false) != false) {
	...
	$_SESSION[$gsSesVar . "_count"]= "select count(*) as totalRecs
			    ...
			    JOIN bitacoradetalle det  ON det.bit_tipoDoc = cab.bit_tipoDoc 
						    ...
						    AND (	/*Todos los documentos que han sido recibidos*/
								(det.bit_secuencia = (SELECT MAX(d2.bit_secuencia) FROM bitacoradetalle d2
											WHERE d2.bit_tipoDoc = det.bit_tipoDoc
											...
											AND det.bit_estado = '2')
									     
								) 
								OR /*Los documentos que hayan sido recibidos por el usuario actual pero que han sido enviados a otro usuario*/
								(det.bit_secuencia = (SELECT MAX(d2.bit_secuencia)-1 FROM bitacoradetalle d2  
											WHERE d2.bit_tipoDoc = det.bit_tipoDoc
											AND d2.bit_secDoc = det.bit_secDoc
											AND d2.bit_emiDoc = det.bit_emiDoc
											AND d2.bit_numDoc = det.bit_numDoc
	...
	$_SESSION[$gsSesVar]= "select   cab.bit_tipoDoc as bit_tipoDoc
				    ...
				    ,CASE 
					     WHEN det.bit_usuariodestino IS NULL THEN NULL
					     ELSE bit_observacionenvio END AS bit_observacionenvio
				    ,CASE 
					     WHEN det.bit_usuariodestino IS NULL THEN NULL
					     ELSE DATE_FORMAT(det.bit_fechaenvio,'%Y-%m-%d %h:%i:%s') END AS bit_fechaenvio
				    ,CASE 
					     WHEN det.bit_usuariodestino IS NULL THEN NULL
					     ELSE DATE_FORMAT(det.bit_fechaenvio,'%Y-%m-%d') END AS FechaEnv
				    ,CASE 
					     WHEN det.bit_usuariodestino IS NULL THEN NULL
					     ELSE DATE_FORMAT(det.bit_fechaenvio,'%h:%i:%s %p') END AS HoraEnv
			    ...
			    LEFT JOIN 09_base.genparametros est ON est.par_Clave = 'BITEST' AND est.par_Secuencia = det.bit_estado
			    WHERE cab.bit_cerrado = 0
			    and cab.bit_anulado = 0 and DATE_FORMAT(cab.bit_fechaReg,'%Y-%m-%d') between '".fgetparam('fecha_desde')."' and '".fgetparam('fecha_hasta')."'
			    and {filter}
			    order by {sort} {dir} 
			    LIMIT {start}, {limit} ";
	...
	$_SESSION[$gsSesVar . "_count"]= "select count(*) as totalRecs
			    ...
			    LEFT JOIN 09_base.genparametros mov ON mov.par_Clave = 'BITMOV' AND mov.par_Secuencia = det.bit_movimiento
			    LEFT JOIN 09_base.genparametros est ON est.par_Clave = 'BITEST' AND est.par_Secuencia = det.bit_estado
			    WHERE cab.bit_usuarioActual = '".$_SESSION['g_user']."'
    ...
    class clsQueryGrid extends clsQueryToJson {
	    ...
	    $goGrid->setFieldOpt('AuxNombre',array("header"=>"Proveedor", "hidden"=>0, "width"=>30));
	    ...
	    $goGrid->setFieldOpt('movimiento',array("header"=>"Movimiento:", "hidden"=>0, "width"=>15));
	    $goGrid->setFieldOpt('bit_observacion',array("header"=>"Observacion:", "hidden"=>0, "width"=>20));
	    $goGrid->setFieldOpt('FechaRecib',array("header"=>"Fecha Recibido", "hidden"=>0, "width"=>15));
	    $goGrid->setFieldOpt('HoraRecib',array("header"=>"Hora Recibido", "hidden"=>0, "width"=>15));
	    $goGrid->setFieldOpt('bit_usuariodestino',array("header"=>"Destino:", "hidden"=>0, "width"=>10));
	    $goGrid->setFieldOpt('bit_observacionenvio',array("header"=>"Detalle:", "hidden"=>0, "width"=>20));
	    ...
	    $goGrid->setFieldOpt('bit_registro',array("header"=>"Registro", "hidden"=>1, "width"=>15));
