<?php
   ...
   /**
	...
	function fAscendentes($ilBase, $ilcta, $dbg ) {
    	...
    	if ($ilcta > 1 ) {
            ++$ignivel;
	        $query = "SELECT cue_id, cue_padre, cue_codcuenta FROM concuentas WHERE cue_ID = " . $ilcta;
 ...
 */
 function fGeneraEstructura($ilCuenta = -1, $p_mode = "r", $dbg = false) {
	    ...
	    $query = "SELECT cue_ID, cue_padre, cue_codcuenta, cue_posicion FROM concuentas ";
	    ...
	    while ($line = mysql_fetch_array($rst)) {
	      ...
	      switch ($p_mode ) {
	        case "R":       // Reconstruye la estructura de todas las cuentas
	            $query = "DELETE FROM conredcuentas WHERE red_cueid = " .   $igCuenta;
