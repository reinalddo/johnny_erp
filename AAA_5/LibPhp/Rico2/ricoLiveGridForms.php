...
class TableEditClass {
  ...
  function AddEntryField($ColumnName, $Heading, $EntryTypeCode, $DefaultValue) {
    if (!in_array($EntryTypeCode, array("S", "N", "R", "H", "D", "DT", "I", "B", "T", "TA", "SL", "RL", "CL"))) {
  ...
  function SortAsc() {
    if (array_key_exists("Formula",$this->CurrentField))
      $this->AddSort($this->CurrentField["Formula"],"ASC");
    elseif (array_key_exists("ColName",$this->CurrentField))
      $this->AddSort($this->CurrentField["ColName"],"ASC");
  }

  function SortDesc() {
    if (array_key_exists("Formula",$this->CurrentField))
      $this->AddSort($this->CurrentField["Formula"],"DESC");
    elseif (array_key_exists("ColName",$this->CurrentField))
      $this->AddSort($this->CurrentField["ColName"],"DESC");
  }
      ...
      for ($i=0; $i<count($this->Fields); $i++) {
        if (array_key_exists("EntryType",$this->Fields[$i])) {
          if ($this->Fields[$i]["EntryType"] != "H") {
            $this->Fields[$i]["ColData"]=$a[$i];  // don't overwrite hidden field data
          }
        }
      }
    }
  ...
  function FormatFormValue($idx,$fldname) {
      ...
      elseif ($this->Fields[$i]["TableIdx"] == $this->MainTbl) {
        ...
        if (substr($this->Fields[$i]["EntryType"],1) == "L" && array_key_exists("SelectSql",$this->Fields[$i])) {
          ...
          if (count($oParseLookup->arSelList) == 2) {
            ...
            if (preg_match("/^\\w+$/",$descField)) $descField="t".$i.".".$descField;
            $oParseMain->AddJoin("left join ".$oParseLookup->FromClause." t".($i)." on t.".$this->Fields[$i]["ColName"]."=t".($i).".".$codeField);
            $oParseMain->arSelList[$i]=$this->objDB->concat(array($descField, "'<span class=\"ricoLookup\">'", $this->objDB->Convert2Char("t.".$this->Fields[$i]["ColName"]), "'</span>'"), false) . " as rico_col".$i;
          }
          else {
  ...
  function InitScript() {
    ...
    $s.="\n".$this->menuVar."=new Rico.GridMenu();";
    $s.="\nif(typeof ".$this->gridID."_GridInit=='function') ".$this->gridID."_GridInit();";
    $s.="\n".$this->gridVar."=new Rico.LiveGrid('".$this->gridID."',".$this->menuVar.",".$this->bufferVar.",".$this->optionsVar.");";
