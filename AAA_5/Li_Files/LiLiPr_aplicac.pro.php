...
function fAplicaProc(&$db)   {
    ...
    $slSql= "SELECT  distinct tad_NumTarja, tad_Secuencia, tac_Embarcador, IFNULL(tad_LiqNumero,0) as tad_LiqNumero,
              ...
              FROM liqembarques  
			  		JOIN liqtarjacabec ON tac_RefOperativa = emb_RefOperativa
...
//                                  Tabla temp con resumen de productores y Nro de liquidacion
    if(!$db->Execute("INSERT INTO tmp_product ".
	                       "SELECT DISTINCT tac_Embarcador as tmp_CodProductor,
			                         tad_LiqNumero as tmp_LiqNumero, ifnull(max(com_refOperat),0)
	                        FROM tmp_liqdetal
		                    LEFT JOIN concomprobantes on com_tipocomp = 'LI' and com_codreceptor = tac_embarcador
	                        GROUP BY 1,2")) fErrorPage('','NO SE PUDO GENERAR LISTA DE PRODUCTORES', true) ; //   fah Feb/07/07
}
...
/**
*   Procesa las Tarjas que corresponden al nuemro de proceso. Graba la explosi� de insumos
* 	@access public
