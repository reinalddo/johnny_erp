...
/*
    ...
    while ($rec = $rs->FetchNextObject(true)) {
        $gsTitGeneral .= $rec->TMP_BUQUE;
    }
*/
/*   $slTablas = "liqprocesos
                	join liqtarjadetal on tad_liqproceso = pro_ID
                	join liqtarjacabec on tad_numtarja = tar_numtarja
                	join liqembarques  on tac_refoperativa = emb_refoperativa
                	join liqbuques on buq_codbuque = emb_codvapor ";
                	
    $asBuques = fDBvalor($db, $slTablas, "distinct concat(buq_descripcion, '-', emb_numviaje)", $pQry);
    print_r($asBuques);
    foreach ($asBuques as $slNomb) {
            $gsTitGeneral .= $slNomb;
    }
*/
    $slSql = "CREATE TEMPORARY TABLE tmp_liquidaciones
                     SELECT pro_ID AS liq_NumProceso, liq_numliquida, liq_codrubro, sum(liq_valtotal * rub_IndDbCr) as tmp_valtotal
...
/****/
    ...
    $pTipo = (isset($_GET['pTipo']))? $_GET['pTipo']:30;
        /* $slSql ="SELECT tmp_productor AS 'C1',
                    ...
                    SUM(CASE WHEN 11 <= RUB_POSORDINAL and RUB_POSORDINAL <= 11 THEN tmp_valtotal ELSE 0 END) AS 'C12',
                    SUM(CASE WHEN 12 <= RUB_POSORDINAL and RUB_POSORDINAL <= 12 THEN tmp_valtotal ELSE 0 END) AS 'C13',
                    SUM(CASE WHEN 13 <= RUB_POSORDINAL and RUB_POSORDINAL <= 13 THEN tmp_valtotal ELSE 0 END) AS 'C14',
...
$rep->titleOpts=array('T'=>array('font'=>$slFontName, 'fontSize'=>10, 'justification'=>'center', 'leading'=>12),
                      ...
                      'C'=>array('font'=>$slFontName, 'fontSize'=>10 , 'justification'=>'center', 'leading'=>10 )); // Options for T=Title, S=Subtitle, C=Condition
