...
function &fDefineQry(&$db, $pQry=false){
    ...
    $alSql[] = "SELECT concat(conpersonas.per_apellidos, ' ', conpersonas.per_nombres) AS NOM,
                ...
                ORDER BY NOM, liq_numliquida, grupo, secuencia ";
    $rs= fSQL($db, $alSql);
    return $rs;
}
...
function before_group_COD (&$rpt, &$group) {
    ...
    if ($rs) {
        while (!$rs->EOF) {
            ...
            $sltxt .= str_repeat(' ',$i) . ' FECHA: ' . $group->lastRec['FEC'];
            ...
            $sltxt2 .= str_repeat(' ',$i) . ' SEMANA: ' . $group->lastRec['SEMANA'] ;
...
//    $rpt->pdf->setLineStyle(1,'square');
    ...
    $y1 = 40;
    $l=80;
    $rpt->pdf->ezrRoundRectangle($x, 25, 515, 50, 30);
    $x+=10;
    for ($i=0; $i<3; $i++) {
...
if (fGetParam("pTipo", 30) == 30)
    $rep->title="BORRADOR DE LIQUIDACION" ; //"PER LIQUIDACION DE COMPRA DE FRUTA";  #fah15/08/08 #fah05/10/08
else
