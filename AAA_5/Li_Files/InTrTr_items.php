...
//Open Method @59-B2B51B65
    function Open()
    {
        ...
        $this->CountSQL = "SELECT COUNT(*) FROM (conactivos INNER JOIN concategorias ON concategorias.cat_CodAuxiliar = conactivos.act_CodAuxiliar)  " .
        "                 INNER JOIN genunmedida ON conactivos.act_UniMedida = genunmedida.uni_CodUnidad " .
        "WHERE act_Descripcion LIKE '%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%' OR  " .
        "      act_Descripcion1 LIKE '%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%' OR  " .
        "      act_CodAuxiliar LIKE '%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%'";
        $this->SQL = "SELECT act_CodAuxiliar, cat_Activo, uni_Descripcion, concat(act_Descripcion,' ' , act_descripcion1)as act_descrip, " .
