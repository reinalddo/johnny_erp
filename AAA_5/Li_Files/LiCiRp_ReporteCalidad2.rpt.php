...
$sSql = "SELECT  c.tar_NumTarja AS Tarja
		...
		END AS Ubicacion
	       ...
	       ,mrc.par_Valor2 AS MarcaAbrv
	       
               /* ,GROUP_CONCAT(DISTINCT(mrc.par_Valor2)) AS TodasMarcas */
	       
	       ,(
	       SELECT GROUP_CONCAT(DISTINCT(CONCAT(mrc.par_Valor2, ' = ',mrc.par_Descripcion))SEPARATOR '<br>')
	       ...
	       WHERE e.emb_RefOperativa = ".$pEmbarque."
               ...
               ,cj.caj_Abreviatura AS EmpqAbrv	
	       ...
	       /* ,(ROUND(AVG(IFNULL(d.tad_Peso,0)),2) + ROUND(AVG(IFNULL(d.tad_ClusCaja,0)),2)) AS PesoTotal */ /*Peso total de la fruta y el empaque */
               ...
               /* ,ROUND(AVG(IFNULL(d.tad_ClusCaja,0)),2) AS PesoTotal PESO ANTERIOR - NUEVO PESO APLICADO POR PARAMETRO DE INVENTARIO/CALIDAD*/
	       ,ROUND(AVG(IFNULL(d.tad_Peso,0)+IFNULL(iQlt.par_Valor1,0)),2) AS PesoTotal
	       ,IFNULL(iQlt.par_Valor1,0) AS PesoEmpq /* GQLTY */
	       ...
	       /*****************************************************************************TEXTOS DEL REPORTE****************************************/
	       ,( SELECT REPLACE(ltr_txt1Desc,'\n','<br>') FROM liqTextoReporte
		  WHERE ltr_CodCliente = 0 /*predeterminado*/ AND c.tac_Semana >= ltr_semanaDesde
		  AND ltr_semanaDesde = ( SELECT MAX(ltr_semanaDesde) FROM liqTextoReporte WHERE ltr_CodCliente = 0 AND c.tac_Semana >= ltr_semanaDesde and ltr_visible = 1)/*maximo*/
	        ) AS Sec1Tx1
		,( SELECT ltr_txt1Titulo FROM liqTextoReporte
		  WHERE ltr_CodCliente = 0 /*predeterminado*/ AND c.tac_Semana >= ltr_semanaDesde
		  AND ltr_semanaDesde = ( SELECT MAX(ltr_semanaDesde) FROM liqTextoReporte WHERE ltr_CodCliente = 0 AND c.tac_Semana >= ltr_semanaDesde and ltr_visible = 1)/*maximo*/
	        ) AS Sec1Tt1
		
	       ,( SELECT REPLACE(ltr_txt2Desc,'\n','<br>') FROM liqTextoReporte
		  WHERE ltr_CodCliente = 0 /*predeterminado*/ AND c.tac_Semana >= ltr_semanaDesde
		  AND ltr_semanaDesde = ( SELECT MAX(ltr_semanaDesde) FROM liqTextoReporte WHERE ltr_CodCliente = 0 AND c.tac_Semana >= ltr_semanaDesde and ltr_visible = 1)/*maximo*/
	        ) AS Sec4Tx1
		,( SELECT ltr_txt2Titulo FROM liqTextoReporte
		  WHERE ltr_CodCliente = 0 /*predeterminado*/ AND c.tac_Semana >= ltr_semanaDesde
		  AND ltr_semanaDesde = ( SELECT MAX(ltr_semanaDesde) FROM liqTextoReporte WHERE ltr_CodCliente = 0 AND c.tac_Semana >= ltr_semanaDesde and ltr_visible = 1)/*maximo*/
	        ) AS Sec4Tt1
		
	        ,( SELECT REPLACE(ltr_txt3Desc,'\n','<br>') FROM liqTextoReporte
		  WHERE ltr_CodCliente = 0 /*predeterminado*/ AND c.tac_Semana >= ltr_semanaDesde
		  AND ltr_semanaDesde = ( SELECT MAX(ltr_semanaDesde) FROM liqTextoReporte WHERE ltr_CodCliente = 0 AND c.tac_Semana >= ltr_semanaDesde and ltr_visible = 1)/*maximo*/
	        ) AS Sec4Tx2
		,( SELECT ltr_txt3Titulo FROM liqTextoReporte
		  WHERE ltr_CodCliente = 0 /*predeterminado*/ AND c.tac_Semana >= ltr_semanaDesde
		  AND ltr_semanaDesde = ( SELECT MAX(ltr_semanaDesde) FROM liqTextoReporte WHERE ltr_CodCliente = 0 AND c.tac_Semana >= ltr_semanaDesde and ltr_visible = 1)/*maximo*/
	        ) AS Sec4Tt2
	...
	LEFT JOIN conpersonas eval ON eval.per_CodAuxiliar = c.tac_CodEvaluador
        ...
        LEFT JOIN genparametros iQlt ON iQlt.par_Clave = 'IQLTY' AND iQlt.par_Secuencia = 1 /*PESO DE LA CAJA -SE AUMENTA AL PESO NETO PARA SACAR EL PESO BRUTO(TOTAL)*/
...
$sSql .= " GROUP BY c.tac_UniProduccion/*hacienda*/,c.tac_Fecha/*fecha tarja*/, d.tad_CodMarca, cj.caj_Abreviatura /*d.tad_CodCaja*/   order by 1 ";
