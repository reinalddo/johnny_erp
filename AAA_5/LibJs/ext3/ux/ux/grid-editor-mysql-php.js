...
myNameSpace.myModule = function(){
    ...
    */
    var setupDataSource = function(){
        ...
        myReader = new Ext.data.JsonReader(	{root: 'results', 
...
//	        	url: 'sheldon.xml', //where to retrieve data
                url: myUrl, //url to data object (server side script)
                method: 'POST'
            }),   
            baseParams:{task: "readStock"},
            reader: myReader,
        ...
        dsCombo = new Ext.data.Store({
            proxy: new Ext.data.HttpProxy({
                url:   myUrl, //url to data object (server side script)
                method: 'POST'
            }),   
            baseParams:{task: "readCombo"},//this parameter is passed for any HTTP request
...
//            function renderPctChange(val){
	...
	///////////////////////////////////////////////////////////////////////////
			...
			dsCombo.load(); //industry column dropdown options
            ...
            colModel = new Ext.grid.ColumnModel([ //instantiate ColumnModel
                 {  
                    ...
                    editor: new Ext.form.DateField({ //DateField editor
                        ...
                        minValue: '10/15/07',
                        disabledDays: [0, 3, 6],
                        disabledDaysText: 'Cerrado en este dia'
                    })
                },{
			...
			this.pagingPlugin = new Ext.ux.Andrie.pPageSize({
				afterText: 'registros por pagina'
			});
        }//end if colModel
    ...
    */
    var buildGrid = function(){
                    ...
                    {   waitMsg: 'Grabando ..... ',
                        ...
                        callback: function (options, success, response) {
                            if (success) { //success will be true if the request succeeded
                                ...
                                Ext.MessageBox.alert('Lo siento, No se pudo ejecutar. [Q304]',response.responseText);
                            }
                        },
                        failure:function(response,options){  //the function to be called upon failure of the request (server script, 404, or 403 errors)
                            Ext.MessageBox.alert('Notificacion','Oops...');
        ...
        function handleDelete() {
            ...
            {
                Ext.MessageBox.confirm('Confirmacion','Realmente desea eliminar lo seleccionado?', deleteRecord);
            }
            else
            {
                Ext.MessageBox.alert('Notificacion','Por favor seleccione al menos un registro');
            }//end if/else block
        }; // end handleDelete 


		/**
		 * Handler to control grid editing: Determines the edited col, calls the action  
		 * @param {Object} oGrid_Event
		 */
		...
		function onCheckColumn(element, e, record) {
			...
			Ext.Ajax.request(
                { //ajax request configuration  
                    waitMsg: 'Grabando ...',
                    url: myUrl, //url to server side script
                    ...
                    failure:function(response,options){
                        Ext.MessageBox.alert('Alerta','Oops...');
                    },//end failure block                                      
                    success:function(response,options){
						//Ext.MessageBox.alert('Success','Yeah...');
                        if(checkID == 0){
...
//        var tpl = new Ext.XTemplate(
    ...
    var p = new Ext.Panel({
        title: 'Detalles',
        width: 300,
        html: '<p><i>Ver resutados aqui</i></p>',
        tbar: [
			{
	            text: 'Apply Data',
	            handler: function(){
...
*/			
    ...
    var renderGrid = function() {  
        grid.render('grid');//1st argument is the container,
