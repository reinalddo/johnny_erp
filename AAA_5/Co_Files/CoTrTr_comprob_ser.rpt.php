...
//
    $alSql[] = "SELECT
                   com_RegNumero AS 'REG',
            		...
            		concat(concat(IF(det_idauxiliar <> 0 ,
            				concat(IFNULL( concat(act_descripcion, ' ',
            					ifnull(act_descripcion1,' ')),
            						concat(left(pde.per_Apellidos,10), '. ', ifnull(left(pde.per_Nombres,10), ' '))
            				         ),
            					' : '
            				),
            				''
            			...
            			det_Glosa ),
            	       ' /',
            			concat(' ', ifnull(left(por.per_apellidos,12),''),
            				ifnull(left(por.per_nombres,12),'')
            			)
            	
                   	) AS 'DES',
                    IF(cue_reqrefoperat, concat(par_valor2, '-', ord_NUmDocum), det_NumCheque) as 'CHE',
                    det_ValDebito  AS 'VDB',
                    det_ValCredito AS 'VCR'
                 FROM concomprobantes JOIN condetalle on (det_RegNumero = com_RegNumero)
                    LEFT JOIN conpersonas pde ON (pde.per_CodAuxiliar = det_IdAuxiliar)
                    ...
                    LEFT JOIN concuentas ON (cue_codcuenta = det_codcuenta) 
        			  ...
        			  LEFT JOIN genparametros ON par_clave = 'VGTIP' AND par_secuencia = ord_tipo
                      LEFT JOIN  conpersonas por on por.per_codauxiliar = ord_Cliente" ;
...
//    $rpt->pdf->setLineStyle(1,'square');
    ...
    $y1 = $y+20;
    $l=80;
    $rpt->pdf->ezrRoundRectangle($x, $y+1, 605, $y+25, 15);
    $x+=10;
    for ($i=0; $i<3; $i++) {
    ...
    $rpt->pdf->addText($x+ 95,$yt,8," Revisado",$angle=0,$wordSpaceAdjust=0);
    $rpt->pdf->addText($x+190,$yt,8,"Autorizado",$angle=0,$wordSpaceAdjust=0);
...
$rep = new ezPdfReport($rs, array(21.8,14), "portrait", $slFontName, 10);
$rep->title=false;
//$rep->subTitle="-";
...
$rep->rptOpt = array('fontSize'=>10,  'titleFontSize' => 2, 'showHeadings'=>0,'shaded'=>0, 'splitRows'=>0,
                     'showLines'=>0, 'rowGap'=>0, 'maxWidth'=>990,  'xPos'=>-20, 'xOrientation'=>'right');

$rep->printColHead=false;
$rep->printFooter=true;
