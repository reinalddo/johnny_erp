...
if (fGetParam("init", false) != false) {
    ...
    $_SESSION[$gsSesVar]= "select com_regnumero regNum, d.det_secuencia secuencia, det_idauxiliar aux, per_Apellidos banco, det_NumCheque cheque, 
        ...
        join concomprobantes c on c.com_regnumero=det.com_regnum
			    ...
			    and cat_CodAuxiliar not in (103,104,110)
	...
	select  d.com_regnum
        ...
        and det.com_regnum not in 
	(
	select  d.com_regnum
        from concheques_cab c 
        inner join concheques_det d on c.idbatch=d.idbatch
        where tipo=3 /*and c.operacion = 2   */
	    and fecha between '".fGetParam("pFecIni", '2020-12-12')."' and '".fGetParam("pFecFin", '2020-12-12')."'
	)
        and {filter}
        ...
        order by banco,fecha,cheque, {sort} {dir}
        LIMIT {start}, {limit}        ";
    /*$_SESSION[$gsSesVar]=
