 ...
 *	@return		void
 *	@rev	esl	01/03/10 Modificacion del archivo para contabilización de transacciones de Comercio Exterior
...
function fContabPres($pRNum){
	...
	$trSql = "SELECT
		...
		com_Valor VALOR,
		t.tra_Cuotas,
		t.tra_Semana as tra_Semana_cab,
		d.tra_Cuota,
		d.tra_Fecha_vence,
		d.tra_Semana,
		com_emisor EMI,
		...
		li.par_valor3 as LIB
		    ...
		    JOIN genTransac t on t.tra_Id = ".$pTraId."
		    LEFT JOIN genTransacDetal d on d.tra_Id = t.tra_Id
	...
	ferr("<br> Paso c1" );
		...
		'det_numcheque' 	=>  $tra->tra_Semana_cab,
		'det_feccheque' 	=> 0,
		'det_codcuenta' 	=> '' ) ;
...
//	    $fNum+=1;
	    if ($tra->tra_Cuota > 0) {
		$lastrec['det_glosa'] = "Cuota No: ".$tra->tra_Cuota."/".$tra->tra_Cuotas;
		$lastrec['det_feccheque'] = $tra->tra_Fecha_vence;
		$lastrec['det_numcheque'] = $tra->tra_Semana;
	    }
...
function fGrabaComp($TComp){
    global $db, $pPro, $slUsuario, $pTraId, $ilRegNumero;
    
    // ************** leer los datos de la transaccion *******************
    $sltrans="SELECT tra_Id,
		     tra_Emisor,
		     tra_Receptor,
		     tra_Concepto,
		     tra_RefOperat,
    ...
    if (!fAgregaComprobante($db, $TComp, 0, 0, $fecComp, $fecComp,
                $fecComp, 0, $arrData["tra_Receptor"], 0,
                $arrData["tra_Concepto"]." (Trans.#:".$arrData["tra_Id"].")",
                $arrData["tra_Valor"], 1, $arrData["tra_Motivo"], 0, $arrData["tra_Semana"], 5, 1, 0,
 ...
 *	opcionalmente un tipo de comprobante
 ----------------------------------------------------------------------------------------------------------------------------**/
    ...
    $pTraId = fGetparam('pTra', false); //Numero de transaccion
    //-------------------------
    fGrabaComp($TComp);
