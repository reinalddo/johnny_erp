...
if (fGetParam("init", false) != false) {
    ...
    $_SESSION[$gsSesVar . "_count"]= "select count(*) as totalRecs
        ...
        where  tipo=1 and cab.operacion in (1) /*and det_ValCredito<>0*//*para que aparezcan cheques anulados para poner estatus anulado*/
    ...
    $_SESSION[$gsSesVar]= "select com_regnumero regNum, d.det_secuencia secuencia, det_idauxiliar aux, per_Apellidos banco, det_NumCheque cheque, 
        ...
        where  tipo=1 and cab.operacion in (1) /*and det_ValCredito<>0*//*para que aparezcan cheques anulados para poner estatus anulado*/
	and com_FecContab between '" . fGetParam("pFecIni", '2020-12-12') . "' and '" . fGetParam("pFecFin", '2020-12-12') . "'
	/*and com_regnumero not in 
	(
	 select  com_regnum-- , confirmado, tipo
        from concheques_cab c 
        inner join concheques_det d on c.idbatch=d.idbatch
        where tipo=1
		-- and fecha between '2009-12-01' and '2009-12-17'
		and operacion = 2
	)*/
        /*and cab.idbatch not in 
        (
        select c.idbatch -- max(c.idbatch)
        from concheques_cab c 
        inner join concheques_det d on c.idbatch=d.idbatch
        where tipo=1
	    -- and fecha between '" . fGetParam("pFecIni", '2020-12-12') . "' and '" . fGetParam("pFecFin", '2020-12-12') . "'
	    and operacion = 2
        -- group by com_regnum
	...
	/* 07/Mayo/2010 esl
	  Modificaciones en el select principal 
	  Para que filtre solo los emitidos:
	    where  tipo=1 and cab.operacion <> 3 and det_ValCredito<>0
	  Para que se refleje el cambio de estado
	    join conChequeEmi emi on c.com_regnumero=emi.com_regnum and cab.idbatch=emi.idbatch
	*/
    ...
    $loadJSFile = "CoTrTr_chequeEstadoConf";
