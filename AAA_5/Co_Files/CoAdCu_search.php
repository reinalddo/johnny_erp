...
//Open Method @255-D505B9AF
    function Open()
    {
        ...
        $this->SQL = "SELECT concuentas.Cue_ID AS Cue_ID, concuentas.Cue_CodCuenta AS Cue_CodCuenta, concuentas.cue_Descripcion AS cue_Descripcion, " .
                            ...
                            "abuelo.Cue_ID AS abu_Cue_ID, abuelo.cue_Descripcion AS abu_Descripcion, par_Clave, par_Descripcion, concat_ws(\"/\",  " .
                            "if(abuelo.cue_descripcion = '   Raiz'  ,'', abuelo.cue_descripcion) , " .
                    ...
                    "FROM (((concuentas padre RIGHT JOIN concuentas ON padre.Cue_ID = concuentas.Cue_Padre ) LEFT JOIN concuentas abuelo ON padre.Cue_Padre = abuelo.Cue_ID)) LEFT JOIN genparametros ON genparametros.par_Secuencia = concuentas.cue_TipAuxiliar  " .
                    "WHERE ( concuentas.Cue_ID > 1 AND par_Clave = 'CAUTI' ) AND ( concuentas.Cue_CodCuenta LIKE '" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%' OR concuentas.cue_Descripcion LIKE '%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%' ) " .
                    "";
