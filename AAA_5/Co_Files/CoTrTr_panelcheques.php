...
// Para la consulta del perfil del usuario
...
echo '<script languaje="JavaScript">
         var ses_empresa="'.$_SESSION['g_empr'].'";
      </script>';
...
// ***********************************************************
    /* esl 14/mayo/2010
    * Consultar que tipo de perfil tiene el usuario en la empresa.
    * CSUPE = Es el perfil de Supervisor de Contabilidad
    * TESR  = Es el perfil de TES Senior
    * Si Count(*) > 0 significa que el usuario tiene uno de estos perfiles en la empresa en que inicio sesion,
    * no se va a filtrar por usuario la consulta para la ubicacion de los cheques ni para confirmar la recepción.
    */
    
    $sSql = "select COUNT(*) as nPerfil from seguridad.segusperfiles 
	    JOIN seguridad.segempresas on emp_BaseDatos = '".$_SESSION['g_dbase']."' 
	    JOIN seguridad.segusuario on usu_login = '".$_SESSION['g_user']."' 
	    where usp_IDusuario = usu_IDusuario
	    and usp_IDempresa = emp_IDempresa
	    and usp_IDperfil in ('CSUPE', 'TESR');";
	    
    $rs = $db->execute($sSql);
    $r = $rs->fetchRow();
    
    $_SESSION["CoTrTr_nPerfil"] = $r['nPerfil'];
// ***********************************************************
...
$_SESSION["CoTrTr_estadoRep"] = "select par_Secuencia cod, par_Descripcion txt from genparametros
