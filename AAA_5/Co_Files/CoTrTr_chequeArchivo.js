...
Ext.onReady(function(){
    ...
    */
    ...
    name="Car";
    app.cheque.gridArchivo = new Ext.ux.AutoGridPanel({
        title:''
        ,plugins: [filters]
        //,id:'gridConciliacion'
        ,height: 200
    ...
    //if(!Ext.getCmp("gridConciliacion")){
        ...
        olPanel.add(app.cheque.gridArchivo);
    //}
    
    
    app.cheque.gridArchivo.store.load({params: {meta: true, start:0, limit:9999, sort:'com_FecContab', dir:'DESC'}});
...
function fGrabaEstado(){
    if (0 == app.cheque.gridArchivo.getSelectionModel().getCount()){
        Ext.Msg.alert('Alerta','Por favor seleccione cheque a autorizar');
        return;
    }
    //debugger;
...
function fConfirmarEstado(){
        //debugger;
    var alSel = app.cheque.gridArchivo.getSelectionModel().selections;
...
function fGrabarEstado(opc, detalles, total){
    ...
    Ext.Ajax.request({
		...
		var atributos = "status=0,width=40,height=45";
                if ("" != olRsp.bolivariano){
                    //window.open(olRsp.bolivariano);
		    window.open( "CoTrTr_archivobanco_bolivariano.rtp.php?pArc="+olRsp.bolivariano, "wPopUp", atributos );
		}
		    
		if ("" != olRsp.pichincha){
                    window.open("CoTrTr_archivobanco_pichincha.rtp.php?batch="+olRsp.ult, "wPopUp2", atributos);
		}
            }else{
