...
function &fDefineCompras(&$db, $pQry=false){
    ...
    $alSql =
	"SELECT ID, tipoTransac, codSustento, devIva, tid.tab_codSecuencial AS tpIdProv, per_Ruc as idProv,
			...
			codRetAir2, baseImpAir2, porcentajeAir2, valRetAir2,
			codRetAir3, baseImpAir3, porcentajeAir3, valRetAir3,
	...
	FROM fiscompras
			...
			LEFT JOIN genautsri   aut    ON aut.aut_ID = autorizacion
...
*/
    $alSql =
	"SELECT tipoTransac, tid.tab_codSecuencial AS tpIdCliente, per_Ruc as idCliente,
			tipoComprobante, DATE_FORMAT(fechaRegistro ,'%d/%m/%Y') AS fechaRegistro,
			1 as numeroComprobantes, DATE_FORMAT(fechaEmision ,'%d/%m/%Y') AS fechaEmision,
			baseImponible, ivaPresuntivo,
            baseImpGrav,porcentajeIva, montoIva,
...
function &fDefineExport(&$db, $pQry=false){
	...
	"SELECT tipoTransac, codSustento, tipImpExp as exportacionDe, 
            ...
            per_Ruc as idFiscalCliente, tid.tab_codSecuencial as tipoSujeto,
...
function &fDefineRecap(&$db, $pQry=false){
    $alSql =
	"SELECT tipoTransac, codSustento, tid.tab_codSecuencial AS tpIdProv, per_Ruc as idProv,
			tipoComprobante, DATE_FORMAT(fechaRegistro ,'%d/%m/%Y') AS fechaRegistro,
			establecimiento, puntoEmision, secuencial, autorizacion,
			DATE_FORMAT(aut.aut_FecVigencia,'%d/%m/%Y') AS fechaCaducidad,
			baseImponible, baseImpGrav,porcentajeIva, montoIva,
...
function fProcesaCampo($alRec, $olDato){	
	...
	$idx = strpos($slK, "Air");
	if ($idx >0) {
	    if (strlen($slK) > $idx + 3) $slK1 = substr($slK,0, strlen($slK) - 1);
	}
...
//				echo "$slV --- $slFmt ---";
				if ($slV == "0000/00/00" or $slV == "00/00/0000" or is_null($slV) or strlen($slV) < 1) {
...
//print_r($alCam);
$ilCount=0;
echo "<br> PROCESO DE IMPORTACIONES ----------------------------------------------<br>";
if ($rs)
	while ($alRec = $rs->FetchRow()) {
	    $airFl = false;
...
//print_r($alCam);
$ilCount=0;
echo "<br> PROCESO DE EXPORTACIONES ----------------------------------------------<br>";
if ($rs)
	while ($alRec = $rs->FetchRow()) {
	    $airFl = false;
...
//print_r($alCam);
$ilCount=0;
echo "<br> PROCESO DE RECAPS (TARJ. DE CREDITO) ---------------------------------------<br>";
if ($rs)
	while ($alRec = $rs->FetchRow()) {
	    $airFl = false;
...
//print_r($alCam);
$ilCount=0;
echo "<br> PROCESO DE FIDEICOMISOS ----------------------------------------------<br>";
if ($rs)
	while ($alRec = $rs->FetchRow()) {
	    $airFl = false;
...
//print_r($alCam);
$ilCount=0;
echo "<br> PROCESO DE RENDIMIENTOS FINANCIEROS ----------------------------------------------<br>";
if ($rs)
	while ($alRec = $rs->FetchRow()) {
	    $airFl = false;
