...
function &fDefineQry(&$db, $pQry=false){
    ...
    $alSql[] = "SELECT 	ID as ID, idProvFact COD, concat(pro.per_Apellidos, ' ', pro.per_Nombres) NOM,
						...
						BaseImpAir as BIR, codRetAir AS TIR, cra.tab_porcentaje as PIR, cra.tab_Descripcion AS DIR_, valRetAir as MIR,
...
function before_group_ID (&$rpt, &$group) {
    ...
    $y-=$flPaso;
    $rpt->putTextWrap( 20, $y, 400, 12       , strtoupper($group->lastRec['FAC']),  '', 'left', 0, 0);
...
//	    $rpt->putTextWrap($flC3, $y, 200, $ilFtSize, strtoupper($group->lastRec['DIB']),  '', 'left', 0, 0);
	    $rpt->putTextWrap($flC3, $y, 200, $ilFtSize, 'IVA BIENES ',  '', 'left', 0, 0);
            $rpt->putTextWrap($flC6, $y, 250, $ilFtSize, $group->lastRec['TIV'] ,  '', 'left', 0, 0);
	    $rpt->putTextWrap($flC4, $y, 30 , $ilFtSize, strtoupper($group->lastRec['PIB']),  '', 'right', 0, 0);
	    $rpt->putTextWrap($flC5, $y, 30, $ilFtSize-2, strtoupper($group->lastRec['MIB']),  '', 'right', 0, 0);
    }
    $y-=$flPaso;
...
//	    $rpt->putTextWrap($flC3, $y, 200,  $ilFtSize, strtoupper($group->lastRec['DIS']),  '', 'left', 0, 0);
	    $rpt->putTextWrap($flC3, $y, 200,  $ilFtSize, 'IVA SERVICIOS', 'left', 0, 0);
   	    $rpt->putTextWrap($flC6, $y, 250, $ilFtSize, $group->lastRec['TIS'] ,  '', 'left', 0, 0);
	    $rpt->putTextWrap($flC4, $y, 30 ,  $ilFtSize, strtoupper($group->lastRec['PIS']),  '', 'right', 0, 0);
	    $rpt->putTextWrap($flC5, $y, 30,  $ilFtSize, strtoupper($group->lastRec['MIS']),  '', 'right', 0, 0);
    }
    $y-=$flPaso;
...
//	    $rpt->putTextWrap($flC1, $y, 250,  $ilFtSize, strtoupper($group->lastRec['TIR']),  '', 'left', 0, 0);
	    ...
	    $rpt->putTextWrap($flC3, $y, 250,  $ilFtSize, 'RENTA  ' ,  '', 'left', 0, 0);
            $rpt->putTextWrap($flC6, $y, 250,  $ilFtSize, $group->lastRec['TIR'] ,  '', 'left', 0, 0);
	    $rpt->putTextWrap($flC4, $y, 30 ,  $ilFtSize, strtoupper($group->lastRec['PIR']),  '', 'right', 0, 0);
	    $rpt->putTextWrap($flC5, $y, 30,  $ilFtSize, strtoupper($group->lastRec['MIR']),  '', 'right', 0, 0);
    }
    $y-=$flPaso * 3;
    ...
    $rpt->putTextWrap($flC5, $y, 30,  10, $group->lastRec['TOT'],  $pLead = 0, $pJust = 'right', $pAng = 0, $pTest=0);

    $slCant = "**** SON: " . strtoupper(num2letras($flValor, true, 2, 2, " US Dolares", " ctvs. ****"));
