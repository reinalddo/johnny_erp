...
Ext.onReady(function(){
    ...
    /* Función para las validaciones iniciales: */   
    iniVal();
...
function VerMantAnexo(){
    ...
    olTipoTransac = {xtype: 'genCmbBox'
           ,sqlId: 'CoRtTr_tipoTrans'
    ...
    olSustento = {xtype: 'genCmbBox'
           ,sqlId: 'CoRtTr_sustento'
	   ,fieldLabel:'Cod Sustento'
           ,id: 'txt_sustento'
	   ...
	   ,listeners: {'select': function (combo,record){
					frmMant.findById("codigosSustento").setValue(this.getXmlField('codigos'));
			}}
    }
    
    olProveedor = {xtype: "genCmbBox"
	,fieldLabel:'Proveedor'
	,id:'txtProv'
	,hiddenName:'codProv'
	,minChars: 2
	,sqlId: 'CoRtTr_proveedor'
	,tabindex:3
	,listeners: {'select': function (combo,record){
		frmMant.findById("txtProvFact").setValue(this.getXmlField('cod'));
		frmMant.findById("tpIdProvFact").setValue(this.getXmlField('tipo'));
		frmMant.findById("txt_rucProv").setValue(this.getXmlField('ruc'));
		frmMant.findById("txt_rucProvFact").setValue(this.getXmlField('ruc'));
		fValidaRuc();
	    }}
	};
	
    olProveedorFact = {xtype: "genCmbBox"
    ...
    olIvaGrabable = {xtype: "genCmbBox"
	,id:'txt_IVA'
	,hiddenName:'porcentajeIva'
	,minChars: 1
	,sqlId: 'CoRtTr_porcIVA'
	,tabindex: 152
	,listeners: {'select': function (combo,record){
					//debugger;
					Ext.getCmp("porc_iva").setValue(this.getXmlField('porc'));
    ...
    olICE = {xtype: "genCmbBox"
	,id:'txt_ICE'
	,hiddenName:'porcentajeIce'
	,minChars: 1
	,sqlId: 'CoRtTr_porcICE'
	,tabindex:156
	,listeners: {'select': function (combo,record){
					//debugger;
					//frmMant.findById("porc_iva").setValue(record.data.porc);
					Ext.getCmp("porc_ice").setValue(this.getXmlField('porc'));
    ...
    olRetIVAbienes = {xtype: "genCmbBox"
	,id:'txt_retIvaBienes'
	,hiddenName:'porRetBienes'
	,minChars: 1
	,sqlId: 'CoRtTr_porcRetIvaBienes'
	,tabindex:201
	,listeners: {'select': function (combo,record){
					//debugger;
					Ext.getCmp("porc_ret_iva_b").setValue(this.getXmlField('porc'));
    ...
    olRetIVAserv = {xtype: "genCmbBox"
	,id:'txt_RetIvaServ'
	,hiddenName:'porRetServicios'
	,minChars: 1
	,sqlId: 'CoRtTr_porcRetIvaServ'
	,tabindex:204
	,listeners: {'select': function (combo,record){
					debugger;
					//frmMant.findById("porc_iva").setValue(record.data.porc);
					Ext.getCmp("porc_ret_iva_s").setValue(this.getXmlField('porc'));
    ...
    olRetCodFuente1 = {xtype: "genCmbBox"
	,id:'txt_codRet1'
	,hiddenName:'codRetAir'
	,minChars: 1
	,sqlId: 'CoRtTr_codRetencion'
	,tabindex:210
	,listeners: {'select': function (combo,record){
					Ext.getCmp("porcentajeAir").setValue(this.getXmlField('porc'));
    ...
    olRetCodFuente2 = {xtype: "genCmbBox"
	,id:'txt_codRet2'
	...
	,listeners: {'select': function (combo,record){
					//debugger;
					Ext.getCmp("porcentajeAir2").setValue(this.getXmlField('porc'));
    ...
    olRetCodFuente3 = {xtype: "genCmbBox"
	,id:'txt_codRet3'
	...
	,listeners: {'select': function (combo,record){
					//debugger;
					Ext.getCmp("porcentajeAir3").setValue(this.getXmlField('porc'));
    ...
    olAutTipoDoc = {xtype: "genCmbBox"
	,fieldLabel:'Tipo Documento'
	,id:'txt_tipoDoc'
	,hiddenName:'aut_TipoDocum'
	,minChars: 1
	,sqlId: 'CoRtTr_tipoDoc'
//	,tabindex:4
	};
    ...
    var olFechaReg ={
			...
			,listeners: {'change': function (field,newValue,oldValue){
					//debugger;
					var fecha_emi = Ext.getCmp("fechaEmiRet1").getValue();
					
					if (newValue != fecha_emi)
					Ext.getCmp("fechaEmiRet1").setValue(newValue);
				    }}
                    } ;
    ...
    var olEmbarque = new Ext.form.ComboBox({
			...
			forceSelection: true,
			emptyText:'',
			allowBlank:     false, //Se lo comento para hacer prueba en la empresa comercial
			listWidth:      350 ,
			tabindex:3
   ...
   olCtaGasto = {xtype: 'genCmbBox'
    ...
    olCtaAux = {xtype: 'genCmbBox'
           ,sqlId: 'CoRtTr_CtaGastoAux'
           ,id: 'txt_CtaAuxiliar'
    ...
    olTab.add({
	id: 'tabGastos',
        title:'Cuenta de Gasto'
    }).show();
    
 
     /*   
    var regNum="799";
    var gUrl= "CoRtTr_GridAnexo.php?init=1";
...
*/
	...
	//Ext.Msg.alert('test','das ist ein test');
        ...
        //Ext.getCmp("tabGastos").add(Ext.getCmp("gridDetalle"))
	//gridDetalle.crearGrid(); 
  ...
  */
  ...
  if ("N" == app.CoRtTr.compValido){
    ...
    */    
    ...
    // Valida que la fecha de emisión no sea mayor a la fecha actual:
...
*/
	...
	totRet = totRet + base_ret_s;
    if (totRet != montoIVA){
	Ext.MessageBox.alert('Mensaje', 'La Base imponible de Retenciones de IVA (' + totRet + ') debe ser igual al monto total de IVA '+montoIVA);
	return anexoValido;
    }
    ...
    var base_ret_1 = Ext.getCmp("frmMant").findById("tabDatos").items.itemAt(1).items.itemAt(1).items.item("baseImpAir").getValue();
    var base_ret_2 = Ext.getCmp("frmMant").findById("tabDatos").items.itemAt(1).items.itemAt(1).items.item("baseImpAir2").getValue();
    var base_ret_3 = Ext.getCmp("frmMant").findById("tabDatos").items.itemAt(1).items.itemAt(1).items.item("baseImpAir3").getValue();
    var totRet2 = 0;
    
    var BASE0 = Ext.getCmp("frmMant").findById("tabDatos").items.itemAt(0).items.itemAt(0).items.item("baseImponible").getValue();
    var BASEIMP = Ext.getCmp("frmMant").findById("tabDatos").items.itemAt(0).items.itemAt(0).items.item("baseImpGrav").getValue();
    var montoImp = 0;
    
    if (BASE0 != "")
        montoImp = BASE0;
    if (BASEIMP != "")
        montoImp = montoImp + BASEIMP;
    
    if (base_ret_1 != "")
	totRet2 = base_ret_1;
    if (base_ret_2 != "")
	totRet2 = totRet2 + base_ret_2;
    if (base_ret_3 != "")
	totRet2 = totRet2 + base_ret_3;
    
    if (totRet2 != montoImp){
    	Ext.MessageBox.alert('Mensaje', 'La Base imponible de Retenciones en la Fuente ('+totRet2+') debe ser igual al monto total de Base Imponible ('+montoImp+')');
	return anexoValido;
    }
...
function fMantAutorizacion(){
    ...
    olAutProveedor = {xtype: "genCmbBox"
		    ,id:'txtAutProv'
...
function validaBaseRetencion(){
    // Validar Base Imponible para Retenciones en la fuente:
					var base_ret_1 = Ext.getCmp("baseImpAir").getValue();
					var base_ret_2 = Ext.getCmp("baseImpAir2").getValue();
					var base_ret_3 = Ext.getCmp("baseImpAir3").getValue();
					var totRet2 = 0;
					var BASE0 = Ext.getCmp("baseImponible").getValue();
					var BASEIMP = Ext.getCmp("baseImpGrav").getValue();
					var montoImp = 0;
					...
					if (base_ret_3 != "")
					    totRet2 = totRet2 + base_ret_3;
					if (totRet2 > montoImp){
					    Ext.MessageBox.alert('Mensaje', 'La Base imponible de Retenciones en la Fuente ('+totRet2+') no debe ser mayor al monto total de Base Imponible ('+montoImp+')');
					    //return false;
					    return false;
					}else{
					    return true;
